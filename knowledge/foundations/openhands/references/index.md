...
- `./conversation-stream-kernel.md` — How does a client subscribe to an agent event stream without re-receiving preloaded history, and survive refetch/reconnect races?
- `./websocket-reconnect-ladder.md` — How do parallel sockets retry without lockstep thundering-herd, stale sockets clobbering fresh ones, or hung handshakes blocking the host?
- `./streaming-delta-batcher.md` — How do per-token stream deltas commit at most once per frame while never letting durable events overtake their own streamed text?
- `./event-store-dedup-projection.md` — How does a global event store dedup replayed backlogs, stay O(1) under token floods, and swap conversations atomically?
- `./optimistic-user-message-queue.md` — How does an optimistic "Sending…" bubble get cleared by the right server echo and never hang forever?
- `./automation-cron-preset-mapping.md` — How can a schedule editor round-trip cron strings to friendly presets without silently rewriting unsupported expressions?
- `./client-tool-roundtrip-protocol.md` — How does a web UI execute an agent tool call locally, report the result back through the conversation, and stay exactly-once across event-stream replays?
- `./dev-stack-launch-orchestrator.md` — How does an npm-launched full-stack script start a Python server + Vite frontend safely against port conflicts, cached wheels, reaped tmp sockets, and hard-killed predecessors?
- `./read-only-terminal-projection.md` — How is a live agent terminal rendered as a disposable xterm projection of an event-sourced command store without fit crashes or remount history loss?
- `./run-health-fanout-polling.md` — How does an N-card dashboard poll every item's latest run within a bounded request budget and degrade to "unknown" instead of lying?
- `./backend-split-automation-service.md` — How does one thin REST client serve a local sidecar and a cloud host through one class, and keep an import's POST→PATCH→cleanup pinned to one backend?
- `./automation-cache-scope-invalidation.md` — How should query keys be scoped so backend switches refire fetches automatically while one invalidation busts every pagination slice?
- `./msw-contract-fixture-fleet.md` — How does a browser mock fleet ship zero prod bytes, install before any query, and stay byte-faithful to a published API contract?
- `./desktop-boot-splash-chain.md` — How does an Electron shell start a local full stack with staged readiness, a startup-log splash, and cross-platform cleanup of detached children?
- `./agent-server-version-gate.md` — Where should frontend/backend compatibility be enforced, against which host, and how do "too old", "unknown version", and "unreachable" differ?
- `./start-conversation-payload-translation.md` — How does an adapter build one conversation-start payload for inline agents, server profiles, and ACP subprocesses without leaking secrets or tools across paths?
- `./client-event-dispatch-pipeline.md` — In a socket handler that projects events AND fires non-idempotent effects, where must replay dedup sit?
- `./backend-selection-kernel.md` — How does a multi-backend client pick ONE active backend per tab so cloud selections never strand local-only callers and cmd-clicked links land on their owning backend?
- `./backend-health-disable-latch.md` — How does connectivity polling stop after repeated failures without lying about "connected", survive page refreshes, and re-arm only when the connection config actually changes?
- `./git-controls-prompt-injection-plane.md` — How does a chat UI drive clone/push/PR actions through an agent via authored prompts while keeping optimistic bubbles honest about send failure?
- `./workspace-file-content-plane.md` — How does a browser authenticate top-level iframe/img requests against a workspace fileserver and classify bytes without downloading streamable kinds?
- `./acp-auth-status-probe-plane.md` — How can a GUI detect provider-CLI logins on the agent-server host, and what may it claim when detection cannot run?
- `./command-menu-manifest-registry.md` — How should a ⌘K palette resolve copy between i18n keys and external manifests and keep feature entries admission-gated?
...
- [`acp-auth-status-probe-plane.md`](./acp-auth-status-probe-plane.md)
- [`agent-server-version-gate.md`](./agent-server-version-gate.md)
- [`automation-cache-scope-invalidation.md`](./automation-cache-scope-invalidation.md)
- [`automation-cron-preset-mapping.md`](./automation-cron-preset-mapping.md)
- [`backend-health-disable-latch.md`](./backend-health-disable-latch.md)
- [`backend-selection-kernel.md`](./backend-selection-kernel.md)
- [`backend-split-automation-service.md`](./backend-split-automation-service.md)
- [`client-event-dispatch-pipeline.md`](./client-event-dispatch-pipeline.md)
- [`client-tool-roundtrip-protocol.md`](./client-tool-roundtrip-protocol.md)
- [`command-menu-manifest-registry.md`](./command-menu-manifest-registry.md)
- [`conversation-stream-kernel.md`](./conversation-stream-kernel.md)
- [`desktop-boot-splash-chain.md`](./desktop-boot-splash-chain.md)
- [`dev-stack-launch-orchestrator.md`](./dev-stack-launch-orchestrator.md)
- [`event-store-dedup-projection.md`](./event-store-dedup-projection.md)
- [`git-controls-prompt-injection-plane.md`](./git-controls-prompt-injection-plane.md)
- [`msw-contract-fixture-fleet.md`](./msw-contract-fixture-fleet.md)
- [`optimistic-user-message-queue.md`](./optimistic-user-message-queue.md)
- [`read-only-terminal-projection.md`](./read-only-terminal-projection.md)
- [`run-health-fanout-polling.md`](./run-health-fanout-polling.md)
- [`start-conversation-payload-translation.md`](./start-conversation-payload-translation.md)
- [`streaming-delta-batcher.md`](./streaming-delta-batcher.md)
- [`websocket-reconnect-ladder.md`](./websocket-reconnect-ladder.md)
- [`workspace-file-content-plane.md`](./workspace-file-content-plane.md)
