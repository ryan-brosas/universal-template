...
- `./task-admission-concurrency-gate.md` — how to bound concurrent uploads without a semaphore while re-pumping on every completion.
- `./worker-task-state-machine-stop-ladder.md` — what Stop() must do in each task state and how terminal status is decided.
- `./sta-threadworker-context-marshaling.md` — dedicated STA thread per job with SynchronizationContext-posted events.
- `./thread-dowork-finally-cleanup-contract.md` — cleanup that must survive stop/failure (KeepImage latch, clipboard rollback, deferred delete).
- `./upload-retry-ladder.md` — fail-retry loop with inter-attempt sleep and stop gating.
- `./after-upload-job-pipeline.md` — ordered URL post-processing chain gated on IsURLExpected.
- `./uploader-factory-config-gate.md` — destination resolution: filter override → CheckConfig → CreateUploader, errors as results not exceptions.
- `./recent-task-bounded-queue.md` — lock-guarded bounded recent-items queue with settings persistence.

### Entry/dispatch plane (pass 2)
- `./executejob-safe-settings-dispatch.md` — one-time GetSafeTaskSettings normalization before the HotkeyType switch; filePath-vs-picker duality.
- `./cli-command-dispatch-chain.md` — ordered consume-or-fall-through CLI checker chain; named-workflow settings resolved once; bad args consumed with log.
- `./clipboard-content-type-ladder.md` — Image→Text→FileDropList priority ladder; ExternalException user-consented recursive retry.
- `./processtextupload-url-triage.md` — valid-URL three-way flag gate (download/shorten/share) with early returns; folder-index special case.
- `./runimagetask-deferred-start-recursion.md` — interactive pre-upload gates via skip-flag recursion; single creation closure; dismissal still progresses.
- `./worker-task-factory-null-contract.md` — factories return null on load failure; TaskManager.Start owns the null guard; History tasks are display-only.
- `./native-messaging-consume-once.md` — JSON file IPC deleted in finally regardless of parse outcome; per-branch field validation.
- `./finddatatype-extension-triage.md` — settings-driven extension triage to Image/Text/File at factory time, File as terminal default.
...
- [`after-upload-job-pipeline.md`](./after-upload-job-pipeline.md)
- [`cli-command-dispatch-chain.md`](./cli-command-dispatch-chain.md)
- [`clipboard-content-type-ladder.md`](./clipboard-content-type-ladder.md)
- [`executejob-safe-settings-dispatch.md`](./executejob-safe-settings-dispatch.md)
- [`finddatatype-extension-triage.md`](./finddatatype-extension-triage.md)
- [`native-messaging-consume-once.md`](./native-messaging-consume-once.md)
- [`processtextupload-url-triage.md`](./processtextupload-url-triage.md)
- [`recent-task-bounded-queue.md`](./recent-task-bounded-queue.md)
- [`runimagetask-deferred-start-recursion.md`](./runimagetask-deferred-start-recursion.md)
- [`sta-threadworker-context-marshaling.md`](./sta-threadworker-context-marshaling.md)
- [`task-admission-concurrency-gate.md`](./task-admission-concurrency-gate.md)
- [`thread-dowork-finally-cleanup-contract.md`](./thread-dowork-finally-cleanup-contract.md)
- [`upload-retry-ladder.md`](./upload-retry-ladder.md)
- [`uploader-factory-config-gate.md`](./uploader-factory-config-gate.md)
- [`worker-task-factory-null-contract.md`](./worker-task-factory-null-contract.md)
- [`worker-task-state-machine-stop-ladder.md`](./worker-task-state-machine-stop-ladder.md)
