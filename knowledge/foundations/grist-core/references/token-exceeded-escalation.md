...
**Probe:** `bash -c 'cd $REFERENCE_ROOT/platforms/grist-core && sed -n "270,290p" app/server/lib/OpenAIAssistantV1.ts | grep -n "TokensExceededError\|this._model" && grep -n "includeAllTables: false" app/server/lib/OpenAIAssistantV1.ts'` → catch gates at relative :5-8/:16-18 and :308.
