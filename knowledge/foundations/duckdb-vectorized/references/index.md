...
- `./orrify-unified-format.md` — How do you read ANY vector without materializing it? (UnifiedVectorFormat / Orrify protocol, per-type free conversion.)
- `./selection-vector-identity.md` — How do you filter tuples without copying them? (Three-state SelectionVector, identity sentinel, sel composition.)
- `./reference-not-copy.md` — When does an operation alias buffers vs. copy? (Reference/Slice/Dictionary triad, deleted copy ctor, Flatten as the only logical-preserving mutation.)
- `./vector-layout-zoo.md` — Which encodings must an operator tolerate? (BufferType×VectorType matrix, nested child recursion rules.)
- `./constant-folding-execution.md` — When can a whole chunk be computed once? (all-constant fold protocol, VOLATILE veto, cardinality restore.)
- `./case-selection-threading.md` — How does CASE run each branch only on tuples that reach it? (WHEN funnel, FillSwitch scatter, short-circuit semantics.)
- `./conjunction-select-funnel.md` — How does an AND/OR chain evaluate each predicate on the shrinking survivor set? (AND/OR duality, sort-on-OR rule.)
- `./adaptive-filter-permutation.md` — How do conjunctions learn their cheapest predicate order at runtime? (Swap/observe/revert loop, CanThrow veto.)
- `./select-vs-execute.md` — Why do predicates have a Select entry point? (Native select paths, outer-space index mapping, NO_NULL template arms.)
- `./dictionary-reuse-optimization.md` — How is a scalar function evaluated once per dictionary VALUE? (Eligibility gates, same-sel re-emission, id-keyed cache.)
- `./dictionary-entry-identity.md` — How does dictionary identity survive re-encoding? (id + global_dictionary contract through Reinterpret.)
- `./data-chunk-reset-caching.md` — How do executors reuse buffers across millions of chunks allocation-free? (Parallel VectorCache slots, type-copy rationale, Reset contract.)
- `./constant-null-semantics.md` — How are constant NULLs represented and propagated into STRUCT/ARRAY children?
- `./debug-vector-verification.md` — How does DuckDB stress operators against adversarial encodings in debug builds?
...
- [`adaptive-filter-permutation.md`](./adaptive-filter-permutation.md)
- [`case-selection-threading.md`](./case-selection-threading.md)
- [`conjunction-select-funnel.md`](./conjunction-select-funnel.md)
- [`constant-folding-execution.md`](./constant-folding-execution.md)
- [`constant-null-semantics.md`](./constant-null-semantics.md)
- [`data-chunk-reset-caching.md`](./data-chunk-reset-caching.md)
- [`debug-vector-verification.md`](./debug-vector-verification.md)
- [`dictionary-entry-identity.md`](./dictionary-entry-identity.md)
- [`dictionary-reuse-optimization.md`](./dictionary-reuse-optimization.md)
- [`orrify-unified-format.md`](./orrify-unified-format.md)
- [`reference-not-copy.md`](./reference-not-copy.md)
- [`select-vs-execute.md`](./select-vs-execute.md)
- [`selection-vector-identity.md`](./selection-vector-identity.md)
- [`vector-layout-zoo.md`](./vector-layout-zoo.md)
