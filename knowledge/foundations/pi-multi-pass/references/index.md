...
- `./config-normalization-plane.md` — how does multi-account config stay entry-stable across corrupt files and env overlays?
- `./project-allowlist-policy.md` — how does per-project policy constrain subs, pools, and chains without mutating global config?
- `./member-eligibility-cooldown.md` — who is eligible right now, and when does a cooled-down member return?
- `./forward-only-failover-plan.md` — how are failover candidates ordered with an auditable skip taxonomy?
- `./strategy-reorder-failopen.md` — how do quota/schedule/custom selection strategies degrade without breaking failover?
- `./cascade-turn-boundary.md` — how does failover survive internal replay retries without re-attempting exhausted accounts?
- `./quota-snapshot-parsing.md` — how does untrusted provider quota JSON become a worst-case health signal that never throws?
- `./preset-switch-resolution.md` — how do account switches preserve model id and presets fall back without partial state?
- `./pool-health-status-reporting.md` — how do you render live rotation health that never disagrees with actual selection behavior?
- `./quota-dispatch-plane.md` — how do you discover checkable accounts from a checker registry and fan out concurrent health checks that never reject as a batch?
- `./provider-clone-registration.md` — how do N accounts of one base provider become distinct registered providers sharing one template implementation?
- `./pool-reference-integrity-writes.md` — how do pool rename/delete keep every chain reference coherent with auditable counts?
- `./subscription-teardown-cascade.md` — in what order do auth, registration, membership, chains, and the config entry tear down when one account is deleted?
- `./create-validation-gates.md` — what must be validated against live config before a new pool or chain may persist?
...
- [`cascade-turn-boundary.md`](./cascade-turn-boundary.md)
- [`config-normalization-plane.md`](./config-normalization-plane.md)
- [`create-validation-gates.md`](./create-validation-gates.md)
- [`forward-only-failover-plan.md`](./forward-only-failover-plan.md)
- [`member-eligibility-cooldown.md`](./member-eligibility-cooldown.md)
- [`pool-health-status-reporting.md`](./pool-health-status-reporting.md)
- [`pool-reference-integrity-writes.md`](./pool-reference-integrity-writes.md)
- [`preset-switch-resolution.md`](./preset-switch-resolution.md)
- [`project-allowlist-policy.md`](./project-allowlist-policy.md)
- [`provider-clone-registration.md`](./provider-clone-registration.md)
- [`quota-dispatch-plane.md`](./quota-dispatch-plane.md)
- [`quota-snapshot-parsing.md`](./quota-snapshot-parsing.md)
- [`strategy-reorder-failopen.md`](./strategy-reorder-failopen.md)
- [`subscription-teardown-cascade.md`](./subscription-teardown-cascade.md)
