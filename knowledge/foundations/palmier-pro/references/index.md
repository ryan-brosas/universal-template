...
- `./agent-run-loop-contract.md` — how does a turn loop survive cancellation, refusal, and half-finished tool calls without corrupting the conversation?
- `./stream-snapshot-coalescing.md` — how do token deltas become UI snapshots without flooding the main actor?
- `./anthropic-sse-fold.md` — how does a raw Anthropic SSE byte stream fold into provider-neutral events?
- `./tool-executor-envelope.md` — what must wrap every tool call: gates, read fencing, diff publication, telemetry?
- `./tool-arg-decode-no-trap.md` — how are LLM-supplied arguments decoded without trapping on overflow or unknown keys?
- `./short-id-roundtrip.md` — how do 8-char ids shown to the model resolve back to full UUIDs safely?
- `./dual-origin-mcp-dispatch.md` — how does one executor serve both the in-app chat and external MCP clients?
- `./skill-progressive-disclosure.md` — how are skills indexed into the prompt without loading bodies?
- `./save-queue-serialization.md` — how do overlapping NSDocument saves serialize without writing a torn or stale snapshot?
- `./async-write-main-unblock.md` — how does an off-main document write avoid deadlocking the AppKit main thread?
- `./manifest-load-failure-preservation.md` — how does a save avoid clobbering an unreadable sidecar file with an empty replacement?
- `./package-mutation-coordinator.md` — how do side-effecting media commits wait for in-flight saves without deadlocking or corrupting the package?
- `./close-save-drain.md` — how do you guarantee the final save persists unedited-state changes and that a failed close-save refuses the close?
- `./staged-media-commit-rebase-retry.md` — how does a media file land in the project package even while the project is being renamed mid-save?
- `./registry-standardized-url-ledger.md` — what happens to registry mutations that race its async disk load, and what is the identity key?
- `./package-layout-atomic-writes.md` — what write order keeps a directory-bundle document internally consistent across saves and Save As?
...
- [`agent-run-loop-contract.md`](./agent-run-loop-contract.md)
- [`anthropic-sse-fold.md`](./anthropic-sse-fold.md)
- [`async-write-main-unblock.md`](./async-write-main-unblock.md)
- [`close-save-drain.md`](./close-save-drain.md)
- [`dual-origin-mcp-dispatch.md`](./dual-origin-mcp-dispatch.md)
- [`manifest-load-failure-preservation.md`](./manifest-load-failure-preservation.md)
- [`package-layout-atomic-writes.md`](./package-layout-atomic-writes.md)
- [`package-mutation-coordinator.md`](./package-mutation-coordinator.md)
- [`registry-standardized-url-ledger.md`](./registry-standardized-url-ledger.md)
- [`save-queue-serialization.md`](./save-queue-serialization.md)
- [`short-id-roundtrip.md`](./short-id-roundtrip.md)
- [`skill-progressive-disclosure.md`](./skill-progressive-disclosure.md)
- [`staged-media-commit-rebase-retry.md`](./staged-media-commit-rebase-retry.md)
- [`stream-snapshot-coalescing.md`](./stream-snapshot-coalescing.md)
- [`tool-arg-decode-no-trap.md`](./tool-arg-decode-no-trap.md)
- [`tool-executor-envelope.md`](./tool-executor-envelope.md)
