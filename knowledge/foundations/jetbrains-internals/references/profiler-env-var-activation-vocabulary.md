...
**Probe:** deterministic content probes executed this pass on `$REFERENCE_ROOT/dotmemory/JetBrains.Profiler.Windows.SysTools.xml`: `grep -c "PUBLIC FOR 3-RD CUSTOMERS"` = **6** (prefix field + 5 transparent-set members), `grep -c "No that variable == no log"` = **2**, `grep -c "for chain integration"` = **5**, `grep -c "(x64 only) / .NET 5.0+"` = **2** — all verified against the full 270-line read.
