...
- `./event-parse-gate.md` — how do untrusted JSONL lines become validated events without ever leaking protected ones into memory-visible output?
- `./fail-soft-shard-loader.md` — how does a caller load the newest N events across daily shards without any error path escaping?
- `./newest-first-id-dedup.md` — how do you stop reading once you hold the newest N unique events across many files?
- `./append-cache-trailing-bytes.md` — how does a read cache stay correct when a writer appends mid-line between reads?
- `./atomic-privacy-scrub.md` — how do you physically delete protected records from shard files without corrupting or losing malformed evidence?
- `./sticky-privacy-boundary-filter.md` — how do you redact a *time interval* of activity, not just single events, while keeping the timeline shape readable?
- `./gap-duration-boundary-segmentation.md` — where are the exact episode cut points (idle gap, max span, sleep/wake latch, privacy boundary), and which episode owns the terminator event?
- `./episode-id-stability-digest.md` — what must an episode id be derived from so growing episodes never orphan already-persisted derived records?
- `./work-window-trim-coalesce.md` — which context events survive into an episode, and when do repeated passive events collapse before summarization?
- `./task-context-switch-signal.md` — when does a different-app or same-app context change start a NEW task without shattering rapid multi-app workflows?
- `./pending-episode-admission-gate.md` — when is an episode ready to be summarized exactly once, and which inference failures skip one item vs abort the batch?
- `./claim-ceiling-evidence-packet.md` — how do you convert telemetry into LLM briefs whose title verbs can never exceed the observed evidence?
- `./derived-store-privacy-reconciliation.md` — after purging protected raw records, how do you cascade deletion through tiered derived stores by reproducing provenance?
- `./owned-data-directory.md` — how do I claim, adopt, and delete an on-disk data directory without ever touching one we don't own.
...
- [`append-cache-trailing-bytes.md`](./append-cache-trailing-bytes.md)
- [`atomic-privacy-scrub.md`](./atomic-privacy-scrub.md)
- [`claim-ceiling-evidence-packet.md`](./claim-ceiling-evidence-packet.md)
- [`derived-store-privacy-reconciliation.md`](./derived-store-privacy-reconciliation.md)
- [`episode-id-stability-digest.md`](./episode-id-stability-digest.md)
- [`event-parse-gate.md`](./event-parse-gate.md)
- [`fail-soft-shard-loader.md`](./fail-soft-shard-loader.md)
- [`gap-duration-boundary-segmentation.md`](./gap-duration-boundary-segmentation.md)
- [`newest-first-id-dedup.md`](./newest-first-id-dedup.md)
- [`owned-data-directory.md`](./owned-data-directory.md)
- [`pending-episode-admission-gate.md`](./pending-episode-admission-gate.md)
- [`sticky-privacy-boundary-filter.md`](./sticky-privacy-boundary-filter.md)
- [`task-context-switch-signal.md`](./task-context-switch-signal.md)
- [`work-window-trim-coalesce.md`](./work-window-trim-coalesce.md)
