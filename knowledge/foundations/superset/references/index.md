...
- `./df-payload-acquisition-ladder.md` — how one query acquires its dataframe payload with timing kept outside the payload.
- `./stale-cache-shape-miss-downgrade.md` — when a loaded cache entry is deliberately demoted to a miss.
- `./query-cache-manager-fault-tolerance.md` — backend-outage-to-miss fall-through, force_cached failure, and never-cache-failure gating.
- `./annotation-cache-context-binding.md` — binding co-cached annotation data to requesting user and per-source RLS.
- `./contribution-totals-two-phase-sync.md` — totals-query detection, injection, and cache_values re-sync.
- `./cache-key-contribution-exclusion.md` — QueryObject.cache_key normalization rules that keep keys stable across workers.
- `./cache-timeout-precedence-ladder.md` — five-step timeout resolution incl. the native-filter-options override trap.
- `./grouping-sets-emulation-fallback.md` — per-rollup-level fan-out when an engine lacks native GROUPING SETS.
- `./report-state-machine-dispatch.md` — persisted-last_state keyed selection of exactly one state class; unknown states fail loudly.
- `./working-state-concurrency-refusal.md` — duplicate-execution guard: timeout recovery vs refused recompute with a side audit row.
- `./create-log-working-row-promotion.md` — one execution ⇒ one log row via in-place promotion of the WORKING trigger row.
- `./retry-window-staleness-anchor.md` — crontab-window identity carried through task re-enqueues; stale-window reset and in-flight skip.
- `./retry-or-error-backoff-ladder.md` — boolean-outcome retry funnel with capped exponential backoff and never-masked originals.
- `./error-grace-notification-dedup.md` — one owner email per failure streak via marker-row queries that require a success to reset.
- `./webhook-peer-validation-toctou.md` — resolve-time URL check plus connect-time peer-IP re-validation closing DNS-rebinding TOCTOU.
- `./webhook-readback-oracle-containment.md` — remote response bodies logged server-side only; raised errors carry status codes, not bytes.
- `./chart-data-async-job-ladder.md` — four-conjunct sync/async gate, try-cache-first handoff, and identity-carrying job submission for cache-key consistency.
- `./cache-replay-opaque-key-skip.md` — serving pre-computed results from unguessable cache keys with the novel-SQL check skipped by replay flag.
- `./annotation-layer-permission-gate.md` — annotation read permission enforced before any layer lookup; typed missing-id errors.
- `./viz-annotation-override-injection.md` — chart-as-annotation overrides applied before re-authorization of the nested execution.
- `./raise-for-access-before-validate.md` — authorization strictly precedes rendering caller-supplied filter expressions.
...
- [`annotation-cache-context-binding.md`](./annotation-cache-context-binding.md)
- [`annotation-layer-permission-gate.md`](./annotation-layer-permission-gate.md)
- [`cache-key-contribution-exclusion.md`](./cache-key-contribution-exclusion.md)
- [`cache-replay-opaque-key-skip.md`](./cache-replay-opaque-key-skip.md)
- [`cache-timeout-precedence-ladder.md`](./cache-timeout-precedence-ladder.md)
- [`chart-data-async-job-ladder.md`](./chart-data-async-job-ladder.md)
- [`contribution-totals-two-phase-sync.md`](./contribution-totals-two-phase-sync.md)
- [`create-log-working-row-promotion.md`](./create-log-working-row-promotion.md)
- [`df-payload-acquisition-ladder.md`](./df-payload-acquisition-ladder.md)
- [`error-grace-notification-dedup.md`](./error-grace-notification-dedup.md)
- [`grouping-sets-emulation-fallback.md`](./grouping-sets-emulation-fallback.md)
- [`query-cache-manager-fault-tolerance.md`](./query-cache-manager-fault-tolerance.md)
- [`raise-for-access-before-validate.md`](./raise-for-access-before-validate.md)
- [`report-state-machine-dispatch.md`](./report-state-machine-dispatch.md)
- [`retry-or-error-backoff-ladder.md`](./retry-or-error-backoff-ladder.md)
- [`retry-window-staleness-anchor.md`](./retry-window-staleness-anchor.md)
- [`stale-cache-shape-miss-downgrade.md`](./stale-cache-shape-miss-downgrade.md)
- [`viz-annotation-override-injection.md`](./viz-annotation-override-injection.md)
- [`webhook-peer-validation-toctou.md`](./webhook-peer-validation-toctou.md)
- [`webhook-readback-oracle-containment.md`](./webhook-readback-oracle-containment.md)
- [`working-state-concurrency-refusal.md`](./working-state-concurrency-refusal.md)
