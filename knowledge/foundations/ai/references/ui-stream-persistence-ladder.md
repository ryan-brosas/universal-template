...
**Probe:** `bash -c "grep -n '@ts-expect-error cancel is still new' $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/handle-ui-message-stream-finish.ts && grep -n 'simulating browser close' $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/handle-ui-message-stream-finish.test.ts && grep -c structuredClone $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/handle-ui-message-stream-finish.ts"` → `:179`, `:425`, `3`.
