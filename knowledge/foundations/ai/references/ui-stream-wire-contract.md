...
**Probe:** `bash -c "grep -n 'rejects chunk types unknown to the client' $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/ui-message-chunks.test.ts && grep -n 'value.startsWith' $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/ui-message-chunks.ts"` → `:78` and `:173`.
