...
**Probe:** `bash -c "grep -c \"it('\" $REFERENCE_ROOT/ai/packages/ai/src/ui/ui-messages.test.ts"` → 7 tests across three describes (getStaticToolName ×2 incl. dash case, isCustomContentUIPart ×3, isDataUIPart ×2); `bash -c "sed -n '21,31p' $REFERENCE_ROOT/ai/packages/ai/src/ui/ui-messages.test.ts | grep -c 'get-location'"` → 2 (test title + expectation).
