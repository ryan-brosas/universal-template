...
**Probe:** `bash -c "grep -n \"part.type !== 'start'\" $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/to-ui-message-stream.ts && grep -n 'emits separate metadata chunks for non-lifecycle parts' $REFERENCE_ROOT/ai/packages/ai/src/ui-message-stream/to-ui-message-stream.test.ts"` → `:72` and `:158`.
