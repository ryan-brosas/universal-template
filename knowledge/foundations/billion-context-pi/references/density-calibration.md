...
**Probe:** `cd $REFERENCE_ROOT/billion-context-pi && npx tsx --test tests/density.test.ts tests/density-usage-fixes.test.ts` — 18/18 GREEN at pin 6a88c556 (executed pass 12; anchor freeze on null usage, ±20% two-round confirmation, clamp bounds, postCompression skip+late re-anchor).
**Retrieve:**
```ts
