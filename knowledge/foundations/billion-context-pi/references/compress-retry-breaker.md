...
**Probe:** `cd $REFERENCE_ROOT/billion-context-pi && npx tsx --test tests/compress-retry.test.ts` — 10/10 GREEN at pin 6a88c556 (executed pass 12; outcome classification incl. noop-vs-error, per-turn reset, cap reachability, stale-failure immunity, issue-#6 loop breaker).
**Retrieve:**
```ts
