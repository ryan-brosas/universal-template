...
- `./runloop-turn-machine.md` — what one turn covers, stop conditions, copy-vs-mutate discipline (`max_turns` counts appended messages).
- `./handoff-protocol.md` — Agent-returning functions switch the active agent while the model only ever sees `{"assistant": "<name>"}` text.
- `./context-variables-plumbing.md` — name-keyed injection of a shared dict into tools/instructions with schema stripping so it never reaches the wire.
- `./toolcall-execution-errors.md` — per-call isolation; unknown-tool errors are tool messages, but arg-parse/exec exceptions kill the run.
- `./function-to-json-schema.md` — 40-line signature→tools-schema converter; untyped params silently become strings.
- `./streaming-merge-chunk.md` — index-keyed recursive string-append accumulator that rebuilds parallel tool_calls from SSE shards.
- `./run-and-stream-consumer.md` — generator contract (delims, sender-enriched deltas, terminal response envelope) over the same loop.
- `./types-agent-response-result.md` — the complete ontology: 6-field Agent, Response, Result; stateless reentrant agents.
- `./request-assembly.md` — per-turn system-prompt regeneration + verbatim param forwarding (`parallel_tool_calls` only when tools exist).
- `./history-dict-boundary.md` — one serialization boundary (`model_dump_json` round-trip) keeps history uniformly dicts and preserves `sender`.
- `./mock-client-harness.md` — one-method duck-typed fake client + scripted ChatCompletions = deterministic multi-turn loop tests.
- `./repl-streaming-consumer.md` — reference chunk consumer (latch sender, reset on delim-end) and extend-don't-replace history accumulation.
- `./triage-router-pattern.md` — canonical router/specialist wiring: zero-arg transfer closures plus explicit back-transfers.
...
- [`context-variables-plumbing.md`](./context-variables-plumbing.md)
- [`function-to-json-schema.md`](./function-to-json-schema.md)
- [`handoff-protocol.md`](./handoff-protocol.md)
- [`history-dict-boundary.md`](./history-dict-boundary.md)
- [`mock-client-harness.md`](./mock-client-harness.md)
- [`repl-streaming-consumer.md`](./repl-streaming-consumer.md)
- [`request-assembly.md`](./request-assembly.md)
- [`run-and-stream-consumer.md`](./run-and-stream-consumer.md)
- [`runloop-turn-machine.md`](./runloop-turn-machine.md)
- [`streaming-merge-chunk.md`](./streaming-merge-chunk.md)
- [`toolcall-execution-errors.md`](./toolcall-execution-errors.md)
- [`triage-router-pattern.md`](./triage-router-pattern.md)
- [`types-agent-response-result.md`](./types-agent-response-result.md)
