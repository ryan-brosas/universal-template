...
- `./changes-builder-dual-ledger.md` — how to accumulate one atomic edit carrying redo steps + exact inverse undo steps + a live working copy of file-data.
- `./changes-processor-two-phase.md` — how ~40 heterogeneous change types reduce onto one immutable file-data map with a bound-scoped touched second phase.
- `./modifiers-oplog-builders.md` — how live drag operations compose into an ordered, mergeable op log that stays cheap during interaction.
- `./modifiers-matrix-application.md` — how an op log folds into ONE affine matrix, including resize-with-pre-transform on rotated shapes.
- `./safe-size-rect-ladder.md` — how to resize shapes whose stored geometry is corrupted (four-rung validated fallback).
- `./objects-map-lazy-codec.md` — a UUID→object map that decodes lazily and encodes only dirty fields for per-field persistence.
- `./undo-stack-bounded.md` — minimal bounded undo/redo stack (100 entries) with branch discard and duplicate suppression.
- `./shape-tree-structure-guards.md` — reparent/delete guards: descendant-set cycle blocking, component-copy rules, subtree cascades.
- `./data-helpers-sentinel-updates.md` — sentinel-guarded conditional path updates (absence ≠ nil) powering idempotent change replays.
- `./uuid-v8-identity.md` — time-sortable v8 identity across JVM/JS with deterministic fakes for tests.
- `./geometry-kernel-point-matrix.md` — one Point/Matrix record pair over two runtimes with epsilon-only float equality.
- `./file-migrations-registry-kernel.md` — how a document declares which data migrations already ran and what applies the rest.
- `./file-migrations-repair-idiom.md` — the repair-migration shape: predicate gate, strip-then-seed, try/catch-to-original idempotence.
- `./update-objects-tree-protocol.md` — one result-triple visitor (`:keep/:update/:remove`) walking pages AND component copies.
- `./features-compatibility-ladder.md` — feature-set taxonomy plus directional-difference compatibility checks with named error codes.
- `./file-data-metadata-envelope.md` — per-blob metadata envelope surviving db/storage/legacy backends via merge/strip discipline.
- `./binfile-manifest-provenance.md` — persisting archive-manifest provenance onto imported files with legacy-key fallback.
- `./workspace-stats-audit-event.md` — one enriched stats audit event per workspace open, emitted after async deps settle.
- `./error-hygiene-hardening.md` — stripping internal fields from 500 bodies while keeping curated hints; SQLSTATE→copy table.
- `./session-authz-logout-invalidation.md` — dual-token-version session resolution, renewal window, and server-side logout deletion.
- `./file-data-backend-switch.md` — row-keyed read dispatch + config-keyed write dispatch for a safe storage-backend flip.
- `./affine-recovery-corners.md` — reconstructing the affine matrix from four transformed corner points (closed-form solve with structurally-dead terms).
- `./transform-dispatch-move-vs-generic.md` — why pure translation takes a different application path than any other transform, and the flip dot-product algebra.
- `./modifiers-four-bucket-record.md` — the ordered four-bucket op log (geometry/structure × parent/child) with head-merge and zero-op elision.
- `./group-mask-selrect-regeneration.md` — recomputing container bounds from transformed children without double-applying the group transform; mask adopts first child.
- `./modif-tree-persistence.md` — id→modifiers map with dissoc-on-empty semantics and projected structure-child fan-out to descendants.
- `./numeric-precision-guards.md` — two-epsilon vocabulary (1e-4 snap vs 1e-3 compare) and snap-BEFORE-arithmetic placement.
- `./point-transform-convention.md` — row-vector point application and AABB-midpoint centers that the whole stack assumes.
- `./transform-matrix-pair.md` — composing rotation+flip+transform into one render matrix, and why its inverse mirrors the body order.
- `./convenience-modifier-builders.md` — foreign-center rotation compensation, dimension clamping, proportion-lock, orientation swaps as primitive ops.
- `./transform-shape-entry.md` — the single funnel: strip modifiers, compile, apply; root exemption split between geometry and structure.
- `./align-parent-axis-projection.md` — aligning against a rotated parent: wrapper rect in parent space, scalar delta projected onto parent-local axis vectors before one move.
- `./distribute-space-unit-interval.md` — equal-gap distribution: center-sort, unit = free/(n−1), single forward reposition loop anchored at both extremes.
- `./viewport-fit-min-zoom-floor.md` — aspect-ratio fit with symmetric padding, then a min-zoom floor that abandons the fit and re-anchors at the original corner.
- `./bounds-map-delay-volatile-propagation.md` — lazy `{id delay}` bounds map whose delays close over a volatile holding the NEW map, so group bounds see children's modified geometry.
- `./create-bounds-mask-single-child.md` — per-node bounds rule: masks take one child, groups merge children∪self-modified, leaves self; 0.01 clamps on degenerate sizes.
- `./grid-generic-algebra.md` — one `[size item-length next-v gutter]` calculator behind column/row/square grids with ordered derivations and a stretch-gutter max(0,…)+NaN ladder.
- `./snap-point-vocabulary.md` — snap candidate tiers (rect corners+center ⊂ frame midpoints), tri-conditional guide suppression, display-gated axis-filtered grid lines.
- `./constraints-vocabulary-defaults.md` — 8 stored constraints reduce to 4 behaviors; tree-position-derived defaults never persisted.
- `./constraint-anchor-line-intersection.md` — rotation-safe :start/:end/:center anchors via frame-local ray/segment intersection.
- `./constraint-displacement-sign-algebra.md` — before-magnitude × after-direction restore with compared-angle flip sign and degenerate no-op guard.
- `./fixed-constraint-resize-sandwich.md` — :leftright/:topbottom compiles to grow-by-both-displacements resize + start move, after a per-axis deformation pre-strip.
- `./calc-child-constraint-gates.md` — five ordered exits (guards → move fan-out → scale pass-through → layout early-return → axis-aligned reset) with lazy parent-bounds deref.
- `./wasm-constraint-twin-axis-aligned.md` — Rust renderer's AABB approximation: pass-through/scale/displace pipeline and its exact parity boundary vs the Clojure editor.
...
- [`affine-recovery-corners.md`](./affine-recovery-corners.md)
- [`align-parent-axis-projection.md`](./align-parent-axis-projection.md)
- [`binfile-manifest-provenance.md`](./binfile-manifest-provenance.md)
- [`bounds-map-delay-volatile-propagation.md`](./bounds-map-delay-volatile-propagation.md)
- [`calc-child-constraint-gates.md`](./calc-child-constraint-gates.md)
- [`changes-builder-dual-ledger.md`](./changes-builder-dual-ledger.md)
- [`changes-processor-two-phase.md`](./changes-processor-two-phase.md)
- [`constraint-anchor-line-intersection.md`](./constraint-anchor-line-intersection.md)
- [`constraint-displacement-sign-algebra.md`](./constraint-displacement-sign-algebra.md)
- [`constraints-vocabulary-defaults.md`](./constraints-vocabulary-defaults.md)
- [`convenience-modifier-builders.md`](./convenience-modifier-builders.md)
- [`create-bounds-mask-single-child.md`](./create-bounds-mask-single-child.md)
- [`data-helpers-sentinel-updates.md`](./data-helpers-sentinel-updates.md)
- [`distribute-space-unit-interval.md`](./distribute-space-unit-interval.md)
- [`error-hygiene-hardening.md`](./error-hygiene-hardening.md)
- [`features-compatibility-ladder.md`](./features-compatibility-ladder.md)
- [`file-data-backend-switch.md`](./file-data-backend-switch.md)
- [`file-data-metadata-envelope.md`](./file-data-metadata-envelope.md)
- [`file-migrations-registry-kernel.md`](./file-migrations-registry-kernel.md)
- [`file-migrations-repair-idiom.md`](./file-migrations-repair-idiom.md)
- [`fixed-constraint-resize-sandwich.md`](./fixed-constraint-resize-sandwich.md)
- [`geometry-kernel-point-matrix.md`](./geometry-kernel-point-matrix.md)
- [`grid-generic-algebra.md`](./grid-generic-algebra.md)
- [`group-mask-selrect-regeneration.md`](./group-mask-selrect-regeneration.md)
- [`modif-tree-persistence.md`](./modif-tree-persistence.md)
- [`modifiers-four-bucket-record.md`](./modifiers-four-bucket-record.md)
- [`modifiers-matrix-application.md`](./modifiers-matrix-application.md)
- [`modifiers-oplog-builders.md`](./modifiers-oplog-builders.md)
- [`numeric-precision-guards.md`](./numeric-precision-guards.md)
- [`objects-map-lazy-codec.md`](./objects-map-lazy-codec.md)
- [`point-transform-convention.md`](./point-transform-convention.md)
- [`safe-size-rect-ladder.md`](./safe-size-rect-ladder.md)
- [`session-authz-logout-invalidation.md`](./session-authz-logout-invalidation.md)
- [`shape-tree-structure-guards.md`](./shape-tree-structure-guards.md)
- [`snap-point-vocabulary.md`](./snap-point-vocabulary.md)
- [`transform-dispatch-move-vs-generic.md`](./transform-dispatch-move-vs-generic.md)
- [`transform-matrix-pair.md`](./transform-matrix-pair.md)
- [`transform-shape-entry.md`](./transform-shape-entry.md)
- [`undo-stack-bounded.md`](./undo-stack-bounded.md)
- [`update-objects-tree-protocol.md`](./update-objects-tree-protocol.md)
- [`uuid-v8-identity.md`](./uuid-v8-identity.md)
- [`viewport-fit-min-zoom-floor.md`](./viewport-fit-min-zoom-floor.md)
- [`wasm-constraint-twin-axis-aligned.md`](./wasm-constraint-twin-axis-aligned.md)
- [`workspace-stats-audit-event.md`](./workspace-stats-audit-event.md)
