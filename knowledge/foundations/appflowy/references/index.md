...
- `./collab-builder-lifecycle.md` — build→domain-open→finalize assembly; plugin attach exactly once under lock.
- `./collab-disk-persistence.md` — load-into-one-txn, flush_doc + explicit commit protocol.
- `./document-lifecycle-two-maps.md` — documents/removing_documents graveyard with 120s delayed eviction.
- `./document-cloud-open-fallback.md` — disk→cloud→RecordNotFound ladder; empty remote doc_state is 404, never a blank doc.
- `./dispatch-event-map.md` — flat event map, duplicate-event startup panic, total error→response conversion.
- `./dispatch-localset-runtime.md` — LocalSet-only dispatch contexts; handler panics become JoinError responses.
- `./dispatch-handler-extraction.md` — Extract→Handle state machine; decode failures surface as success-shaped error responses.
- `./priority-task-queue.md` — QoS-desc/id-desc max-heap (LIFO within a level), pop-time cancel, per-task timeout.
- `./ws-reconnect-ladder.md` — state→action table with cancel-then-jitter reconnect ([min..10)s).
- `./local-server-null-cloud.md` — offline mode answers wire-shaped queries from the KV store; typed refusals for the rest.
- `./snowflake-id-generator.md` — 41|10|12 bit layout; panic-on-clock-backwards fail-fast policy.
- `./instant-index-writer.md` — Weak-registry reaper loop feeding search consumers every 30s without owning docs.
- `./ffi-byte-protocol.md` — length-prefixed port protocol; Rust leaks, Dart frees; one-thread LocalSet host.
- `./core-composition-root.md` — single-strong-owner/Weak-everywhere wiring order for circular manager deps.
- `./notification-bridge.md` — callback-vs-stream notification split; mpsc(1) profile-change coalescing.
- `./workspace-identity-guard.md` — last-moment workspace-id check before any persisted write.
- `./sync-kill-switch.md` — Option-typed client access sampled at accessor time; DataSyncRequired on disabled sync.
...
- [`collab-builder-lifecycle.md`](./collab-builder-lifecycle.md)
- [`collab-disk-persistence.md`](./collab-disk-persistence.md)
- [`core-composition-root.md`](./core-composition-root.md)
- [`dispatch-event-map.md`](./dispatch-event-map.md)
- [`dispatch-handler-extraction.md`](./dispatch-handler-extraction.md)
- [`dispatch-localset-runtime.md`](./dispatch-localset-runtime.md)
- [`document-cloud-open-fallback.md`](./document-cloud-open-fallback.md)
- [`document-lifecycle-two-maps.md`](./document-lifecycle-two-maps.md)
- [`ffi-byte-protocol.md`](./ffi-byte-protocol.md)
- [`instant-index-writer.md`](./instant-index-writer.md)
- [`local-server-null-cloud.md`](./local-server-null-cloud.md)
- [`notification-bridge.md`](./notification-bridge.md)
- [`priority-task-queue.md`](./priority-task-queue.md)
- [`snowflake-id-generator.md`](./snowflake-id-generator.md)
- [`sync-kill-switch.md`](./sync-kill-switch.md)
- [`workspace-identity-guard.md`](./workspace-identity-guard.md)
- [`ws-reconnect-ladder.md`](./ws-reconnect-ladder.md)
