...
**Flow:** the factory returns **30 cases in 5 groups** (entries-and-lanes 8, records-and-log 8, queries-and-facts 4, validation-and-immutability 4, repository-and-forks 6 — there is NO separate "lane operations" group; lane-operation behavior lives in records-and-log and entries-and-lanes), each asserting cross-backend behavior via error-code contracts (`rejectsWithCode(promise, "not_found" | "already_exists" | "invalid_entry" | "invalid_query" | "invalid_fork_target" | …)`), never message matching:
...
- **queries and facts (4)** — full inventory (pass 10): :232-248 "rejects invalid queries before empty reads" pins the facade-validation plane with ELEVEN invalid shapes across five read methods (`limit: 0` on findEntries/findEntry/findEntriesOnBranch/findEntryOnBranch/findRecords, `cursor: {afterSeq: -1}`, `findRecords({operationKind})` without type, `findOpenOperations` limit 0/-1, `getLog({afterSeq: -1})`) all rejecting `invalid_query` BEFORE any storage read; :250-306 "supports bounded filtered and cursor-based queries" (witnessed by the branch-window and sequence-log capsules); :551-641 "keeps latest-value facts and computes ledger statistics across lanes" — the decisive case: a NEGATIVE usage adjustment (`cause: "adjustment"`, input -2, totalTokens -2, cost.total -0.5) folds into stats through the same addUsageToStats path, final `getStats()` = `{messageCount: 2, cachedTokens: 3, uncachedTokens: 10, totalTokens: 18, costTotal: 9.5}` — stats are a correction-capable ledger, not a monotone counter; the same case pins label tombstones (`setLabel(entry, undefined)` removes the label; `setLabel("missing", …)` rejects `not_found`) and the `deferred_fetch` usage cause carrying zero usage; :643-664 "clears session names durably" — `getLog()` shows BOTH name facts (seq 1 value, seq 2 undefined), and a reopened session AND a fork both report undefined name (the fork copies the LATEST name fact, which IS the tombstone).
...
**Probe:** the harness itself IS the probe generator; deterministic check executed this pass: `packages/agent/test/harness/session/{memory,jsonl}` + `packages/server/test/conformance.test.ts` exist as sibling consumers (graph-verified pass 4), proving multi-backend reuse rather than a single-consumer helper. Full 30-case/5-group inventory above was read directly from :92–1016 (fork-group assertions quoted verbatim from the source); pass 6 deep-read the records-and-log group (:146–534) and quoted its open-operation cases verbatim above; pass 10 deep-read the queries-and-facts group (:232–664) with the negative-adjustment invariant quoted from :551–641.
