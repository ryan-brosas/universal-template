...
- `./shutdown-choreography.md` — In what order does the server stop accepting, drain connections/tasks, and replay signals?
- `./max-requests-jitter.md` — Why does each worker pick a private randomized request limit at startup?
- `./config-load-middleware-stack.md` — What wrap order turns a user app into the served object, and why can't it change?
- `./asgi2-sniffing.md` — How does auto interface detection distinguish ASGI3 from ASGI2 without running the app?
- `./header-preencoding.md` — Why are custom headers lowercased latin-1 at load time, and who refreshes the Date header?
- `./supervisor-dispatch-import-gate.md` — When must the app be an import string, and who binds the shared socket?
- `./zero-downtime-restart-ladder.md` — Why does the replacement worker start BEFORE the old worker dies?
- `./signal-queue-supervisor.md` — How do OS signals become named handlers without running work on the signal stack?
- `./pipe-healthcheck-ping-pong.md` — How does the parent distinguish hung from ready workers via a tri-state pipe probe?
- `./spawn-subprocess-bootstrap.md` — What must run again inside every worker child before the server starts?
- `./reload-strategy-split.md` — How do watchfiles and stat fallback share one restart skeleton, and what is the .* special case?
- `./flow-control-admission.md` — When does backpressure pause reads, and why is the 503 a fake ASGI app rather than a branch?
- `./http-pipelining-deque.md` — Where do requests parsed mid-response wait, and what starts them in order?
- `./h11-paused-pipelining-twin.md` — What replaces the deque when the parser itself buffers pipelined requests?
- `./http-framing-rules.md` — When is chunked encoding auto-added, and which app header forces connection close?
- `./lazy-100-continue.md` — Why is Expect: 100-continue answered only when the app first reads the body?
- `./lifespan-auto-fallback.md` — How does a lifespan-less app still boot, and how is startup state shared with request scopes?
- `./reset-contextvars-dual-branch.md` — Why does the contextvars leak workaround differ between Python 3.10 and 3.11+?
- `./ws-upgrade-relay.md` — What bytes are replayed to hand a live HTTP transport to a WebSocket protocol?
- `./ws-keepalive-fsm.md` — What keeps the ping loop alive with vs without ping_timeout, and how are stale pongs rejected?
- `./ws-close-handshake.md` — Who closes first, what disconnect codes are synthesized, and why must send() never hang on a dead socket?
- `./ws-accept-date-strip.md` — Why is the library's handshake Date deleted before merging accept headers?
- `./websockets-legacy-inversion.md` — Why does the ASGI app start BEFORE the handshake in the websockets-legacy backend?
- `./wsproto-buffer-errors.md` — How is ws_max_size enforced across fragments, and what closes on protocol errors?
- `./proxy-trust-walk.md` — Why is X-Forwarded-For scanned right-to-left, and what makes a hop trusted?
- `./wsgi-bridge-threading.md` — What runs on the executor vs the loop when streaming a sync WSGI app into ASGI?
- `./zttp-framing-ladder.md` — Which response headers are dropped or rewritten when the parser is strict?
- `./h2-stream-multiplexing.md` — How do per-stream cycles, GOAWAY, and cross-stream read gating extend the HTTP/1 model?
- `./h2-negotiation-preface.md` — When is the protocol chosen from TLS metadata vs sniffed bytes?
...
- [`asgi2-sniffing.md`](./asgi2-sniffing.md)
- [`config-load-middleware-stack.md`](./config-load-middleware-stack.md)
- [`flow-control-admission.md`](./flow-control-admission.md)
- [`h11-paused-pipelining-twin.md`](./h11-paused-pipelining-twin.md)
- [`h2-negotiation-preface.md`](./h2-negotiation-preface.md)
- [`h2-stream-multiplexing.md`](./h2-stream-multiplexing.md)
- [`header-preencoding.md`](./header-preencoding.md)
- [`http-framing-rules.md`](./http-framing-rules.md)
- [`http-pipelining-deque.md`](./http-pipelining-deque.md)
- [`lazy-100-continue.md`](./lazy-100-continue.md)
- [`lifespan-auto-fallback.md`](./lifespan-auto-fallback.md)
- [`max-requests-jitter.md`](./max-requests-jitter.md)
- [`pipe-healthcheck-ping-pong.md`](./pipe-healthcheck-ping-pong.md)
- [`proxy-trust-walk.md`](./proxy-trust-walk.md)
- [`reload-strategy-split.md`](./reload-strategy-split.md)
- [`reset-contextvars-dual-branch.md`](./reset-contextvars-dual-branch.md)
- [`shutdown-choreography.md`](./shutdown-choreography.md)
- [`signal-queue-supervisor.md`](./signal-queue-supervisor.md)
- [`spawn-subprocess-bootstrap.md`](./spawn-subprocess-bootstrap.md)
- [`supervisor-dispatch-import-gate.md`](./supervisor-dispatch-import-gate.md)
- [`websockets-legacy-inversion.md`](./websockets-legacy-inversion.md)
- [`ws-accept-date-strip.md`](./ws-accept-date-strip.md)
- [`ws-close-handshake.md`](./ws-close-handshake.md)
- [`ws-keepalive-fsm.md`](./ws-keepalive-fsm.md)
- [`ws-upgrade-relay.md`](./ws-upgrade-relay.md)
- [`wsgi-bridge-threading.md`](./wsgi-bridge-threading.md)
- [`wsproto-buffer-errors.md`](./wsproto-buffer-errors.md)
- [`zero-downtime-restart-ladder.md`](./zero-downtime-restart-ladder.md)
- [`zttp-framing-ladder.md`](./zttp-framing-ladder.md)
