...
- `./loopback-control-plane.md` — how does a privileged local API stay unreachable from non-loopback web origins while failing loud on port conflicts?
- `./li-proxy-rewrite.md` — how do you iframe/proxy a hostile foreign page locally without tripping CORS or frame-busting, behind an SSRF fence?
- `./lh-backend-cloud-mock.md` — what does it take to bootstrap a vendored SPA against a fake cloud backend (JWT shape, envelopes, persistence sidecar)?
- `./smart-default-total-mock.md` — how can a mock answer EVERY unknown endpoint with a type-appropriate value instead of rejecting?
- `./run-coordinator-fsm.md` — what guards keep a single-writer run lifecycle consistent across memory and SQLite?
- `./lead-dedup-merge-store.md` — how do you dedupe leads by URL and merge without clobbering existing fields?
- `./extension-bridge-bootstrap.md` — how does an unpacked MV3 extension discover and authenticate to a per-launch local service?
- `./pas-launcher-rpc-shim.md` — how do you impersonate a desktop launcher's Socket.IO RPC surface so an Electron-targeted SPA boots locally?
- `./mock-sidecar-restart-persistence.md` — where does mock state live so it survives restarts but stays out of the product database?
- `./browser-electron-bridge-shim.md` — how does an Electron-renderer webapp run in a plain browser against local mocks without touching the bundle?
- `./static-webdir-gate.md` — how do you serve a vendored SPA under mounted prefixes without feeding HTML to asset requests?
...
- [`browser-electron-bridge-shim.md`](./browser-electron-bridge-shim.md)
- [`extension-bridge-bootstrap.md`](./extension-bridge-bootstrap.md)
- [`lead-dedup-merge-store.md`](./lead-dedup-merge-store.md)
- [`lh-backend-cloud-mock.md`](./lh-backend-cloud-mock.md)
- [`li-proxy-rewrite.md`](./li-proxy-rewrite.md)
- [`loopback-control-plane.md`](./loopback-control-plane.md)
- [`mock-sidecar-restart-persistence.md`](./mock-sidecar-restart-persistence.md)
- [`pas-launcher-rpc-shim.md`](./pas-launcher-rpc-shim.md)
- [`run-coordinator-fsm.md`](./run-coordinator-fsm.md)
- [`smart-default-total-mock.md`](./smart-default-total-mock.md)
- [`static-webdir-gate.md`](./static-webdir-gate.md)
