...
- `./live-hocuspocus-bootstrap-lifecycle.md` — what order must redis, hocuspocus, routes, and shutdown run in, and why is extension order load-bearing?
- `./live-page-binary-empty-fallback.md` — how do you serve a CRDT document when storage still only has legacy HTML?
- `./live-store-413-force-close-unload.md` — what does the server do when a collaborative save exceeds the backend size limit?
- `./live-force-close-choreography.md` — how do you evict every client of one document across a server fleet and guarantee the doc leaves memory?
- `./live-redis-admin-command-bus.md` — how do instances coordinate admin commands and broadcast stateless events to ALL servers holding a document?
- `./live-debounced-title-persistence.md` — how do you debounce per-document field writes without losing the last edit or leaking observers?
- `./live-apperror-sanitization-taxonomy.md` — how do upstream axios errors become safe, branchable errors at the collaboration edge?
- `./live-ws-auth-cookie-ladder.md` — how do you authenticate a browser WebSocket when cookies may or may not ride the upgrade request?
- `./api-ip-blocklist-fail-closed.md` — which IP ranges may a server-side fetch never target, and how does the verdict survive IPv6 transition encodings and stdlib drift?
- `./api-resolve-validate-escape-hatches.md` — how can trusted internal hosts bypass an SSRF block WITHOUT skipping resolution or pinning?
- `./api-ssrf-pinned-ip-adapter.md` — how do you fetch a user-supplied URL so the validated IP is exactly the reached IP?
- `./api-manual-redirect-revalidation.md` — how do you follow redirects without letting a 3xx bounce you into the internal network?
- `./api-webhook-event-fanout.md` — where do diff, subscription filtering, and per-subscriber delivery live, and how do deletions stay safe?
- `./api-webhook-delivery-envelope.md` — what goes on the webhook wire and how is it signed so receivers verify byte-exactly?
- `./api-webhook-retry-deactivation-ladder.md` — which failures retry, which deactivate the webhook, and which are only recorded?
- `./api-webhook-url-admission.md` — what must be true of a webhook URL at create/update time, including the PATCH-context bug class?
...
- [`api-ip-blocklist-fail-closed.md`](./api-ip-blocklist-fail-closed.md)
- [`api-manual-redirect-revalidation.md`](./api-manual-redirect-revalidation.md)
- [`api-resolve-validate-escape-hatches.md`](./api-resolve-validate-escape-hatches.md)
- [`api-ssrf-pinned-ip-adapter.md`](./api-ssrf-pinned-ip-adapter.md)
- [`api-webhook-delivery-envelope.md`](./api-webhook-delivery-envelope.md)
- [`api-webhook-event-fanout.md`](./api-webhook-event-fanout.md)
- [`api-webhook-retry-deactivation-ladder.md`](./api-webhook-retry-deactivation-ladder.md)
- [`api-webhook-url-admission.md`](./api-webhook-url-admission.md)
- [`live-apperror-sanitization-taxonomy.md`](./live-apperror-sanitization-taxonomy.md)
- [`live-debounced-title-persistence.md`](./live-debounced-title-persistence.md)
- [`live-force-close-choreography.md`](./live-force-close-choreography.md)
- [`live-hocuspocus-bootstrap-lifecycle.md`](./live-hocuspocus-bootstrap-lifecycle.md)
- [`live-page-binary-empty-fallback.md`](./live-page-binary-empty-fallback.md)
- [`live-redis-admin-command-bus.md`](./live-redis-admin-command-bus.md)
- [`live-store-413-force-close-unload.md`](./live-store-413-force-close-unload.md)
- [`live-ws-auth-cookie-ladder.md`](./live-ws-auth-cookie-ladder.md)
