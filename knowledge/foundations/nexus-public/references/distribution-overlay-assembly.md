...
**Invariant:** (1) Defaults file is NEVER edited in place — every operational knob is overridden in data-dir; deployments (like railway-nexus3's env gate) that write into the distribution instead of data-dir break on upgrade. (2) Overlay order is whole-file replacement: adding a jetty config means re-declaring nexus-args, not patching it. (3) The port/host/context-path trio here is what the healthcheck and reverse-proxy capsules assume.
