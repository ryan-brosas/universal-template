...
- `./redirect-fallback-chain.md` — Which URL wins per device and why in-app browsers swap store-vs-web order.
- `./click-id-pregen-async-tracking.md` — Click UUID minted before the 302; row written fire-and-forget with the same id.
- `./link-safety-gate.md` — allow/warn/block precedence and the response-indistinguishability invariant under block.
- `./owner-suspension-probe-factory.md` — Memoised information_schema probe returning a shared SQL fragment for optional columns.
- `./app-scheme-interstitial.md` — Scheme-then-store HTML page that preserves the URL fragment servers never see.
- `./warning-interstitial-xss-posture.md` — Protocol allowlist beats HTML escaping when rendering flagged destinations.
- `./fingerprint-attribution-scoring.md` — Weighted factor scoring, NAT-range exclusion, per-link attribution windows.
- `./write-time-bot-classification.md` — edge > method > ua authority ladder persisted on the click row.
- `./trusted-client-ip-resolution.md` — One choke-point IP resolver with a documented proxy-trust precondition.
- `./resolution-cache-invalidation.md` — Dual-key Redis invalidation wired into every link mutation path.
- `./social-scraper-preview-hook.md` — preHandler content negotiation serving OG meta to crawler UAs.
- `./targeting-rules-semantics.md` — AND-of-dimensions audience rules with data-absent-fails and 404 camouflage.
- `./webhook-hmac-delivery-engine.md` — sha256= signature envelope, 30s-capped backoff, unawaited fan-out.
- `./event-fk-recovery-insert.md` — Constraint-name-scoped FK recovery: only the stale attributed-link FK degrades.
- `./additive-schema-bootstrap.md` — CREATE TABLE IF NOT EXISTS + guarded ALTERs as the whole migration plane.
- `./liveness-readiness-split.md` — /health touches nothing; only the store of record flips readiness.
- `./well-known-verification-endpoints.md` — AASA + assetlinks.json shapes and their serving constraints.
- `./sdk-resolve-twin.md` — Non-redirecting resolve for OS-intercepted clicks enforcing the same block gate.
- `./realtime-click-streaming.md` — Process-local EventEmitter broadcast; single-node by construction.
- `./analytics-bot-free-aggregation.md` — is_bot=false on every rollup; LEFT-JOIN filters in the JOIN clause.
- `./short-code-collision-retry.md` — Bounded regenerate-and-probe over a UNIQUE-constraint backstop.
- `./dynamic-update-query-builder.md` — Schema-key camelCase→snake_case partial updates with JSONB arm.
- `./template-defaults-exclusivity.md` — Preset settings as middle-tier defaults; one default template per scope.
- `./qr-generation-plane.md` — Full-param cache keys and PNG-buffer/SVG-text dual encoding.
- `./debug-simulate-replay.md` — Read-only redirect-decision replay with per-dimension match detail.
...
- [`additive-schema-bootstrap.md`](./additive-schema-bootstrap.md)
- [`analytics-bot-free-aggregation.md`](./analytics-bot-free-aggregation.md)
- [`app-scheme-interstitial.md`](./app-scheme-interstitial.md)
- [`click-id-pregen-async-tracking.md`](./click-id-pregen-async-tracking.md)
- [`debug-simulate-replay.md`](./debug-simulate-replay.md)
- [`dynamic-update-query-builder.md`](./dynamic-update-query-builder.md)
- [`event-fk-recovery-insert.md`](./event-fk-recovery-insert.md)
- [`fingerprint-attribution-scoring.md`](./fingerprint-attribution-scoring.md)
- [`link-safety-gate.md`](./link-safety-gate.md)
- [`liveness-readiness-split.md`](./liveness-readiness-split.md)
- [`owner-suspension-probe-factory.md`](./owner-suspension-probe-factory.md)
- [`qr-generation-plane.md`](./qr-generation-plane.md)
- [`realtime-click-streaming.md`](./realtime-click-streaming.md)
- [`redirect-fallback-chain.md`](./redirect-fallback-chain.md)
- [`resolution-cache-invalidation.md`](./resolution-cache-invalidation.md)
- [`sdk-resolve-twin.md`](./sdk-resolve-twin.md)
- [`short-code-collision-retry.md`](./short-code-collision-retry.md)
- [`social-scraper-preview-hook.md`](./social-scraper-preview-hook.md)
- [`targeting-rules-semantics.md`](./targeting-rules-semantics.md)
- [`template-defaults-exclusivity.md`](./template-defaults-exclusivity.md)
- [`trusted-client-ip-resolution.md`](./trusted-client-ip-resolution.md)
- [`warning-interstitial-xss-posture.md`](./warning-interstitial-xss-posture.md)
- [`webhook-hmac-delivery-engine.md`](./webhook-hmac-delivery-engine.md)
- [`well-known-verification-endpoints.md`](./well-known-verification-endpoints.md)
- [`write-time-bot-classification.md`](./write-time-bot-classification.md)
