---
title: rust-coding-practices
summary: Use when authoring or reviewing Rust, rustfmt defaults, RFC 430 naming, meaningful Error types, Result/? in docs, common trait impls, iter/into_iter conventions, and predictable public APIs.
kind: playbook
---

# Rust Coding Practices

Application skill for Rust style. For async/concurrency or framework crates, combine with stack-specific guidance.

## Core Principle

Follow the project rustfmt, lint, and API conventions. The API
guidelines are options for predictable public surfaces, not universal requirements
...
1. **Format & names**, `cargo fmt`, casing, conversion prefixes.
2. **Errors**, choose useful error information for callers. Add `Error`, `Send`,
   ...
   sentinel error can be enough.
3. **Interop**, common traits, `From`, iterator naming.
4. **API shape**, methods, `new`, no out-params, Deref discipline, docs.
