...
### Blocking-call budgets and recovery

`ask_codebase` is a multi-step model run, not a quick retrieval request. Scope
one decision seam in the relevant repositories, exclude adjacent investigations,
and request a short answer with decisive references. Split an overbroad question
rather than repeating it unchanged; answer-length and step-count requests are
prompt guidance, not enforced execution limits.

Match the **whole-program deadline** to the configured MCP request budget plus
connection/return overhead. In Fabric, pass `timeoutMs` on the outer
`fabric_exec` call, not in `ask_codebase` arguments. Extending only that deadline
cannot override a shorter MCP request timeout; with the adapter backend, its
per-server `requestTimeoutMs` owns that limit. Prefer a Sourcebot-specific budget
over relaxing every server's deadline. Host configuration owns the actual values.
Run a long research call separately from local reads so a timeout does not discard
their results.

An outer `Execution timed out` and an MCP `Request timed out` identify different
cutoffs, not an authentication failure or proof that Sourcebot is down. Distinguish
those from transport errors, HTTP 401/403, missing models and absent corpus coverage.
After a timeout, inspect any available research result/status before retrying:
client cancellation is not proof that server-side generation stopped. Do not start
duplicate research blindly. Narrow the remaining question or use direct evidence;
report incomplete research rather than citing an answer that never returned.
...
Keep discovery, corpus ingestion and implementation as separate scopes. A request
for inspiration calls for candidate repositories and source-backed comparisons,
not edits to the current project. Discover new candidates on GitHub when the
existing corpus cannot answer the question. An explicit ingestion request permits
work on the named corpus within its admission and access policy; it does not
authorize implementing the ideas or pushing project code. Verify ingestion with
repository-scoped retrieval through the consumer, not a configuration entry alone.
If access or admission blocks it, name the blocker and return the research.
Carry these limits through delegation and model handoffs; continuation is not new
authorization. Implement only when the user's task includes implementation.
