...
You may bet any fraction $f \in [0,1)$ of current wealth $W\_k$ on each
round. Win (probability $p$): you gain $b f W\_k$ (net odds $b$). Lose
(probability $q = 1-p$): you lose $f W\_k$. Wealth multiplies:

$$
W\_{k+1} = \begin{cases} (1 + bf)\  W\_k & \text{with prob. } p, \\
(1 - f)\  W\_k & \text{with prob. } q. \end{cases}
$$

The wrong question: "which $f$ maximizes $\mathbb{E}[W\_{k+1} \mid W\_k]$?"
Since $\mathbb{E}[W\_{k+1}] = (1 + (pb - q)f)\  W\_k$, expected value demands
...
With $S\_n = \sum X\_k$ the win count:

$$
\frac{1}{n}\log\frac{W\_n}{W\_0}
= \frac{S\_n}{n}\log(1+bf) + \frac{n-S\_n}{n}\log(1-f).
$$

The multiplicative process became an *average of i.i.d. terms*, so the
strong law of large numbers applies ($S\_n/n \to p$ a.s.):

$$
\frac{1}{n}\log\frac{W\_n}{W\_0} \xrightarrow{\text{a.s.}}
...
Numeric fixture (recompute before trusting): $p = 0.6$, $b = 1$, $f^\* = 0.2$:
$g = 0.6\ln 1.2 + 0.4\ln 0.8 = 0.6(0.182322) + 0.4(-0.223144) = 0.109393 - 0.089257 = 0.020136$
nats/round; and $\ln 2 - H(0.6) = 0.693147 - 0.673012 = 0.020136$. ✓ Both forms
agree, as proven.
