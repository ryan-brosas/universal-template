import Euler.DriftGevreyInviscidEnergyCompactness
import Euler.CorrectionLimitEquation
import Euler.InviscidSobolevEvolution

/-! Drift-aware version: Whole-interval inviscid correction retaining quantitative Gevrey bounds and its actual finite-Sobolev pressure equation. -/
...
open MeasureTheory Set EulerLiftedGradientSpace EulerCylinderSobolevSpace EulerCylinderSobolev
  ...
  EulerQuadraticSource EulerPacketWeights EulerDriftGevreyInviscidEnergyCompactness EulerCorrectionLimitEquation EulerVolterraConvolution
...
theorem exists_global_inviscid_gevrey_PDE {q : ℕ} (hq : 6 ≤ q) (S : ℝ) (hS : 0 < S)
    ...
    (B : EulerDriftCorrectionBudget.Budget period (hq.trans (Nat.le_succ q) |>.trans (Nat.le_succ (q+1))) D N R) (K : MetricBudget period S hS.le D)
  ...
  obtain ⟨u,e,hu,hconv,hi,hd,hM,hE⟩ := EulerDriftGevreyInviscidEnergyCompactness.exists_gevrey_inviscid_energy_limit period (q := q+1)
