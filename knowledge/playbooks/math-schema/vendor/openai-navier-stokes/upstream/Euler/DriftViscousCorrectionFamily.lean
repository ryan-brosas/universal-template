import Euler.DriftGlobalGevreyCorrection
...
theorem exists_viscous_correction_family {q : ℕ} (hq : 6 ≤ q) (S : ℝ) (hS : 0 < S)
    ...
    (B : EulerDriftCorrectionBudget.Budget period (by omega : 6 ≤ q+1) D N R) (K : MetricBudget period S hS.le D)
    ...
    ∃ e : ℕ → C(Icc (0 : ℝ) S,SobolevSpace period (q+1)),
      ...
      Filter.Tendsto (fun n => viscousDefect period (by omega : 2 ≤ q+1) (viscositySequence n) S (e n))
        Filter.atTop (𝓝 0) := by
  have h := fun n => EulerDriftGlobalGevreyCorrection.exists_global_gevrey_correction period hq S hS D KG KL KQ hGq hLq hQq hG
