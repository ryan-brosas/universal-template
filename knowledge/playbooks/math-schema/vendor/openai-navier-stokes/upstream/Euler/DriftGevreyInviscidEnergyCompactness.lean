import Euler.DriftViscousCorrectionFamily
...
theorem exists_gevrey_inviscid_energy_limit {q : ℕ} (hq : 6 ≤ q) (S : ℝ) (hS : 0 < S)
    ...
    (B : EulerDriftCorrectionBudget.Budget period (hq.trans (Nat.le_succ q)) D N R) (K : MetricBudget period S hS.le D)
  ...
  obtain ⟨u,hu,hdefect⟩ := EulerDriftViscousCorrectionFamily.exists_viscous_correction_family period (q := q) hq S hS D KG KL KQ hGq hLq hQq hG
  ...
  have hlim := exists_limit_of_gevrey_family period (q := q) hq S hS.le D KG KL KQ hGq hLq hQq N R B.full K
