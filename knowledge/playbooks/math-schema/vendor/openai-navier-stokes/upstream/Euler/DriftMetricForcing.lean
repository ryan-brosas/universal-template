import Euler.DriftNonlinearEstimate
...
theorem correctionForcing_metric {s : ℕ} (hs : 6 ≤ s) {A : SmoothCoefficient period}
  ...
  let f := orderZeroSource period hs L hL (coefficientSobolevOperator period K0)
    (fun i => coefficientSobolevOperator period (K i)) z r (truncateOperator period s e)
  have h := EulerDriftNonlinearEstimate.correctionForcing_polynomial period hs KG KG0 κ m c hc hpos N hN ρ Rc M B hρ hRc hM hB
  ...
  have hlin : 0 ≤ sourceConstant B M*(productConstant period 3*B1+A0+2*A2*productConstant period 3*Z0)+
  ...
  simpa only [EulerDriftEnergyConstants.forcingPolynomial,
    EulerNonlinearEnergyConstants.linearCoefficient, EulerNonlinearEnergyConstants.quadraticCoefficient,
