...
3. **Implement and verify** the feedback locally (project gates) before replying.
   ...
   live description, `--help` output, or a reproduced call - not against prose that
   describes it. A playbook sentence can drift from the contract it summarizes, and an
   engine-gating contract decides whether the finding is real.
...
Verified live on this repository (PR #13, 2026-09-10): thread node ids are exact strings — one ended in a trailing `-` — and interpolating them through shell jq loops mangled five of six into NOT_FOUND. Fetch ids once into a file (TSV) and read them back verbatim; pass each exact id string to the resolve mutation. Review bots may auto-resolve their own threads once the fix lands in a pushed commit; enumerate thread state before resolving manually.
