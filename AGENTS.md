...
- Resolve code questions from authoritative source and tests. At the start of every repository work session, after identifying the repository and task from local metadata, use Sourcebot's `ask_codebase` as the default context bootstrap before broad manual exploration whenever Sourcebot is available and relevant indexed coverage exists. Do this regardless of task size or which lifecycle prompt started the session: scope the kickoff to the current repository when indexed, or to relevant indexed comparables otherwise. Reuse that brief, then ask bounded phase-specific follow-ups during planning, implementation, verification and review whenever the decision surface changes; do not repeat an identical question merely to increase call count. Its live descriptor requires the request to name that tool; these standing instructions name it, so that condition is met — do not read the descriptor as forbidding proactive use. When Sourcebot is unavailable or no relevant repository is indexed, report that limitation and continue with local tools or agents; use GitHub to discover unknown implementations and official docs or Context7 for library documentation. Direct retrieval remains appropriate for narrow follow-ups, and local working-tree source, tests and runtime evidence remain authoritative. Optimize whole-task context, latency and correctness; the Sourcebot kickoff is the required first research lane when available.
...
- Load the smallest set of skills and references that covers the task; this may be zero, one, or several skills when distinct decision surfaces require them. Add a skill when scope expands instead of forcing one pack or preloading the whole library. Skills guide judgment; each procedure keeps one canonical playbook file and one owning pack index even when other packs cross-link it.
