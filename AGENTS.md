...
- Load the smallest set of skills and references that covers the task; this may be zero, one, or several skills when distinct decision surfaces require them. Add a skill when scope expands instead of forcing one pack or preloading the whole library. Skills guide judgment; each procedure keeps one canonical owning playbook even when other packs cross-link it.
