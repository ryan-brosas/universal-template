...
- `./supervisor-run-spine.md` — ordered 13-step async leader loop; which failure classes retry vs terminate.
- `./task-list-fsm.md` — blocked/failed dependency cascade that guarantees plan-loop termination.
- `./task-mode-loop.md` — autonomous plan→delegate→observe iteration with dual exit keys.
- `./cancel-before-start-registry.md` — setdefault-vs-blind-write cancellation intent + member drain bucket.
- `./delegate-message-bus.md` — member event streaming, drain-don't-break, cascade-cancel mid-flight.
- `./parallel-fanout-backpressure.md` — deepcopy-per-branch isolation + cancel piercing through `gather`.
- `./tool-routing-table.md` — fixed assembly order, mode-exclusive delegation/task tools, first-wins dedupe.
- `./background-manager-chaining.md` — per-retry memory/learning tasks: cancel-then-await, never leak.
- `./member-resolution-hitl.md` — nested member finders (deep vs route) + pause propagation contract.
- `./closure-capture-fanout.md` — default-arg binding regression suite for loop-spawned delegates.

### Run-control / models-core plane
- `./run-loop-retry-ladder.md` — How does the synchronous agent run loop order its stages and retry without losing background work?
- `./dispatch-entry-validation.md` — What must be validated and initialized before a run can start?
- `./cancel-before-start-intent.md` — How does cancellation survive the race where cancel() arrives before register()?
- `./redis-nx-cancellation.md` — How is the same intent-preservation guaranteed atomically across processes?
- `./cancel-wins-over-pause.md` — Why must cancelling a paused run clear its pause flags before persisting?
- `./pair-safe-truncation-index.md` — How do you cut a transcript mid-run without orphaning a tool call?
- `./fork-fresh-metrics.md` — Why does a forked run need its metrics/events/timer wiped?
- `./regenerate-always-forks.md` — What exactly does regenerate drop, and why does it always create a new run_id?
- `./hitl-tool-update-ladder.md` — How does the resume dispatch route each paused tool kind back into execution?
- `./pause-stamp-and-store.md` — What is the exact pause exit sequence when a tool needs human action?
- `./tool-assembly-pipeline.md` — How do declared, default, and client tools become model-ready Functions with media injected?
- `./approval-idempotent-stamping.md` — How are approval records created exactly once across double-fired pause hooks?
- `./background-futures-disconnect.md` — What must always happen in the run-loop finally block, even on success?

### Models-core resilience plane
- `./model-fallback-taxonomy.md` — Which provider errors may fall back to another model, and which must surface?
- `./model-fallback-ladder.md` — error-specific fallback lists; 4xx auth errors never masked.
- `./retry-with-guidance.md` — transient sleep-retry vs teach-retry via temporary guidance messages.
- `./error-taxonomy-late-classify.md` — idempotent late classification of provider errors (message beats status).
...
Add one `./<seam>.md` capsule for one graph-selected, source-confirmed porting question against project `ext-agno`. Add one matching loader line and map entry; keep evidence in the capsule, not this leaf.
...
- [`approval-idempotent-stamping.md`](./approval-idempotent-stamping.md)
- [`background-futures-disconnect.md`](./background-futures-disconnect.md)
- [`background-manager-chaining.md`](./background-manager-chaining.md)
- [`cancel-before-start-intent.md`](./cancel-before-start-intent.md)
- [`cancel-before-start-registry.md`](./cancel-before-start-registry.md)
- [`cancel-wins-over-pause.md`](./cancel-wins-over-pause.md)
- [`closure-capture-fanout.md`](./closure-capture-fanout.md)
- [`delegate-message-bus.md`](./delegate-message-bus.md)
- [`dispatch-entry-validation.md`](./dispatch-entry-validation.md)
- [`error-taxonomy-late-classify.md`](./error-taxonomy-late-classify.md)
- [`fork-fresh-metrics.md`](./fork-fresh-metrics.md)
- [`hitl-tool-update-ladder.md`](./hitl-tool-update-ladder.md)
- [`member-resolution-hitl.md`](./member-resolution-hitl.md)
- [`model-fallback-ladder.md`](./model-fallback-ladder.md)
- [`model-fallback-taxonomy.md`](./model-fallback-taxonomy.md)
- [`pair-safe-truncation-index.md`](./pair-safe-truncation-index.md)
- [`parallel-fanout-backpressure.md`](./parallel-fanout-backpressure.md)
- [`pause-stamp-and-store.md`](./pause-stamp-and-store.md)
- [`redis-nx-cancellation.md`](./redis-nx-cancellation.md)
- [`regenerate-always-forks.md`](./regenerate-always-forks.md)
- [`retry-with-guidance.md`](./retry-with-guidance.md)
- [`run-loop-retry-ladder.md`](./run-loop-retry-ladder.md)
- [`supervisor-run-spine.md`](./supervisor-run-spine.md)
- [`task-list-fsm.md`](./task-list-fsm.md)
- [`task-mode-loop.md`](./task-mode-loop.md)
- [`tool-assembly-pipeline.md`](./tool-assembly-pipeline.md)
- [`tool-routing-table.md`](./tool-routing-table.md)
