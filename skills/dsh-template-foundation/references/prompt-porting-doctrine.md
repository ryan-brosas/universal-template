...
**Signature:** row shape `<old-command> → <new-file.md> → <surface list>`; e.g. `/plan → .dsh/prompts/plan.md → planning-and-task-breakdown, fovea_focus/impact, fabric_mesh`; `/research → .dsh/prompts/research.md → evidence-router, codebase-memory + context7 + exa + deepwiki MCP`.
