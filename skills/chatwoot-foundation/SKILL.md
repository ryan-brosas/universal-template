...
- `references/event-dispatch-fanout.md` — How does a model callback fan one event out to every subscribed account webhook plus the channel inbox webhook?
- `references/webhook-subscription-model.md` — What makes a webhook subscription row valid, and which events may it receive?
- `references/webhook-hmac-delivery.md` — How is an outgoing delivery signed, timed out, and classified for failure handling?
- `references/agent-bot-retry-ladder.md` — When does an agent-bot delivery retry instead of failing, and what happens after the last attempt?
- `references/webhook-error-compensation.md` — What compensating state change repairs a conversation/message when a delivery fails mid-flight?
- `references/safe-fetch-boundary.md` — How do outbound fetches stay SSRF-safe while still allowing private-network deployments?
- `references/round-robin-redis-queue.md` — How does a Redis list implement fair round-robin over inbox agents with self-healing membership?
- `references/legacy-v1-in-save-assignment.md` — Why must legacy V1 auto-assignment run inside before_save on a locked row?
- `references/v2-bulk-assignment-plane.md` — How does bulk V2 assignment coalesce triggers, skip stale backlog, and atomically claim rows?
- `references/fair-distribution-rate-limit.md` — How is per-agent assignment volume capped within a sliding window?
- `references/api-token-tenant-resolution.md` — How does one access token resolve user vs bot identity across accounts?
- `references/bot-endpoint-allowlist.md` — Which endpoints may a bot token call, and where is that enforced?
- `references/bitpacked-feature-flags.md` — How are up to 126 features packed into two bigint flag columns from one YAML source of truth?
- `references/policy-role-ladder.md` — Which account actions need administrator versus agent membership, and where are policies mounted?
- `references/editor-markdown-webhook-normalizer.md` — Why do stored message bodies carry backslash line breaks, and when are they stripped?
