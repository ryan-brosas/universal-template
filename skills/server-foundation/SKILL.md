...
- `references/multiservice-fire-forget-fanout.md` — how one notification reaches every engine without failing the caller.
- `references/hub-pool-comb-sharding.md` — deterministic device→hub routing with time-windowed registration admission.
- `references/hub-tag-grammar.md` — how template tags encode target/exclusion/client-type on the send path.
- `references/relay-endpoint-guards.md` — what a self-host→cloud relay receive endpoint must enforce.
- `references/registration-templates-tag-patch.md` — cross-platform install registration and org-tag patching without full rewrite.
- `references/cipher-sync-flagged-fanout.md` — flag-gated org-cipher fan-out that degrades to non-mobile-only delivery.
- `references/hub-group-grammar-connect-lifecycle.md` — SignalR group naming grammar and mirrored connect/disconnect membership.
- `references/queue-consumer-poison-pill-poll-loop.md` — inert-start gate, bounded redelivery poison drop, and the WhenAny stop ladder.
- `references/dual-dialect-wire-contract-tables.md` — producer-replayed RoutingCase tables keeping queue + HTTP ingestion format-safe across rolling deploys.
- `references/hubhelpers-type-switch-routing.md` — discriminator-peek then typed-reparse routing over inner payload fields.
- `references/anonymous-token-as-group-hub.md` — pre-auth devices listening on a server-minted id as the group name.
