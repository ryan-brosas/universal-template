...
- **Use when:** the user asks to search accumulated foundations in
 `foundation-pack/`, plain `grep -ril "<topic>" foundation-pack/` is enough
 (the pack is separate from the active skill catalog).
...
- `scripts/skill-catalog.py`, deterministic catalog tool (list, search, show,
 stats, generate).
- `foundation-pack/`, accumulated implementation foundations; search with plain `grep`,
 not the catalog.
