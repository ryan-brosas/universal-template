...
**Probe:** `bash -c "grep -n \"should handle empty streams correctly\" /mnt/hdd/utopia/inspo/ai/packages/ai/src/ui/transform-text-to-ui-message-stream.test.ts && grep -c 'text-1' /mnt/hdd/utopia/inspo/ai/packages/ai/src/ui/transform-text-to-ui-message-stream.test.ts"` → :56 and ≥6.
