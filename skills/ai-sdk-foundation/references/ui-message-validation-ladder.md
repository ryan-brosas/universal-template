...
**Probe:** `bash -c "grep -n \"should not re-validate tool input when state is output-available\\|should throw error when no tool schema is found\\|should skip validation for tool part in output-denied state\" /mnt/hdd/utopia/inspo/ai/packages/ai/src/ui/validate-ui-messages.test.ts"` → lines 1145, 1430, 1514.
