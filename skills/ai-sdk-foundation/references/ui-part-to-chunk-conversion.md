...
**Probe:** `bash -c "grep -n 'so tool outputs always keep the output field' /mnt/hdd/utopia/inspo/ai/packages/ai/src/ui-message-stream/to-ui-message-chunk.ts && grep -c 'isDynamic(part)' /mnt/hdd/utopia/inspo/ai/packages/ai/src/ui-message-stream/to-ui-message-chunk.ts"` → `:281` (comment) and `4`.
