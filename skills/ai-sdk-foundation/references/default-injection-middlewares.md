...
**Probe:** `bash -c "grep -n 'should not use default temperature if param temperature is null' /mnt/hdd/utopia/inspo/ai/packages/ai/src/middleware/default-settings-middleware.test.ts"` → `:185`.
...
**Probe:** `bash -c "grep -n 'should respect system messages added by an earlier middleware' /mnt/hdd/utopia/inspo/ai/packages/ai/src/middleware/default-instructions-middleware.test.ts"` → `:230`.
