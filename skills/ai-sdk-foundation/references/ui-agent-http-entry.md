...
**Probe:** `bash -c "grep -n 'tools are compatible' /mnt/hdd/utopia/inspo/ai/packages/ai/src/agent/create-agent-ui-stream.ts && grep -n 'should pass sandbox to tool execution' /mnt/hdd/utopia/inspo/ai/packages/ai/src/agent/create-agent-ui-stream-response.test.ts"` → `:82`, `:366`.
