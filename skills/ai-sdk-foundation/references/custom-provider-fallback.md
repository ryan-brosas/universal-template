...
**Probe:** `bash -c "grep -c 'NoSuchModelError' /mnt/hdd/utopia/inspo/ai/packages/ai/src/registry/custom-provider.ts && grep -n 'asProviderV4(fallbackProviderArg)' /mnt/hdd/utopia/inspo/ai/packages/ai/src/registry/custom-provider.ts"` → `9` (import + @throws doc + one throw per family accessor) and `:106`.
