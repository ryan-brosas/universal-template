...
**Probe:** `bash -c 'cd $REFERENCE_ROOT/joplin && grep -cF "join('"'"'_'"'"')}.json" packages/lib/services/synchronizer/LockHandler.ts && grep -cF "a.updatedTime < b.updatedTime ? -1" packages/lib/services/synchronizer/LockHandler.ts && grep -cF "either the lock could be written but not read back" packages/lib/services/synchronizer/LockHandler.ts'` (anchored at repo root; expects 1 / 1 / 1).
