...
**Probe:** `bash -c 'cd /mnt/hdd/utopia/inspo/joplin && grep -cF "if (!this.enabled)" packages/lib/services/synchronizer/LockHandler.ts && grep -c "enabled_ = false" packages/lib/services/synchronizer/LockHandler.ts && grep -c "\.enabled = true" packages/lib/Synchronizer.ts'` (anchored at repo root; expects 7 / 1 / 0).
