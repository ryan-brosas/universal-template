...
- `./scheduler-minheap-id-tiebreak.md` — How do two equal-priority tasks keep FIFO order through a binary heap that only compares numbers?
- `./scheduler-two-queue-delay-ladder.md` — How do delayed tasks wait without blocking ready work, and when exactly does a host timeout get armed?
- `./scheduler-priority-expiration-table.md` — What timeout does each priority level get, and how does expiration override the frame-yield check?
- `./scheduler-continuation-yield-contract.md` — When a task returns a continuation, why must the loop yield immediately even with budget left?
- `./scheduler-error-resume-macrotask.md` — How does the queue survive a throwing task without a try/catch around user code?
- `./scheduler-host-transport-selection.md` — Which host API posts the next work tick, and why is it not always MessageChannel?
- `./scheduler-root-task-dedup-boundary.md` — How does a consumer map its own priorities onto the scheduler and keep one task per root?
- `./rootsched-microtask-gate.md` — When an update hits a root, what runs immediately versus later, and where is it safe to mutate the root schedule list?
- `./rootsched-sync-drain-loop.md` — When flushSync fires, how does sync work finish across all roots, past roots that throw?
- `./rootsched-lane-task-decision.md` — Given computed next lanes for a root, what are all outcomes, and why do sync lanes never produce a Scheduler task?
- `./rootsched-concurrent-entry-continuation.md` — After the work loop yields, what decides whether a Scheduler task continues versus dies, and why is didTimeout only defensive?
- `./workloop-timeslice-decision.md` — At render time, what forces the synchronous loop despite async updates, and how are errored/fatal renders retried?
- `./workloop-sync-vs-concurrent-loops.md` — Where are the yield points, what replaces shouldYield in throttled mode, and which suspensions replay versus unwind?
- `./lane-bitmap-total-order.md` — How does a bitmask encode priority with pure integer ops, and when may an in-progress render be interrupted?
...
- [`lane-bitmap-total-order.md`](./lane-bitmap-total-order.md)
- [`rootsched-concurrent-entry-continuation.md`](./rootsched-concurrent-entry-continuation.md)
- [`rootsched-lane-task-decision.md`](./rootsched-lane-task-decision.md)
- [`rootsched-microtask-gate.md`](./rootsched-microtask-gate.md)
- [`rootsched-sync-drain-loop.md`](./rootsched-sync-drain-loop.md)
- [`scheduler-continuation-yield-contract.md`](./scheduler-continuation-yield-contract.md)
- [`scheduler-error-resume-macrotask.md`](./scheduler-error-resume-macrotask.md)
- [`scheduler-host-transport-selection.md`](./scheduler-host-transport-selection.md)
- [`scheduler-minheap-id-tiebreak.md`](./scheduler-minheap-id-tiebreak.md)
- [`scheduler-priority-expiration-table.md`](./scheduler-priority-expiration-table.md)
- [`scheduler-root-task-dedup-boundary.md`](./scheduler-root-task-dedup-boundary.md)
- [`scheduler-two-queue-delay-ladder.md`](./scheduler-two-queue-delay-ladder.md)
- [`workloop-sync-vs-concurrent-loops.md`](./workloop-sync-vs-concurrent-loops.md)
- [`workloop-timeslice-decision.md`](./workloop-timeslice-decision.md)
