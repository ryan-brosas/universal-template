...
- `./webhook-hmac-envelope.md` — how are outgoing webhook deliveries signed so verifiers accept them byte-exact?
- `./webhook-client-ssrf-selection.md` — which HTTP client delivers webhooks and why do two exist?
- `./webhook-410-tombstone.md` — when does a failed delivery delete the subscription itself?
- `./webhook-limit-funnel.md` — how are third-party webhooks suppressed without breaking internal ones?
- `./webhook-registration-ladder.md` — how does dispatch subscribe to model events exactly once?
- `./webhook-dual-serialization.md` — how does one payload carry both current and previous state?
- `./webhook-composition-root.md` — why is the dispatch pipeline assembled inside listen()?
- `./webhook-add-validation.md` — how is duplicate registration and dangling integration rejected?
- `./event-registry-once-guard.md` — why does the event bus need hasRegisteredListener?
- `./scheduler-token-expiry-window.md` — how long is a scheduler publish URL valid?
- `./scheduler-event-ladder.md` — how do schedule/unschedule/reschedule map to adapter calls?
- `./scheduler-default-wake-ladder.md` — how does an in-memory scheduler hit publish times within a second?
- `./publish-tolerance-ladder.md` — when must a fired schedule job publish, no-op, or refuse?
- `./schedule-url-auth.md` — how does a scheduler ping authenticate without the 5-minute JWT cap?
- `./scheduler-error-capture.md` — how do unawaited schedule/unschedule failures get reported without stalling boot?
- `./internal-keys-autofilling-map.md` — how do in-process integrations get their API keys without per-request lookups?
- `./session-verification-carryover.md` — when does a fresh login inherit trusted-device status?
- `./session-csrf-origin-pin.md` — how are cookie-authenticated admin requests bound to the admin origin?
- `./email-mfa-challenge-lifecycle.md` — what prevents an emailed 6-digit code from verifying another session?
- `./session-store-contract.md` — what must a custom express-session Store implement over app models?
- `./admin-apikey-jwt.md` — what is the exact verification order for Admin-API Ghost tokens?
- `./outbound-ssrf-defense.md` — how is a user-supplied URL prevented from reaching private networks?
- `./stripe-webhook-intake.md` — what status-code protocol does the inbound Stripe receiver implement?
- `./stripe-endpoint-reconcile.md` — how does the remote Stripe endpoint stay provisioned across reconnects?
- `./identity-token-minting.md` — how are members handed short-lived RS256 tokens?
- `./url-required-shape-inference.md` — which columns/relations must a routable record carry before URL math is legal?
- `./url-filter-eval-compat.md` — how do routes.yaml NQL filters evaluate without changing legacy routing semantics?
- `./url-forward-resolution-ladder.md` — how does a resource become exactly one URL, and who wins router conflicts?
- `./url-thin-resource-degrade-pact.md` — what happens when a caller under-fetches the record it asks a URL for?
- `./url-reverse-canonical-resolve.md` — how does a request path resolve to exactly one resource without false positives?
- `./url-boot-registration-readiness.md` — how does the service come alive, survive route reloads, and hold the site during the gap?
- `./url-routable-enumeration-batching.md` — how do you list every routable row without loading the table or tripping driver limits?
...
- [`admin-apikey-jwt.md`](./admin-apikey-jwt.md)
- [`email-mfa-challenge-lifecycle.md`](./email-mfa-challenge-lifecycle.md)
- [`event-registry-once-guard.md`](./event-registry-once-guard.md)
- [`identity-token-minting.md`](./identity-token-minting.md)
- [`internal-keys-autofilling-map.md`](./internal-keys-autofilling-map.md)
- [`outbound-ssrf-defense.md`](./outbound-ssrf-defense.md)
- [`publish-tolerance-ladder.md`](./publish-tolerance-ladder.md)
- [`schedule-url-auth.md`](./schedule-url-auth.md)
- [`scheduler-default-wake-ladder.md`](./scheduler-default-wake-ladder.md)
- [`scheduler-error-capture.md`](./scheduler-error-capture.md)
- [`scheduler-event-ladder.md`](./scheduler-event-ladder.md)
- [`scheduler-token-expiry-window.md`](./scheduler-token-expiry-window.md)
- [`session-csrf-origin-pin.md`](./session-csrf-origin-pin.md)
- [`session-store-contract.md`](./session-store-contract.md)
- [`session-verification-carryover.md`](./session-verification-carryover.md)
- [`stripe-endpoint-reconcile.md`](./stripe-endpoint-reconcile.md)
- [`stripe-webhook-intake.md`](./stripe-webhook-intake.md)
- [`url-boot-registration-readiness.md`](./url-boot-registration-readiness.md)
- [`url-filter-eval-compat.md`](./url-filter-eval-compat.md)
- [`url-forward-resolution-ladder.md`](./url-forward-resolution-ladder.md)
- [`url-required-shape-inference.md`](./url-required-shape-inference.md)
- [`url-reverse-canonical-resolve.md`](./url-reverse-canonical-resolve.md)
- [`url-routable-enumeration-batching.md`](./url-routable-enumeration-batching.md)
- [`url-thin-resource-degrade-pact.md`](./url-thin-resource-degrade-pact.md)
- [`webhook-410-tombstone.md`](./webhook-410-tombstone.md)
- [`webhook-add-validation.md`](./webhook-add-validation.md)
- [`webhook-client-ssrf-selection.md`](./webhook-client-ssrf-selection.md)
- [`webhook-composition-root.md`](./webhook-composition-root.md)
- [`webhook-dual-serialization.md`](./webhook-dual-serialization.md)
- [`webhook-hmac-envelope.md`](./webhook-hmac-envelope.md)
- [`webhook-limit-funnel.md`](./webhook-limit-funnel.md)
- [`webhook-registration-ladder.md`](./webhook-registration-ladder.md)
