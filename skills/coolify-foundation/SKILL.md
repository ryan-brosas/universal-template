...
- `references/deployment-queue-admission.md` — How does Coolify admit, skip, and drain deployments without double-running one?
- `references/deployment-status-state-machine.md` — How do status transitions stay terminal-once and race-free against user cancels?
- `references/buildpack-router.md` — What is the dispatch order from job start through builder prep to the right build pack?
- `references/env-var-pipeline.md` — Which env vars exist at build time vs runtime, and in what order must .env be written?
- `references/rolling-update-ordering.md` — Why does start-before-stop plus health-gate make zero-downtime deploys safe?
- `references/container-cleanup-timeout.md` — How are stop/remove failures contained so cleanup never fails a good deployment?
- `references/remote-command-lifecycle.md` — How does one SSH command become logs, saved outputs, retries, and cancellation?
- `references/ssh-multiplexing-lock.md` — How is the per-server SSH master socket kept alive and lock-safe across workers?
- `references/ssh-retry-classification.md` — Which SSH errors retry with backoff, and which fail immediately?
- `references/sudo-transformation.md` — How do commands get sudo-prefixed for non-root servers without breaking pipelines?
- `references/scheduled-cron-dedup.md` — How do scheduled backups/tasks fire exactly once per cron window despite chunked polling?
- `references/git-commit-resolution.md` — How is the deploy commit resolved from ls-remote, and when does the helper container restart mid-deployment?
- `references/server-check-selfhealing.md` — How does the periodic server job converge containers, sentinel, log drain, and proxy state?
- `references/image-naming-push.md` — How are build/production image names derived per pack, preview, and registry config?
- `references/docker-label-comma-parsing.md` — How does label parsing survive commas inside values and value-less labels?
- `references/team-broadcast-config-refresh.md` — How does saving a required service variable refresh the config page in every open tab?
- `references/livewire-failure-toast-gestures.md` — How do gateway failures toast exactly once per user action while background polling stays silent?
- `references/proxy-status-restart-vs-update.md` — Why do pending proxy config changes and available proxy updates need different labels?
- `references/template-contract-compose-twin.md` — What makes a one-click service template valid, and why does it exist in two registry dialects?
- `references/proxy-network-reconcile-all-containers.md` — Why must proxy network discovery include stopped containers?
- `references/dev-failure-preview-route.md` — How is a fault-injection endpoint shipped for failure-UX tests without ever mounting in production?
