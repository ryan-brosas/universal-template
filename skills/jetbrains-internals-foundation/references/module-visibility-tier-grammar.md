...
**Probe:** anchored at the PyCharm install root `/mnt/hdd/utopia/inspo/reference/jetbrains/pycharm` (the prolog+comment strip loop is LOAD-BEARING: a naive `find('<')` classifies every file as `?xml`, a naive post-`?>` skip misreads the 117 comment-led mirrors as `<!--`):
```bash
python3 -c "
import zipfile,re
