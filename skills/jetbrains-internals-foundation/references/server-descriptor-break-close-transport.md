...
**Probe:** deterministic content probes executed this pass on `/mnt/hdd/utopia/inspo/dotmemory/JetBrains.Profiler.Windows.SysTools.xml`: `grep -c "Rude and fast close"` = **1**, `grep -c "No that variable == no log"` = **2** (adjacency sanity for same file generation); decisive ranges :54-63/:69-78 verified by full 270-line read.
