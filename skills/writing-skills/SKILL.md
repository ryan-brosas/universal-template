...
6. Skill Result Contract — the XML `<skill_result>` block every skill closes with
...
- RED: a subagent WITHOUT the skill fails a pressure scenario (scored by a rubric)
- GREEN: the smallest skill that flips the failure
- REFACTOR: close the loops the test exposed; re-run against advancing adversarial prompts

Pressure scenarios: the user is in a hurry, rationalizing, has a "special case", the description triggers a miss, or two descriptions tie. Rubric scores out of 5; pass = 4/5 twice.
...
- Every execution closes with the same `<skill_result>` shape.
...
- Host-neutral: no pi-only calls; make optional MCP/tool references capability-probed (see `~/.agents/skills/workflow-lifecycle/references/mcp-context.md`).
