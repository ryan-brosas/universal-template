...
Add one `./<seam>.md` capsule for one graph-selected, source-confirmed porting question against Codebase Memory project `litellm` (renamed from `ext-litellm`; same checkout and HEAD). Add one matching loader line and map entry; keep evidence in the capsule, not this leaf. Candidate seams for future passes live in `/mnt/hdd/utopia/inspo/.skill-mining-work/litellm/research.md` NEXT-PASS TARGETS.
