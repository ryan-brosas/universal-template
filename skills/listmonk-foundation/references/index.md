...
- `./readonly-dryrun-query-composition.md` — two-phase dry-run/execute protocol for %query%-templated destructive statements.
- `./subscriber-insert-optin-contract.md` — insert vs 409-exists vs optin-pending outcomes; SQL-side blocklist status mapping.
- `./update-with-lists-permitted-scoping.md` — symmetric permitted-list guard around merge/replace subscription updates.
- `./next-campaigns-side-effecting-scan.md` — CTE that flushes sent counters, claims scheduled campaigns, snapshots max_subscriber_id.
- `./pipe-waitgroup-completion.md` — sentinel-WaitGroup drain protocol; cleanup-once final-status decision.
- `./nonblocking-pipe-admission.md` — try-enqueue-or-release admission vs bounded-blocking transactional pushes.
- `./sliding-window-and-rate-limits.md` — three limiters (worker tick, global sliding window, per-pipe gauge) and where each sleeps.
- `./tracking-dummy-uuid-privacy.md` — dummy-UUID substitution lattice across render/ingest/aggregation.
- `./bounce-threshold-action-ladder.md` — atomic count-in-SQL escalation to blocklist/unsubscribe/delete.
- `./multi-provider-webhook-dispatch.md` — single-endpoint service routing with per-provider handshake headers.
- `./sns-signature-verification-cache.md` — SNS cert-URL allowlist, SHA1WithRSA canonical serialization, race-safe cert cache.
- `./campaign-status-ladder.md` — five-case manual transition ladder vs deliberate system-writer bypasses.
- `./inline-image-cid-embed.md` — one-shot data-embed resolution with negative-result CID cache.
- `./public-subscribe-resubscribe-form.md` — gate-before-write private-list defense and merge-on-conflict resubscribe.
- `./importer-session-lifecycle.md` — singleton state machine over disposable batch-committing sessions.
- `./email-domain-allow-blocklist.md` — single canonicalization choke point with first-label wildcard matching.
- `./campaign-template-two-layer-compile.md` — AddParseTree grafting of body under base template with sanitized shared FuncMap.
- `./tx-message-subscriber-modes.md` — default/fallback/external recipient resolution with reported partial success.
- `./api-token-cache-auth.md` — deferred-failure middleware, hashed constant-time tokens, cookie-over-Basic precedence.
- `./smtp-pool-from-address-routing.md` — bucketed-pointer round-robin with superset fallback pool.
...
- [`api-token-cache-auth.md`](./api-token-cache-auth.md)
- [`bounce-threshold-action-ladder.md`](./bounce-threshold-action-ladder.md)
- [`campaign-status-ladder.md`](./campaign-status-ladder.md)
- [`campaign-template-two-layer-compile.md`](./campaign-template-two-layer-compile.md)
- [`email-domain-allow-blocklist.md`](./email-domain-allow-blocklist.md)
- [`importer-session-lifecycle.md`](./importer-session-lifecycle.md)
- [`inline-image-cid-embed.md`](./inline-image-cid-embed.md)
- [`multi-provider-webhook-dispatch.md`](./multi-provider-webhook-dispatch.md)
- [`next-campaigns-side-effecting-scan.md`](./next-campaigns-side-effecting-scan.md)
- [`nonblocking-pipe-admission.md`](./nonblocking-pipe-admission.md)
- [`pipe-waitgroup-completion.md`](./pipe-waitgroup-completion.md)
- [`public-subscribe-resubscribe-form.md`](./public-subscribe-resubscribe-form.md)
- [`readonly-dryrun-query-composition.md`](./readonly-dryrun-query-composition.md)
- [`sliding-window-and-rate-limits.md`](./sliding-window-and-rate-limits.md)
- [`smtp-pool-from-address-routing.md`](./smtp-pool-from-address-routing.md)
- [`sns-signature-verification-cache.md`](./sns-signature-verification-cache.md)
- [`subscriber-insert-optin-contract.md`](./subscriber-insert-optin-contract.md)
- [`tracking-dummy-uuid-privacy.md`](./tracking-dummy-uuid-privacy.md)
- [`tx-message-subscriber-modes.md`](./tx-message-subscriber-modes.md)
- [`update-with-lists-permitted-scoping.md`](./update-with-lists-permitted-scoping.md)
