...
- `./runtime-envelope-dispatch.md` — one queue, three envelope types; delivery paths pay task_done per envelope, intervention early-return arms leak it.
- `./intervention-pipeline.md` — on_send/on_publish/on_response interception; None-warn, DropMessage-sentinel, asymmetric failure handling.
- `./publish-sender-skip-gather.md` — full-identity sender skip; gather barrier; fail-loud vs fail-latch constructor flag.
- `./lazy-agent-instantiation.md` — factory-per-type vs instance-per-id; construction-time context injection.
- `./handler-routing-ladder.md` — decorator-stamped target types + router predicates; exact-type first-match dispatch.
- `./type-subscription-prefix.md` — source-becomes-key tenancy; colon-prefixed direct-message topics.
- `./subscription-registry.md` — rebuild-on-mutation recipient cache; pair-equality duplicate rejection; one unguarded manager shared by three runtimes.
- `./run-lifecycle-shutdown.md` — stop vs stop_when_idle vs legacy stop_when; vendored queue shutdown; fresh-queue reset.
- `./dequeue-intervention-ordering.md` — enqueue-clean/dequeue-intercepted ordering; Drop precedes recipient resolution; early-return arms skip task_done.
- `./serialization-registry-type-dispatch.md` — (type_name, content-type)-keyed serializer registry; deserialize returns UnknownPayload values, serialize raises; bare-name collision hazard.

### Distributed runtime plane (`autogen-ext` grpc worker/host)
- `./grpc-registration-handshake.md` — local-factory-first then host RegisterAgent unary RPC; exclusive type→client claims with INVALID_ARGUMENT abort.
- `./grpc-request-correlation.md` — lock-guarded monotonic request_ids parking futures before enqueue; errors cross the wire as strings.
- `./grpc-host-disconnect-recovery.md` — OpenChannel finally-cleanup ladder cancels pending futures and revokes types/subscriptions; worker-side reconnect does not exist.

### Group-chat supervisor plane (`autogen-agentchat`)
- `./team-topic-topology.md` — UUID-scoped topic types wiring participants, manager, and output tap.
- `./supervisor-turn-loop.md` — start-RPC → responses-as-events → `_active_speakers` barrier → select next.
- `./fifo-sequential-lock.md` — event-handoff lock making processing order == arrival order under concurrent delivery.
- `./container-buffer-drain.md` — buffer-on-broadcast, drain-on-request; clear-only-on-success; double-reported delegate errors.
- `./termination-algebra.md` — delta-consuming conditions, AND/OR composition, reuse raises outside normal handling.
- `./selector-retry-ladder.md` — violation-specific feedback retries then deterministic fallback; three-variant mention regex.
- `./selector-context-transcript.md` — pluggable ChatCompletionContext flattened into one prompt message; previous-speaker gating; per-turn retry scratchpad.
- `./swarm-handoff-state.md` — last-handoff-wins reversed scan; latest-target validation only.
- `./graph-activation-groups.md` — (target, activation_group) join countdowns, fire-time re-arm, cycle-without-exit rejection.
- `./run-stream-output-plumbing.md` — single output relay, guaranteed GroupChatTermination marker, queue drain in finally.
- `./team-pause-resume-rpc.md` — out-of-band empty-marker RPCs to participants+manager; cooperative no-op agent defaults; run loop untouched.

### Agent, context & memory planes
- `./assistant-tool-call-loop.md` — range-bounded model↔tool loop; gather barrier; None-sentinel stream; errors become results.
- `./rpc-cancellation-ladder.md` — sync-lock token linked after enqueue; cancelled()-guarded set_result/set_exception; dispatch-time recipient failure.
- `./team-state-checkpointing.md` — participant-name-keyed save/load through thin runtime delegation; refuse-while-running mutex.
- `./model-usage-protocol.md` — RequestUsage dataclass rides every completion; clients expose count_tokens/remaining_tokens only.
- `./token-budget-middle-out.md` — pop-the-middle eviction driven by client accounting; orphaned leading function-result repaired.
- `./local-code-executor-limits.md` — wait_for ladder with exit codes 124 (timeout) / 125 (cancelled); terminate+await cleanup; fail-fast blocks.
- `./memory-update-context.md` — stores mutate the context AND return what they added; retrieval strategy stays inside the store.
- `./workbench-stream-holdback.md` — hold-back-one stream protocol; terminal ToolResult guaranteed last; error-as-result; ExceptionGroup flattening.
- `./buffered-context-view-trim.md` — storage stays complete, only get_messages trims; checkpoints serialize the full list.
- `./tool-schema-run-json-gate.md` — declaration-time schema generation; strict violations surface at `.schema` access; model_validate gates execution.
- `./model-client-stream-contract.md` — chunks are cosmetic, one terminal CreateResult is authoritative; missing terminal fails loud.
...
- [`assistant-tool-call-loop.md`](./assistant-tool-call-loop.md)
- [`buffered-context-view-trim.md`](./buffered-context-view-trim.md)
- [`container-buffer-drain.md`](./container-buffer-drain.md)
- [`dequeue-intervention-ordering.md`](./dequeue-intervention-ordering.md)
- [`fifo-sequential-lock.md`](./fifo-sequential-lock.md)
- [`graph-activation-groups.md`](./graph-activation-groups.md)
- [`grpc-host-disconnect-recovery.md`](./grpc-host-disconnect-recovery.md)
- [`grpc-registration-handshake.md`](./grpc-registration-handshake.md)
- [`grpc-request-correlation.md`](./grpc-request-correlation.md)
- [`handler-routing-ladder.md`](./handler-routing-ladder.md)
- [`intervention-pipeline.md`](./intervention-pipeline.md)
- [`lazy-agent-instantiation.md`](./lazy-agent-instantiation.md)
- [`local-code-executor-limits.md`](./local-code-executor-limits.md)
- [`memory-update-context.md`](./memory-update-context.md)
- [`model-client-stream-contract.md`](./model-client-stream-contract.md)
- [`model-usage-protocol.md`](./model-usage-protocol.md)
- [`publish-sender-skip-gather.md`](./publish-sender-skip-gather.md)
- [`rpc-cancellation-ladder.md`](./rpc-cancellation-ladder.md)
- [`run-lifecycle-shutdown.md`](./run-lifecycle-shutdown.md)
- [`run-stream-output-plumbing.md`](./run-stream-output-plumbing.md)
- [`runtime-envelope-dispatch.md`](./runtime-envelope-dispatch.md)
- [`selector-context-transcript.md`](./selector-context-transcript.md)
- [`selector-retry-ladder.md`](./selector-retry-ladder.md)
- [`serialization-registry-type-dispatch.md`](./serialization-registry-type-dispatch.md)
- [`subscription-registry.md`](./subscription-registry.md)
- [`supervisor-turn-loop.md`](./supervisor-turn-loop.md)
- [`swarm-handoff-state.md`](./swarm-handoff-state.md)
- [`team-pause-resume-rpc.md`](./team-pause-resume-rpc.md)
- [`team-state-checkpointing.md`](./team-state-checkpointing.md)
- [`team-topic-topology.md`](./team-topic-topology.md)
- [`termination-algebra.md`](./termination-algebra.md)
- [`token-budget-middle-out.md`](./token-budget-middle-out.md)
- [`tool-schema-run-json-gate.md`](./tool-schema-run-json-gate.md)
- [`type-subscription-prefix.md`](./type-subscription-prefix.md)
- [`workbench-stream-holdback.md`](./workbench-stream-holdback.md)
