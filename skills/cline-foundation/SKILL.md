...
description: "Use when porting Cline's agentic-runtime core — context compaction (trigger budget, overflow-recovery ladder, safe cut boundaries, no-LLM fold), session-compaction state projection with prefix hashes, runtime safety (loop detection, mistake tracker), pending-prompt steer/queue turn gate, local hub-daemon transport plane (discovery record, mkdir-lock mutex, ensure/retire ladders, WS command envelope, subscription refcounts), claim-once env sentinels with supervised-child restart/backoff/adoption, hub command router (authority/drain gates, degraded-capability replies), monotonic shutdown coordination, disk-truth cron spec reconciliation, agenda task persistence kernel (run-admission gate ladder, exactly-once terminal transitions, crash-recovery triage, SQLite revision CAS, content-hash CAS Markdown spec store), strict/tolerant Markdown intent grammars, location-containment kernel, todo-tool scope gate, workspace file-index TTL worker, @mention budget matching. Source code and direct tests are ground truth."
---
