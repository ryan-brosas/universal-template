...
- `references/slack-adapter-identity-normalization.md` — platform quirks repaired at the adapter edge before the shared kernel: DM channel-type rewrite, thread-ts thread identity, team-scoped participant keys, bot self-id from adapter or envelope.
- `references/slack-bot-mention-strip-boundary.md` — strip only LEADING self-mentions with an id-boundary lookahead (`@U1234` must survive bot `U123`); never strip a bare mention to empty input.
- `references/slack-token-scoped-delivery-selfheal.md` — every outbound post wrapped per-call in the team installation's bot token; invalid_thread_ts reaps the dead binding and RE-THROWS — cleanup never hides a delivery failure.
- `references/schedule-delivery-routing-ladder.md` — persisted schedule metadata → instance-ownership userName gate → three-key binding resolution (bindingKey/threadId/participantKey) → explicit no-reply-text body; restart-safe by construction.
- `references/task-update-relay-fingerprint-dedupe.md` — per-concern subscription identity, format-or-suppress noise gate, fingerprint dedupe before disk I/O, dual-slot sessionId routing, adapter injection only at the post boundary; authorize hooks fail OPEN.
- `references/discord-gateway-role-mention-normalization.md` — pre-adapter rewrite of forwarded gateway events: promise-cached bot-role lookup that deletes its entry on failure so the next event retries; original Request bytes untouched on every non-rewriting path.
- `references/discord-outbound-mention-resolution.md` — agent-written @names resolved to mention ids via exact-match-only guild search; adapter-split hyphenated names repaired; unresolved names pass through verbatim; never ping a fuzzy match.
- `references/discord-thread-context-persistence.md` — participant identity is per-turn metadata (normalized author first, eight raw-payload slots for interactions) persisted without ever changing the thread's bound sessionId; unchanged context skips disk I/O.
- `references/discord-runtime-context-envelope.md` — `<discord_message_context>` block (authorId, isDirectMention, isSubscribedThreadMessage, isOwner) plus the `/idle` reserved no-op reply: subscribed-thread noise becomes an agent decision, not a code filter.
- `references/discord-repeated-error-circuit-breaker.md` — fixed-window per-thread+message error counter (3 in 60s) that escalates through the normal requestStop channel; restart restores persisted thread subscriptions exactly once per thread id.
