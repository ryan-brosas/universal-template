...
- `references/session-state-snapshot-dedup.md` — how do you persist mutable state to an append-only log without flooding it with identical entries?
- `references/branch-safe-restore-legacy-migration.md` — how do you recover persisted state from a session branch with type validation and forward-compatible schema migration?
- `references/internal-action-guard-flags.md` — how do you prevent infinite feedback loops when an extension both observes and acts on host events?
- `references/eager-first-writer-lifecycle.md` — how do you lazily initialize from any context event while protecting against subagent overwrites and enforcing model stickiness?
...
- **Snapshot dedup** — `session-state-snapshot-dedup`: zero-timestamp JSON comparison before append; type guard checks only 3 fields for forward compatibility.
- **Branch-safe restore** — `branch-safe-restore-legacy-migration`: findLast on filtered branch entries; field-by-field apply with ?? fallbacks; legacy pinTier→pinByProfile migration.
- **Guard flags** — `internal-action-guard-flags`: boolean set-before/finally-reset around internal host calls; event handlers early-return on flag to break observation→action loops.
- **First-writer lifecycle** — `eager-first-writer-lifecycle`: lazy init from any context event (first-writer-wins registry); turn-end re-asserts virtual model if externally displaced.
...
## Boundaries
Adopt the pure decision kernel (`routing.ts` + `RoutingDecision`) and the fail-open classifier posture verbatim; adapt the Pi-specific provider registration and registry/auth lookups to your host; omit the slash-command/UI planes and the example config unless you port the whole extension. The config plane (`config.ts`) is the second fully-mined subsystem: its warn-and-degrade normalization, layered merge, and resolution helpers are adopt-verbatim contracts. The session lifecycle plane (`index.ts` + `state.ts`) is the third fully-mined subsystem: its snapshot-dedup persistence, branch-safe restore, guard-flag loop prevention, and first-writer-wins initialization are adopt-verbatim contracts for any long-lived extension.
