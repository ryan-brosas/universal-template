...
**Probe:** `cd $REFERENCE_ROOT/memory/graphiti && grep -c 'executor.clone(database=gid)' graphiti_core/driver/falkordb/operations/entity_node_ops.py` → `1`; `grep -c 'all_nodes.sort(key=lambda n: n.uuid, reverse=True)' graphiti_core/driver/falkordb/operations/entity_node_ops.py` → `1`; direct tests `tests/driver/test_falkordb_ops_routing.py::test_episode_get_by_group_ids_single_group_routes_to_its_graph` (:55, pins `base.clone.assert_called_once_with(database='group-a')` AND `base.execute_query.assert_not_called()`), `test_get_by_group_ids_empty_group_ids_does_not_route` (:111).
