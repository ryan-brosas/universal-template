...
- `./bootstrap-once-gate.md` — how the entrypoint rotates the generated admin password exactly once across restarts via a volume marker.
- `./least-privilege-drop.md` — root-prep → su-exec UID 200 split so the JVM never runs as root.
- `./anonymous-disable.md` — the full-body PUT that disables Nexus's anonymous realm and why it fires after rotation.
- `./eula-consent-gate.md` — read→check→refuse-unless-opt-in→echo-disclaimer ladder so automation never accepts legal terms by default.
- `./smoke-crud-roundtrip.md` — five-assertion live verification ending in a byte-exact write/read-back through a fresh repo.
- `./platform-healthcheck-contract.md` — matched 900 s budgets between platform probe and in-container bootstrap loop; ON_FAILURE ×10; single-replica sizing.
- `./static-template-tests.md` — six regex pins in plain node that keep the security-critical literals from rotting.
- `./env-gate-fail-closed.md` — fail-closed environment variable validation before service initialization.
- `./initial-credential-single-use.md` — single-use initial credential consumption and secure purging protocol.
- `./readiness-conjunction-poll.md` — multi-port readiness conjunction polling during container startup.
- `./signal-relay-dual-path.md` — dual-path signal relay for graceful shutdown propagation between host and guest.
- `./errexit-fail-fast-budget.md` — `set -eu` as the failure budget for every bootstrap step; probes-under-`if` exemption and its mutation-side trap.
- `./loopback-only-mutation-surface.md` — all credential-bearing API mutations target 127.0.0.1; public surface limited to health + authenticated reads.
- `./version-pinned-supply-chain.md` — digest-pinned base image plus the same version restated across README/TEMPLATE_README/THIRD_PARTY_NOTICES as one four-carrier fact.
- `./docs-as-deployed-contract.md` — doc claims (rotation, anonymous-off, idempotency marker, sizing) are contract statements mirroring entrypoint invariants.
- `./npm-test-template-gate.md` — zero-dependency `"type":"module"` npm-test gate so file-text assertions run anywhere bare node runs.
- `./verifier-request-hygiene.md` — post-deploy verifier request hygiene: normalize-once `BASE_URL.rstrip('/')`, mandatory explicit timeouts classed 60 s (warmup/storage ops) vs 30 s (reads), success marker naming the probe repo.
- `./template-readme-sections.md` — the fixed six-section marketplace README skeleton (title-as-task, About Hosting, Common Use Cases, Dependencies+subsections, Implementation Details, Why Deploy) and which claim class belongs in each.
...
- [`anonymous-disable.md`](./anonymous-disable.md)
- [`bootstrap-once-gate.md`](./bootstrap-once-gate.md)
- [`docs-as-deployed-contract.md`](./docs-as-deployed-contract.md)
- [`env-gate-fail-closed.md`](./env-gate-fail-closed.md)
- [`errexit-fail-fast-budget.md`](./errexit-fail-fast-budget.md)
- [`eula-consent-gate.md`](./eula-consent-gate.md)
- [`initial-credential-single-use.md`](./initial-credential-single-use.md)
- [`least-privilege-drop.md`](./least-privilege-drop.md)
- [`loopback-only-mutation-surface.md`](./loopback-only-mutation-surface.md)
- [`npm-test-template-gate.md`](./npm-test-template-gate.md)
- [`platform-healthcheck-contract.md`](./platform-healthcheck-contract.md)
- [`readiness-conjunction-poll.md`](./readiness-conjunction-poll.md)
- [`signal-relay-dual-path.md`](./signal-relay-dual-path.md)
- [`smoke-crud-roundtrip.md`](./smoke-crud-roundtrip.md)
- [`static-template-tests.md`](./static-template-tests.md)
- [`template-readme-sections.md`](./template-readme-sections.md)
- [`verifier-request-hygiene.md`](./verifier-request-hygiene.md)
- [`version-pinned-supply-chain.md`](./version-pinned-supply-chain.md)
