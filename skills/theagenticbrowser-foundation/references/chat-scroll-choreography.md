...
**Probe:** `cd $REFERENCE_ROOT/TheAgenticBrowser && grep -n 'setInterval(maintainScroll' core/utils/ui/injectOverlay.js` → `578`; `grep -c 'checks >= 5' core/utils/ui/injectOverlay.js` → `1` (:822); `grep -c 'chatBox.scrollTop + 1' core/utils/ui/injectOverlay.js` → `1` (:897). No upstream tests; deterministic source pins.
