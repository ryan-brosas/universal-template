...
**Probe:** `cd /mnt/hdd/utopia/inspo/TheAgenticBrowser && grep -c "@BA_agent.tool_plain" core/agents/browser_agent.py` → `9`; `grep -c "@BA_agent.tool$" core/agents/browser_agent.py` → `1`; `grep -n "pass a detailed prompt" core/agents/browser_agent.py` → `:50`; `grep -n "ctx.deps.current_step" core/agents/browser_agent.py` → `:272`. Coverage caveat: repo ships no tests.
