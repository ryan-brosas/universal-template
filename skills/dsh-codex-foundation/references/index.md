...
- `./auth-provider-adapter.md` — one porting question: one-provider OAuth delegation, logout, and a secret-free read-only status projection.
- `./web-auth-challenge.md` — one porting question: single-flight browser login, validated HTTPS authorization challenge, waiter fan-out, and bounded timeouts.
- `./web-auth-cancellation.md` — one porting question: abort active auth, settle every waiter, drain provider work, then delete on sign-out but not disposal.
- `./auth-status-recovery.md` — one porting question: keep authentication separate from quota/reauth failures and recover stored sign-in after a failed browser flow.
- `./trusted-origin-gate.md` — one porting question: loopback defaults plus exact remote-origin sidecar checks that fail closed.
- `./trusted-origin-store.md` — one porting question: owner-only exact-origin allowlist persistence with fail-closed document validation, lock-serialized mutations, and detached sorted results.
- `./auth-route-contract.md` — one porting question: exact auth endpoints that authorize before side effects and dispose through the host effect.
- `./cli-auth-device-code.md` — one porting question: explicit CLI browser/device-code selection, signal forwarding, safe event output, and mode validation.
- `./fast-mode-registry.md` — one porting question: bounded positive-only per-session boolean state with touch-on-read LRU eviction, and a streamSimple decorator that merges `service_tier: 'priority'` into the (possibly replaced) payload for the owning provider only.
- `./usage-quota-parsing.md` — one porting question: secret-free projection of a provider quota/usage response with fail-closed numeric validation and fixed reauthorization error.
- `./credential-store.md` — one porting question: owner-only file-backed OAuth persistence with strict validation, cross-instance write serialization, and detached copies.
- `./search-provider.md` — one porting question: fixed-endpoint OAuth search with secret-free record-before-dispatch, abortable auth, JWT-derived account id, and citeable source normalization.
- `./search-config-overlay.md` — one porting question: two-layer defaulting of standalone-search knobs from one constant source with parse-time vocabulary validation.
- `./tool-policy.md` — one porting question: live settings-backed image/Responses/model-catalog policy with migration and cross-provider execution gates.
- `./image-client.md` — one porting question: generation/edit endpoint selection, OAuth/account headers, cancellation, and fail-closed image-response decoding.
- `./imagegen-workspace-refs.md` — one porting question: bounded, content-classified workspace paths converted to ordered provider data URLs.
- `./imagegen-conversation-refs.md` — one porting question: recursive recent conversation attachment selection with exact cardinality and stable order.
- `./imagegen-tool-boundary.md` — one porting question: policy/capability gates, attachment persistence, and best-effort write-intent publication.
- `./image-media-type.md` — one porting question: shared magic-byte classification for PNG/JPEG/GIF/WebP image inputs.
- `./read-image-http-input.md` — one porting question: local read_image delegation plus bounded HTTP(S) loading and attachment projection.
- `./read-image-sync-lifecycle.md` — one porting question: per-agent enhanced read_image shadow synchronization and teardown.
- `./responses-transport-choice.md` — one porting question: per-call SSE versus cached WebSocket transport from live preferences, with refcounted compaction marks kept off the continuation chain.
- `./compaction-marker-codec.md` — one porting question: versioned text-checkpoint framing that carries provider-native output losslessly both directions.
- `./compaction-sse-parse.md` — one porting question: strict V2 compaction SSE parsing into retained history plus exactly one compaction item.
- `./compaction-retry-ladder.md` — one porting question: abort-aware retries honoring server Retry-After hints under a hard two-attempt cap.
- `./responses-usage-projection.md` — one porting question: fail-closed-to-zero subscription usage projection with structural zero costs.
- `./native-compaction-request.md` — one porting question: V2 compaction request shaping/auth headers with a guaranteed standard-stream fallback.
- `./doctor-diagnostics.md` — one porting question: secret-free metadata-only credential states, conflict asserts, and non-mutating repair hints.
- `./compatibility-contract.md` — one porting question: exact-version compatibility evaluation with first-class unknown states and a bounded manifest search.
- `./service-facade.md` — one porting question: singleton credentials plus live policy behind a provider-owned context service that any number of optional front doors share without changing host defaults.
- `./public-http-guard.md` — one porting question: resolve-check-pin public HTTP fetching where every DNS answer must be public, redirects are freshly rechecked, and bodies obey declared and streamed byte ceilings.
- `./search-event-ledger.md` — one porting question: registering a plugin-owned secret-free session event in a guarded host vocabulary and appending the exact request strictly before dispatch.
- `./cli-doctor-json.md` — one porting question: versioned single-line doctor/status JSON documents that structurally omit secrets, strict flag matrices, and exit codes encoding actionable failure.
- `./adapter-replay-models.md` — one porting question: read-time identity-preserving replay-state lifting, advertise-versus-resolve model visibility splitting, and a bounded extended retry policy for proxy-blip-prone traffic.
- `./oauth-bearer-provider-auth.md` — one porting question: projecting a subscription OAuth token into the generic adapter's apiKey slot with zero environment discovery and fail-the-request-on-empty semantics.
- `./single-profile-adapter-factory.md` — one porting question: assembling one provider profile through layered wrappers and late-bound closures without forking the generic adapter.
- `./detached-model-catalog-projection.md` — one porting question: exposing the full provider catalog as fresh minimal copies that policy, settings, and UI can never mutate through.
- `./client-slot-injection.md` — one porting question: a browser plugin entry that mounts settings/tool-view/composer surfaces into host slots with injectable dependencies and effect-owned resource reclamation.
- `./settings-account-lifecycle.md` — one porting question: a seven-state OAuth account page with popup-before-await sign-in, state-specific poll intervals, and named untrusted-origin remediation.
- `./client-json-request.md` — one porting question: one same-origin JSON route client with conditional serialization, server-message-first errors, and a single typed error.
- `./quota-indicator-projection.md` — one porting question: fail-soft whole-payload usage validation, exact model-to-bucket selection, threshold colors, and eligibility-gated abort-clean polling.
- `./fast-mode-toggle-contract.md` — one porting question: server-authoritative per-session toggling over GET/POST routes with prior-state restore and identity-checked controller ownership.
- `./imagegen-tool-view.md` — one porting question: defensive tool-result parsing (output_path/output_error markers), injectable promise-cached image loading, and degrade-to-raw presentation.
- `./tui-login-controller.md` — one porting question: a background browser login that returns control immediately, with challenge settlement, logout-cancel versus teardown-dispose asymmetry, and never-resolving abort-wait prompts.
- `./tui-headless-browser-launch.md` — one porting question: cross-platform HTTPS-only argv-element browser spawning with honest headless degradation and launch outcome as data.
- `./tui-command-tree-completion.md` — one porting question: capability-optional dual registration of executable commands and bilingual canonical-path completion trees sharing only the root word.
- `./tui-command-handler-contract.md` — one porting question: total argv grammar with HELP-as-failure arity guards, echo-after-write projections, NaN-tolerant rendering, and redaction-plus-truncation error results.
- `./binary-local-write-contract.md` — one porting question: fail-closed sandbox ladders checked before locking, strict replaceIfVersion/createIfAbsent intent gates, and capability-probing non-local dispatch.
- `./binary-atomic-publish-lock.md` — one porting question: same-directory exclusive-temp publication committed by link-or-rename under tail-chained per-path in-process locks.
- `./image-vision-gate.md` — one porting question: one module-level gate both image tools share, resolving the current conversation model route and refusing unknown or non-image modalities before any request.
- `./plugin-assembly-order.md` — one porting question: conflict-assert-before-register wiring with an ascending inject ladder and closure-based late binding of request-time state.
- `./noop-invariant-companion.md` — one porting question: registering an explicit empty invariant installer whose comment names which owning operation validates each risk.
- `./settings-route-gates.md` — one porting question: browser-writable Fast Mode and preference-patch routes failing closed through one bounded-body kernel with exact-shape validators and secret-free refusals.
- `./version-injection-bridge.md` — one porting question: embedding the package version at runtime through an ambient-declare bridge defined identically by every build/test surface from one package.json read.
- `./profile-composition-patch.md` — one porting question: shipping composition defaults as repo-owned patch rows that saved user settings override, with a dependency-scoped optional terminal door.
- `./release-provenance-gate.md` — one porting question: gating npm releases on tag-version parity before install and publishing keylessly via OIDC provenance.
...
Add one `./<seam>.md` capsule for one graph-selected, source-confirmed porting question against project `dsh-codex`. Add one matching loader line and map entry; keep evidence in the capsule, not this leaf. Candidate seams for future passes live in the dsh-codex-work record next-pass targets.
...
- [`adapter-replay-models.md`](./adapter-replay-models.md)
- [`auth-provider-adapter.md`](./auth-provider-adapter.md)
- [`auth-route-contract.md`](./auth-route-contract.md)
- [`auth-status-recovery.md`](./auth-status-recovery.md)
- [`binary-atomic-publish-lock.md`](./binary-atomic-publish-lock.md)
- [`binary-local-write-contract.md`](./binary-local-write-contract.md)
- [`cli-auth-device-code.md`](./cli-auth-device-code.md)
- [`cli-doctor-json.md`](./cli-doctor-json.md)
- [`client-json-request.md`](./client-json-request.md)
- [`client-slot-injection.md`](./client-slot-injection.md)
- [`compaction-marker-codec.md`](./compaction-marker-codec.md)
- [`compaction-retry-ladder.md`](./compaction-retry-ladder.md)
- [`compaction-sse-parse.md`](./compaction-sse-parse.md)
- [`compatibility-contract.md`](./compatibility-contract.md)
- [`credential-store.md`](./credential-store.md)
- [`detached-model-catalog-projection.md`](./detached-model-catalog-projection.md)
- [`doctor-diagnostics.md`](./doctor-diagnostics.md)
- [`fast-mode-registry.md`](./fast-mode-registry.md)
- [`fast-mode-toggle-contract.md`](./fast-mode-toggle-contract.md)
- [`image-client.md`](./image-client.md)
- [`image-media-type.md`](./image-media-type.md)
- [`image-vision-gate.md`](./image-vision-gate.md)
- [`imagegen-conversation-refs.md`](./imagegen-conversation-refs.md)
- [`imagegen-tool-boundary.md`](./imagegen-tool-boundary.md)
- [`imagegen-tool-view.md`](./imagegen-tool-view.md)
- [`imagegen-workspace-refs.md`](./imagegen-workspace-refs.md)
- [`native-compaction-request.md`](./native-compaction-request.md)
- [`noop-invariant-companion.md`](./noop-invariant-companion.md)
- [`oauth-bearer-provider-auth.md`](./oauth-bearer-provider-auth.md)
- [`plugin-assembly-order.md`](./plugin-assembly-order.md)
- [`profile-composition-patch.md`](./profile-composition-patch.md)
- [`public-http-guard.md`](./public-http-guard.md)
- [`quota-indicator-projection.md`](./quota-indicator-projection.md)
- [`read-image-http-input.md`](./read-image-http-input.md)
- [`read-image-sync-lifecycle.md`](./read-image-sync-lifecycle.md)
- [`release-provenance-gate.md`](./release-provenance-gate.md)
- [`responses-transport-choice.md`](./responses-transport-choice.md)
- [`responses-usage-projection.md`](./responses-usage-projection.md)
- [`search-config-overlay.md`](./search-config-overlay.md)
- [`search-event-ledger.md`](./search-event-ledger.md)
- [`search-provider.md`](./search-provider.md)
- [`service-facade.md`](./service-facade.md)
- [`settings-account-lifecycle.md`](./settings-account-lifecycle.md)
- [`settings-route-gates.md`](./settings-route-gates.md)
- [`single-profile-adapter-factory.md`](./single-profile-adapter-factory.md)
- [`tool-policy.md`](./tool-policy.md)
- [`trusted-origin-gate.md`](./trusted-origin-gate.md)
- [`trusted-origin-store.md`](./trusted-origin-store.md)
- [`tui-command-handler-contract.md`](./tui-command-handler-contract.md)
- [`tui-command-tree-completion.md`](./tui-command-tree-completion.md)
- [`tui-headless-browser-launch.md`](./tui-headless-browser-launch.md)
- [`tui-login-controller.md`](./tui-login-controller.md)
- [`usage-quota-parsing.md`](./usage-quota-parsing.md)
- [`version-injection-bridge.md`](./version-injection-bridge.md)
- [`web-auth-cancellation.md`](./web-auth-cancellation.md)
- [`web-auth-challenge.md`](./web-auth-challenge.md)
