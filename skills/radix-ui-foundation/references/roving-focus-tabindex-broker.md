...
**Probe:** direct test `packages/react/roving-focus/src/roving-focus-group.test.tsx`. Byte-exact anchors: `bash -c "cd $REFERENCE_ROOT/external/ui-radix-ui && grep -nF \"key === 'ArrowLeft' ? 'ArrowRight' : key === 'ArrowRight' ? 'ArrowLeft' : key\" packages/react/roving-focus/src/roving-focus-group.tsx"` (:361) and `grep -cF 'onFocusableItemAdd();' packages/react/roving-focus/src/roving-focus-group.tsx"` (=2, both effects present).
**Retrieve:**
```ts
