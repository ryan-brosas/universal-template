...
**Probe:** direct tests `packages/react/presence/src/presence.test.tsx` — `does not loop when the child has a stable ref` (:10) and `…unstable callback ref that triggers a render on attach` (:25). Byte-exact anchors: `bash -c "cd /mnt/hdd/utopia/inspo/external/ui-radix-ui && grep -nF 'CSS.escape(event.animationName)' packages/react/presence/src/presence.tsx"` (:119) and `grep -cF 'unmountSuspended' packages/react/presence/src/presence.tsx"` (=4).
**Retrieve:**
```ts
