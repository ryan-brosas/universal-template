...
**Probe:** direct tests `packages/react/select/src/select.test.tsx` — form-reset matrix :272-390 (uncontrolled/controlled/external-form), clear-value suite #2706 :77-145 incl. `renders a single empty native option when a clear item is provided` (:124). Byte-exact anchor: `bash -c "cd $REFERENCE_ROOT/external/ui-radix-ui && grep -nF 'defaultValue={selectValue}' packages/react/select/src/select.tsx"` (:1838).
**Retrieve:**
```ts
