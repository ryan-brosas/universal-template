...
- `references/completion-recovery-ladder.md` — how does one sync completion call turn a provider failure into exactly one recovery action without unbounded recursion?
- `references/budget-guard.md` — how is a max_budget made a genuine hard cap rather than one overshootable by a whole LLM call?
- `references/before-llm-hook-gate.md` — how do blocking hooks/plugins refuse an LLM dispatch instead of failing open?
- `references/streaming-fallback-routing.md` — which exceptions may trigger the non-streaming fallback, and which must never?
- `references/tool-retry-ladder.md` — when does a failed tool call get retried, and what must always short-circuit the retry?
- `references/circuit-breaker-scoping.md` — how is a shared breaker registry prevented from cross-agent contamination and stale-id reuse?
- `references/loop-detection-detectors.md` — how does a sliding window of hashed tool calls distinguish stuck loops from legitimate repetition?
- `references/guardrail-fail-closed-regen.md` — how does a guardrail retry regenerate with feedback while its LLM judge fails closed on ambiguity?
