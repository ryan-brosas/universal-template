...
- `./doc-source-contract.md` — the three-method storage contract (pull/push/subscribe) every backend must honor.
- `./sync-peer-state-machine.md` — SyncPeer's three concurrent loops and the origin-name echo guard.
- `./doc-engine-peer-lifecycle.md` — main-before-shadow startup ordering and push-drained graceful stop.
- `./priority-queue-shared-target.md` — abortable wake-up queues with dequeue-time priority rules.
- `./local-first-source-impls.md` — IndexedDB row compaction + BroadcastChannel init/update handshake.
- `./blob-engine-shadows.md` — content-addressed blob replication: await main, fanout shadows, list-diff sweep.
- `./y-block-encoding.md` — sys:/prop: wire format, parent-side child ordering, root discovery scan.
- `./schema-validation-glob.md` — bidirectional minimatch + @role matching over parent/children allowlists.
- `./store-islocal-heuristic.md` — the four-clause isLocal classification duplicated across four observers.
- `./sync-controller-stash-pop.md` — prop-level optimistic edits that bypass Yjs until popped.
- `./reactive-proxy-loop.md` — createYProxy loop-breaker trio: proxy:true origin tags, WeakMap registry, intercepted array methods.
- `./flat-native-y-map.md` — dotted-path keys in one flat Y.Map; prefix-sweep deletes; single-transaction subtree writes.
- `./text-delta-ops.md` — delta-preserving join, delete-the-tail split, CRLF normalization.
- `./boxed-native-wrapper.md` — tagged-Y.Map atomic boxing for whole-value (non-merged) payloads.
- `./move-blocks-contiguity.md` — multi-block move preconditions, contiguity throw, insert-index drift rule.
- `./doc-sync-peer-jobs.md` — production DocSyncPeer: five job types, fixed drain order, three-clock metadata, terminal permission errors.
- `./server-snapshot-squash.md` — read-time squash into snapshots with timestamp-guarded upsert and fail-open native validation.
- `./awareness-broadcast-source.md` — ephemeral presence sync: 'remote' origin convention, announce-on-connect handshake.
- `./transformer-error-capture-funnel.md` — swallow-and-log funnel + zod gates at both ends of every doc/slice/block conversion.
- `./transformer-flatten-insert-pipeline.md` — flatten→serial-convert→rebuild tree, DFS pre-order root rule, nextTick yield every 100 inserts.
- `./transformer-slice-move-vs-insert.md` — temporary-root envelope; hasBlock(first.id) fork between moveBlocks and insert; parent-required throw on the move path.
- `./transformer-per-flavour-hook.md` — schema.transformer?.() with BaseBlockTransformer fallback; version −1 sentinel for unversioned snapshots.
- `./transformer-tagged-json-roundtrip.md` — `$blocksuite:internal:text$` / `:native$` envelopes; Reflect.has revival; Text rides as delta.
- `./transformer-assets-manager-ladder.md` — File-rename/MIME-trust/octet-stream-sniff blob ladder; conflict renames the copy only.
- `./transformer-middleware-slots.md` — four rxjs Subjects (before/after × import/export), payload unions with parent/index context, cleanup contract.
- `./transformer-replace-id-middleware.md` — six subscriptions over one closure idMap; before-assign ids, after-repair cells/deltas/refs/surface elements.
- `./transformer-upload-middleware.md` — view-driven re-arm loop, abort race arm, sha-addressed blobs, withoutTransact undo-pollution guard.
- `./transformer-base-adapter-template.md` — BaseAdapter template method + wrapFakeNote synthetic note + unclosed-nodes throw in ASTWalker.walk.
- `./transformer-astwalker-context-stack.md` — openNode/closeNode stack attach into array props only; global vs per-node context lifetime.
- `./transformer-astwalker-skip-protocol.md` — skipAllChildren vs skipChildren(n) resume semantics; per-visit flag resets are structural.
- `./transformer-memory-blob-crud.md` — MemoryBlobCRUD sha-keyed dual-overload set; getAssetName naming ladder over the 38-entry MIME table.
- `./transformer-doccrud-composition.md` — production wiring recipe: collection.blobSync + null-returning docCRUD.get + direction-specific middleware sets.
- `./transformer-config-middlewares.md` — title→adapterConfigs map vs fileName→snapshot mutation: two distinct injection channels.
- `./transformer-snapshot-schemas.md` — BlockSnapshot/DocSnapshot/SliceSnapshot zod shapes incl. recursive lazy children and backward-compat tags.
...
- [`awareness-broadcast-source.md`](./awareness-broadcast-source.md)
- [`blob-engine-shadows.md`](./blob-engine-shadows.md)
- [`boxed-native-wrapper.md`](./boxed-native-wrapper.md)
- [`doc-engine-peer-lifecycle.md`](./doc-engine-peer-lifecycle.md)
- [`doc-source-contract.md`](./doc-source-contract.md)
- [`doc-sync-peer-jobs.md`](./doc-sync-peer-jobs.md)
- [`flat-native-y-map.md`](./flat-native-y-map.md)
- [`local-first-source-impls.md`](./local-first-source-impls.md)
- [`move-blocks-contiguity.md`](./move-blocks-contiguity.md)
- [`priority-queue-shared-target.md`](./priority-queue-shared-target.md)
- [`reactive-proxy-loop.md`](./reactive-proxy-loop.md)
- [`schema-validation-glob.md`](./schema-validation-glob.md)
- [`server-snapshot-squash.md`](./server-snapshot-squash.md)
- [`store-islocal-heuristic.md`](./store-islocal-heuristic.md)
- [`sync-controller-stash-pop.md`](./sync-controller-stash-pop.md)
- [`sync-peer-state-machine.md`](./sync-peer-state-machine.md)
- [`text-delta-ops.md`](./text-delta-ops.md)
- [`transformer-assets-manager-ladder.md`](./transformer-assets-manager-ladder.md)
- [`transformer-astwalker-context-stack.md`](./transformer-astwalker-context-stack.md)
- [`transformer-astwalker-skip-protocol.md`](./transformer-astwalker-skip-protocol.md)
- [`transformer-base-adapter-template.md`](./transformer-base-adapter-template.md)
- [`transformer-config-middlewares.md`](./transformer-config-middlewares.md)
- [`transformer-doccrud-composition.md`](./transformer-doccrud-composition.md)
- [`transformer-error-capture-funnel.md`](./transformer-error-capture-funnel.md)
- [`transformer-flatten-insert-pipeline.md`](./transformer-flatten-insert-pipeline.md)
- [`transformer-memory-blob-crud.md`](./transformer-memory-blob-crud.md)
- [`transformer-middleware-slots.md`](./transformer-middleware-slots.md)
- [`transformer-per-flavour-hook.md`](./transformer-per-flavour-hook.md)
- [`transformer-replace-id-middleware.md`](./transformer-replace-id-middleware.md)
- [`transformer-slice-move-vs-insert.md`](./transformer-slice-move-vs-insert.md)
- [`transformer-snapshot-schemas.md`](./transformer-snapshot-schemas.md)
- [`transformer-tagged-json-roundtrip.md`](./transformer-tagged-json-roundtrip.md)
- [`transformer-upload-middleware.md`](./transformer-upload-middleware.md)
- [`y-block-encoding.md`](./y-block-encoding.md)
