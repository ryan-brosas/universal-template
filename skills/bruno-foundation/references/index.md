...
- `./agent-cache-lru.md` — hashed-key agent LRU (cap 100), destroy-on-evict, timeline repoint on cache hits, add-on-top secureContext so custom CAs don't replace the default store.
- `./oauth2-token-lifecycle.md` — the valid → refresh → refetch → expired-but-requested ladder driven by `autoFetchToken`/`autoRefreshToken`, identical across electron/CLI/helper twins.
- `./oauth2-credential-vault.md` — encrypted per-collection token store keyed `(url, credentialsId)` with lazy session-id minting and never-throw accessors.
- `./oauth2-authorize-window.md` — BrowserWindow redirect interception: error-before-match ordering, response-indicator-gated callback matching, fail-closed state validation after close, exhaustive listener teardown; protocol-handler twin for deep links.
- `./oauth2-pkce-state.md` — fresh-per-attempt PKCE S256 challenge; random-when-absent state with strict-equality check; the comment-vs-code divergence in `generateState`.
- `./oauth2-additional-params.md` — enabled-gated header/query/body routing for token/refresh/authorize extras, pre-stringify body mutation, URL-reparse appends.
- `./oauth2-debug-envelope.md` — every token attempt yields a main-request-shaped evidence entry; three-branch capture (success / HTTP error / transport errno).
- `./vault-client-shim.md` — node-vault-compatible minimal Vault HTTP client: validateStatus-true classification, `/sys/health` status amnesty, dual-shaped `VaultError`, lazy TLS agents.
- `./pac-resolver-cache.md` — promise-cache single-flight PAC downloads with TTL, self-evicting failures, QuickJS-sandboxed evaluation, injected `myIpAddress`.
- `./system-proxy-merge.md` — env beats OS settings via `||` merge, single-flight OS detection, fail-open-to-env on detector failure.
- `./proxy-normalization-safeexec.md` — execFile-only OS probing returning null on failure; idempotent scheme and no_proxy normalization.
- `./cookie-jar-wrapper.md` — singleton jar wrapper: `__Host-` no-explicit-domain rule, Infinity expiry coercion, ignoreError sets, the callback-mode never-resolving-promise trap.
- `./ws-client-fsm.md` — WebSocket lifecycle: CONNECTING-reuse, close-promise coalescing with 5s terminate fallback, queue-drop-before-handshake, identity-checked late close events.
- `./sent-headers-reconcile.md` — read Node's `_header` block for what truly went out, length-mask proxy-authorization, additive case-insensitive merge into prepared headers.
- `./header-delete-set-null.md` — script-driven header deletion must tombstone (`set(name, null)`) because axios's default-guard checks existence; host/connection protected.
- `./timeline-agent-instrumentation.md` — subclass-wrapped agents logging DNS/TLS/ALPN events to a locally captured array (survives cached-agent repointing), errors carry partial timelines.
- `./expression-interpolation.md` — compile-cached template literals: literal short-circuit ladder incl. MAX_SAFE_INTEGER string guard, keyword-filtered context destructuring.
- `./ipc-clean-json.md` — realm-safe JSON sanitizer for cross-process payloads: circular refs, duck-typed Errors, typed-array envelopes, fail-open wrapper.
- `./grpc-client-transport.md` — gRPC transport ladder: URL grammar (unix/pipe grpcs default), insecure-for-local + fail-closed rejectUnauthorized TLS, manual HTTP CONNECT proxy replication, stream-type dispatch, single-fire event latch, close-on-complete channel discipline.
...
- [`agent-cache-lru.md`](./agent-cache-lru.md)
- [`cookie-jar-wrapper.md`](./cookie-jar-wrapper.md)
- [`expression-interpolation.md`](./expression-interpolation.md)
- [`grpc-client-transport.md`](./grpc-client-transport.md)
- [`header-delete-set-null.md`](./header-delete-set-null.md)
- [`ipc-clean-json.md`](./ipc-clean-json.md)
- [`oauth2-additional-params.md`](./oauth2-additional-params.md)
- [`oauth2-authorize-window.md`](./oauth2-authorize-window.md)
- [`oauth2-credential-vault.md`](./oauth2-credential-vault.md)
- [`oauth2-debug-envelope.md`](./oauth2-debug-envelope.md)
- [`oauth2-pkce-state.md`](./oauth2-pkce-state.md)
- [`oauth2-token-lifecycle.md`](./oauth2-token-lifecycle.md)
- [`pac-resolver-cache.md`](./pac-resolver-cache.md)
- [`proxy-normalization-safeexec.md`](./proxy-normalization-safeexec.md)
- [`sent-headers-reconcile.md`](./sent-headers-reconcile.md)
- [`system-proxy-merge.md`](./system-proxy-merge.md)
- [`timeline-agent-instrumentation.md`](./timeline-agent-instrumentation.md)
- [`vault-client-shim.md`](./vault-client-shim.md)
- [`ws-client-fsm.md`](./ws-client-fsm.md)
