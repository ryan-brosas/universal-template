...
- `references/db-claim-locking.md` — atomic `UPDATE…LIMIT` claim stamps, batch co-claiming by `(batch_key, ip_address_id)`, retry debounce, stale-lock sweep.
- `references/worker-process-loop.md` — multi-thread job polling with an exit-pipe interruptible sleep, graceful TERM drain, and pool sizing before threads start.
- `references/worker-role-election.md` — single-row DB election: renew → steal-after-idle → create, releasing on shutdown.
- `references/dequeuer-guard-chain.md` — StopProcessing exception control flow threading one shared State through initial → single → scope processors.
- `references/outgoing-delivery-ladder.md` — ordered pre-send gates, send-limit three-state columns, hard-fail suppression add / success suppression remove.
- `references/incoming-route-modes.md` — bounce linking, spam thresholds vs route Quarantine/Fail modes, endpoint-typed sender dispatch, IP re-allocation on retry.
- `references/smtp-endpoint-session-reuse.md` — MX/relay resolution, endpoint walking with Auto-SSL downgrade, RSET-based session reset, one retry on connection reset, per-batch `finish`.
- `references/send-result-taxonomy.md` — the SendResult contract every sender returns; Net::SMTP exception table; server-busy retry-time parsing; HTTP status classification with 429 bounce suppression.
- `references/ssrf-address-guard.md` — resolve → block-if-ANY-bad → family filter → pin `connection.ipaddr`; negative-code error taxonomy for outbound HTTP.
- `references/webhook-retry-funnel.md` — event fan-out to enabled webhooks, five-step delivery service, fixed retry ladder with attempt-count give-up, attempt ledger in the message DB.
- `references/message-db-per-server.md` — per-server MySQL databases, hand-rolled SQL builder with backtick-doubling identifier escaping, daily raw-message tables with auto-provision retry, slow-query EXPLAIN logging.
- `references/connection-pool-reconnect.md` — lost-connection classification discards the socket and retries the block exactly once on a fresh connection.
- `references/live-stats-minute-window.md` — minute-keyed upsert that resets counters after a 30-minute gap, capped 60-minute reads.
- `references/held-message-expiry.md` — every Held delivery arms `hold_expiry`; a scheduled task mass-cancels expired holds.
- `references/smtp-command-state-machine.md` — phase-gated verb dispatch with labeled error counters and two-line CR memory.
- `references/proxy-protocol-preauth.md` — deferred banner until a strict one-shot PROXY header sets the client IP.
- `references/rcpt-recipient-classification.md` — ordered bounce/route/credential/relay classifier with longest-prefix IP-trust fallback.
- `references/smtp-auth-ladder.md` — PLAIN/LOGIN/CRAM-MD5 resolving `org/server` usernames to one session credential.
