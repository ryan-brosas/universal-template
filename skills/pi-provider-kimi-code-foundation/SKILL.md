...
- `references/payload-mutation-pipeline.md` — how do you apply every vendor-specific payload adaptation in one ordered, testable, in-place step list?
- `references/file-upload-edge.md` — how do you recover a media upload from token rotation without failing the request?
- `references/uploaded-file-cache.md` — how do you avoid re-uploading conversation images when payloads are rebuilt every turn?
- `references/schema-type-backfill.md` — how do you make a strict server-side schema validator accept typeless JSON Schemas from arbitrary tools?
