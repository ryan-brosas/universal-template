...
- `./orchestrator-flatten.md` — how `scrape_jobs` flattens N per-site results into one sorted, column-stable DataFrame.
- `./orchestrator-failure-contract.md` — SCRAPER_MAPPING registry + unshielded futures: why partial success is an adapter-side obligation.
- `./contract.md` — the typed contract: ScraperInput, Scraper ABC, JobPost union schema, Country/JobType routing tables.
- `./employment-type-mappers.md` — JobType alias-tuple lookups: first-vs-last-match divergence, raise-vs-None misses, dead Glassdoor twin.
- `./sessions-proxies.md` — RotatingProxySession, the two asymmetric session flavors, create_session factory.
- `./logging-verbosity-plane.md` — import-before-tune verbose retune of eight JobSpy:* loggers; display-name fixups.
- `./salary-parsing.md` — threshold-based salary extraction, locale-safe currency parsing, annualization.
- `./contact-email-harvest.md` — extract_emails_from_text regex plus the six-adapter wiring matrix and flatten join guard.
- `./scraper-pattern.md` — the per-site adapter pattern with guest endpoints, pagination ceilings, partial-success.
- `./remote-detection.md` — keyword OR-ing across title/description/location, per-site variants.
- `./date-normalization-ladders.md` — per-board recency inputs (ageInDays, format ladders, relative labels, ms epochs) normalized degrade-to-None.
- `./indeed-graphql-driver.md` — Indeed's GraphQL cursor pagination, composite filters, employer-dossier enrichment.
- `./indeed-compensation-mapper.md` — Indeed structured compensation: baseSalary-vs-estimated split provenance, strict interval raise, int() truncation.
- `./glassdoor-adapter.md` — Glassdoor's CSRF bootstrap, location resolution, threaded description fetch.
- `./ziprecruiter-adapter.md` — ZipRecruiter's mobile-app API, session-event bootstrap, continue-token pagination.
- `./google-adapter.md` — Google's SERP async-cursor pagination and brittle nested-JSON extraction.
- `./naukri-adapter.md` — Naukri's placeholder-driven location/salary parsing and Indian Lakh/Crore conversion.
- `./linkedin-guest-pagination.md` — LinkedIn's guest-endpoint cursor ladder, 1000-ceiling, partial-success error policy.
- `./linkedin-card-parsing.md` — LinkedIn search-card → JobPost: salary span, degrade-to-default selectors, datetime fallback (#343).
- `./linkedin-details-enrichment.md` — LinkedIn details-page enrichment: signup guard, criteria tables, applyUrl extraction.
- `./linkedin-util-plane.md` — LinkedIn job-type/level/industry utils: F/P/I/C/T codes, enum-membership traps, live crash edge.
- `./html-scrapers-bdjobs-bayt.md` — BDJobs/Bayt selector-fallback ladders and responsibilities-section extraction.
- `./description-conversion.md` — markdown/plain/HTML converters and the remove-attributes prettify pattern.
...
Add one `./<seam>.md` capsule for one graph-selected, source-confirmed porting question. Add one matching loader line and map entry; keep evidence in the capsule, not this leaf. For a new site, copy the per-site package shape (`__init__.py` + `constant.py` + `util.py`) from `scraper-pattern.md`.
...
- [`contact-email-harvest.md`](./contact-email-harvest.md)
- [`contract.md`](./contract.md)
- [`date-normalization-ladders.md`](./date-normalization-ladders.md)
- [`description-conversion.md`](./description-conversion.md)
- [`employment-type-mappers.md`](./employment-type-mappers.md)
- [`glassdoor-adapter.md`](./glassdoor-adapter.md)
- [`google-adapter.md`](./google-adapter.md)
- [`html-scrapers-bdjobs-bayt.md`](./html-scrapers-bdjobs-bayt.md)
- [`indeed-compensation-mapper.md`](./indeed-compensation-mapper.md)
- [`indeed-graphql-driver.md`](./indeed-graphql-driver.md)
- [`linkedin-card-parsing.md`](./linkedin-card-parsing.md)
- [`linkedin-details-enrichment.md`](./linkedin-details-enrichment.md)
- [`linkedin-guest-pagination.md`](./linkedin-guest-pagination.md)
- [`linkedin-util-plane.md`](./linkedin-util-plane.md)
- [`logging-verbosity-plane.md`](./logging-verbosity-plane.md)
- [`naukri-adapter.md`](./naukri-adapter.md)
- [`orchestrator-failure-contract.md`](./orchestrator-failure-contract.md)
- [`orchestrator-flatten.md`](./orchestrator-flatten.md)
- [`remote-detection.md`](./remote-detection.md)
- [`salary-parsing.md`](./salary-parsing.md)
- [`scraper-pattern.md`](./scraper-pattern.md)
- [`sessions-proxies.md`](./sessions-proxies.md)
- [`ziprecruiter-adapter.md`](./ziprecruiter-adapter.md)
