...
- `references/runloop-exit-machine.md` — when does `_run_stream` stop, and what happens at max steps?
- `references/runloop-code-action-spine.md` — how does model text become executed code and an observation?
- `references/runloop-parallel-tool-fanout.md` — concurrent tool calls without corrupting agent state?
- `references/runloop-managed-agents.md` — sub-agents as callable tools; name/shape rules.
- `references/runloop-planning-cadence.md` — planning_interval gate + summary-mode replan isolation.
- `references/runloop-prompt-template-contract.md` — StrictUndefined templates and per-class variables.
- `references/runloop-structured-outputs-mode.md` — response_format pipeline swap and provider gates.
- `references/runloop-toolcall-parse-fallback.md` — text JSON-blob scraping when providers emit no native calls.
- `references/sandbox-security-ladder.md` — layered denial of imports/builtins/dunders/return-values.
- `references/sandbox-import-authorization-tree.md` — wildcard/prefix import grant semantics.
- `references/sandbox-final-answer-escape.md` — BaseException control signal vs generated try/except.
- `references/sandbox-resource-fences.md` — op counter, loop cap, timeout, print truncation.
- `references/sandbox-state-model.md` — what persists across steps; three call namespaces.
- `references/sandbox-python-semantics-gaps.md` — evaluator divergences from CPython semantics.
- `references/remote-final-answer-patching.md` — exception wrapper serialized across process boundary.
- `references/remote-safe-serializer.md` — safe:/pickle: prefix protocol, two-sided pickle gate.
- `references/remote-jupyter-wire-family.md` — kernel-gateway choreography shared by Docker/Modal/Blaxel.
- `references/remote-e2b-executor.md` — result/error channel translation incl. SDK v1/v2 fork.
- `references/remote-trust-boundary.md` — local "not a sandbox" line vs remote-only/local-only features.
- `references/tools-contract-gates.md` — definition/serialization/call-time validation layers.
- `references/tools-decorator-source-capture.md` — @tool AST source capture for self-regenerating tools.
- `references/tools-mcp-ingestion.md` — MCP server → Tool adaptation and transport rules.
- `references/tools-registry-deserialization.md` — AGENT/MODEL registries blocking arbitrary instantiation.
- `references/tools-agent-packaging.md` — save()/push_to_hub artifact bundle + requirements discovery.
- `references/default-tools-family.md` — base tool set and interchangeable web_search twins.
- `references/model-completion-precedence.md` — kwargs merge ladder + REMOVE_PARAMETER sentinel.
- `references/model-stop-parameter-matrix.md` — o3/o4/gpt-5/grok denylist + client-side trimming.
- `references/model-message-cleaning.md` — role conversion, image encoding, consecutive-role merging.
- `references/model-stream-stitching.md` — index-keyed delta reassembly rules.
- `references/model-api-resilience.md` — rate-limit retry/backoff + dual truncation budgets.
- `references/memory-step-rendering.md` — steps → messages projection; error coaching; plan pivot.
- `references/agent-types-boundary-wrapping.md` — AgentImage/Audio raw/string duality at boundaries.
- `references/monitoring-rich-safety.md` — Text-not-markup console discipline; Monitor counters.
- `references/gradio-streaming-surface.md` — consuming the run generator in a chat UI.
- `references/replay-diagnostics-surface.md` — print-only step replay; why every replay log is ERROR level.
- `references/run-reset-memory-contract.md` — what run(reset=True/False) wipes vs keeps (state survives).
- `references/memory-step-dict-serialization.md` — step `.dict()` shapes; lossy image bytes vs raw retention.
- `references/chatmessage-value-plane.md` — tool-call coercion on construct, role re-typing on from_dict, raw-stripping wire JSON.
- `references/agent-error-taxonomy.md` — six-class error tree, log-on-construct side effect, {type,message} dict contract.
- `references/make-json-serializable-coercion.md` — local heuristic codec; JSON-looking strings promoted to containers.
- `references/tool-schema-synthesis.md` — get_json_schema pipeline; docstring-required Google format, choices→enum, OpenAI envelope.
- `references/type-hint-schema-vocabulary.md` — hint→type ladder without pydantic; union folding, tuple prefixItems rejections, Image/Tensor specials.
- `references/tool-calltime-argument-validation.md` — two-pass arg check at execute_tool_call only; int→number widening; Tool.__call__ never validates.
- `references/nullable-semantics-overload.md` — one flag = has-default + accepts-None + omittable; two acknowledged-bug skipped tests.
- `references/method-checker-name-resolution.md` — ten-set allowlist machine; dead check_imports, visit_Call asymmetry, self.* blind spot.
- `references/class-body-tool-rules.md` — literal-only class attrs, literal-defaulted __init__ via zip_longest alignment; serialization-gate consumers.
