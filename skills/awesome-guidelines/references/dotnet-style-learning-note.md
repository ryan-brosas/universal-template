...
**Scope:** Cross-language .NET **public API and security baseline** (libraries, shared components, multi-language repos). Language syntax/layout: load `csharp-coding-practices`, `vb-coding-practices`, or `fsharp-coding-practices`. Stack patterns (ASP.NET, etc.): load stack capsules in `skills/*-foundation`.
