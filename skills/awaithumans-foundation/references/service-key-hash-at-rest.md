...
**Source:** awaithumans Apache-2.0 `main@bc05b8e7`; Codebase Memory `mnt-hdd-utopia-inspo-agents-awaithumans`. **Question:** What must a machine-credential service do so a DB leak yields no usable keys and revocation never leaks existence?
...
**Retrieve:**
```ts
await mcp.codebase_memory.search_graph({ project: "mnt-hdd-utopia-inspo-agents-awaithumans", query: "create_service_key verify_service_key _ulid key_hash", limit: 4 });
```
