...
**Probe:** `cd /mnt/hdd/utopia/inspo/external/open-computer-use && sed -n '10,25p' os_computer_use/streaming.py && grep -n 'killpg\|setsid' os_computer_use/streaming.py && grep -n 'set_timeout' os_computer_use/sandbox_agent.py` (pins listen-server command :12, group-kill :50/:43, and per-turn keep-alive).
**Retrieve:**
```ts
