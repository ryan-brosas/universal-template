...
**Probe:** `bash -c 'cd /mnt/hdd/utopia/inspo/pi-hypercharm-provider && grep -c "AbortSignal.any\|AbortSignal.timeout" index.ts'` → 3; `grep -c creditsInFlight index.ts` → 5; `grep -c "metaFetched = false" index.ts` → 4. Runtime path untested upstream — coverage caveat recorded. (Counts re-verified at HEAD 4520704.)
