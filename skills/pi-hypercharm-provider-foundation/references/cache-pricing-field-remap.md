...
**Probe:** direct test `tests/status.smoke.ts` covers none of this plane (catalog transform) — deterministic probes instead: `bash -c 'cd /mnt/hdd/utopia/inspo/pi-hypercharm-provider && grep -c cost_per_1m_out_cached index.ts scripts/update-models.js'` → 1 file : 1 file; `grep -n "cacheWrite: 0" index.ts` → NO match (the old constant is gone); committed `models.json` rows carry non-zero cacheWrite where the API reports it (782-line golden snapshot). Coverage caveat recorded.
