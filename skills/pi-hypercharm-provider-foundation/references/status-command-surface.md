...
**Probe:** `bash -c 'cd /mnt/hdd/utopia/inspo/pi-hypercharm-provider && grep -cF "Usage: /hypercharm-status" index.ts'` → 1; `grep -c "Number.isFinite(n) || n <= 0" index.ts` → 1; `grep -cF '"off", "10", "25", "50", "100", "200", "500"' index.ts` → 1; `grep -c "% modes.length" index.ts` → 1. Runtime path untested upstream — coverage caveat recorded.
