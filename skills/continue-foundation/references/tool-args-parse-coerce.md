...
**Probe:** `core/tools/parseArgs.vitest.ts` pins the contract directly (:379–417 coerce block: skips keys not in schema, never coerces numbers/booleans to strings, does not mutate the original args object; :419–447 accessor block: parses numeric strings, floors decimals `"42.7"`⇒42, throws typed ``Argument `count` is required (type number)``). Port by extending these table cases to your schema walker.
