...
- `./antibot-tiered-detector.md` - three detection tiers sized 15KB/<10KB/<50KB with data-response exemptions and 403/503-always-block.
- `./antibot-retry-proxy-ladder.md` - attempts x proxies nesting, exception-swallow rule, fallback-fetch promotion, and the crawl_stats ledger every result carries.
- `./smart-cache-freshness.md` - 304/head-fingerprint validation with FRESH/STALE/UNKNOWN/ERROR mapped to hit_validated/recrawl/hit_fallback.
- `./content-hash-db-cache.md` - task-id-pooled aiosqlite (WAL) whose rows store content hashes pointing at filesystem blobs, UPSERTed with etag/lastmod/fingerprint metadata.
- `./cache-mode-context.md` - five CacheModes collapsed onto should_read/should_write predicates with legacy-flag precedence.
- `./deep-crawl-decorator-guard.md` - ContextVar latch installed by decorating arun at construction so nested arun calls fall through to single-URL mode.
- `./bfs-level-loop-resume.md` - level-synchronous traversal cloning deep_crawl_strategy away per delegation, success-gated budgets, and a four-key resume snapshot.
- `./url-filter-chain-scoring.md` - sync-inline/async-gather hybrid chain feeding scored-capacity-trimmed link selection with boundary-checked glob buckets.
- `./seeder-pipeline-backpressure.md` - bounded queue + global QPS semaphore + drain-flush early-stop so max_urls cuts off without hanging joins.
- `./sitemap-cache-lastmod.md` - HEAD-verified sitemap discovery, lastmod-TTL-validated JSON cache envelope, sentinel-counted index fan-out, and write-while-streaming CC mirrors.
- `./browser-recycle-drain.md` - whitelist config signatures + refcounted contexts + version-bump drain capped at 3 pending browsers with 30s force-clean.
- `./managed-browser-prelaunch.md` - port-owner kill, singleton-lock removal, detached process groups, verify-then-connect CDP, and terminate->killpg escalation.
...
- [`antibot-retry-proxy-ladder.md`](./antibot-retry-proxy-ladder.md)
- [`antibot-tiered-detector.md`](./antibot-tiered-detector.md)
- [`bfs-level-loop-resume.md`](./bfs-level-loop-resume.md)
- [`browser-recycle-drain.md`](./browser-recycle-drain.md)
- [`cache-mode-context.md`](./cache-mode-context.md)
- [`content-hash-db-cache.md`](./content-hash-db-cache.md)
- [`deep-crawl-decorator-guard.md`](./deep-crawl-decorator-guard.md)
- [`managed-browser-prelaunch.md`](./managed-browser-prelaunch.md)
- [`seeder-pipeline-backpressure.md`](./seeder-pipeline-backpressure.md)
- [`sitemap-cache-lastmod.md`](./sitemap-cache-lastmod.md)
- [`smart-cache-freshness.md`](./smart-cache-freshness.md)
- [`url-filter-chain-scoring.md`](./url-filter-chain-scoring.md)
