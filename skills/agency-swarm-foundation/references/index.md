...
- `./send-message-bus.md` — synchronous agent-to-agent delegation: pending-set backpressure, errors-as-tool-results, parent-run-id plumbing.
- `./tool-concurrency-guard.md` — wrap-on-add two-tier guard (busy latch + sequential flag) rejecting violations as error strings.
- `./flow-wiring-validation.md` — parse_agent_flows duplicate taxonomy (default-pair vs custom vs same-class) + always-give-a-route fallback.
- `./runtime-subagent-registry.md` — AgentRuntimeState-keyed tools shared BY REFERENCE; add_recipient mutates enum AND description roster.
- `./flat-thread-store.md` — metadata-filtered projections: callerAgent-None = ONE global user thread; agent pairs match BOTH directions.
- `./history-sanitization-ladder.md` — repair → strip → id-dedup order; mixed history protocols raise, never coerce.
- `./guardrail-retry-loop.md` — output tripwires append feedback and retry on grown history; input tripwires fail open as final text.
- `./handoff-reminder-filter.md` — Literal-schema-unified handoffs; reminder stamped timestamp+1 with full caller attribution.
- `./deferred-mcp-oauth.md` — startup never blocks on OAuth discovery; shielded mid-run conversion; owner-scoped tool storage.
- `./loop-affine-mcp-proxy.md` — replace-with-proxy so every call site inherits background-loop affinity; TimeoutError/CancelledError relabeling.
- `./fake-id-stream-normalization.md` — (run_id, output_index)-keyed stable ids; call_id precedence; kind-deque pairing for run-item events.
- `./starter-cache-replay.md` — exact preconditions for cached first replies; any override bypasses; fingerprint mismatch invalidates.
- `./send-message-extra-params.md` — three-pattern subclass param discovery with MRO-wide reserved names; degrade-not-crash merge.
- `./instructions-sandwich-trace.md` — shared→base→additional composition with mandatory restore; regex-gated trace-id inheritance.
...
- [`deferred-mcp-oauth.md`](./deferred-mcp-oauth.md)
- [`fake-id-stream-normalization.md`](./fake-id-stream-normalization.md)
- [`flat-thread-store.md`](./flat-thread-store.md)
- [`flow-wiring-validation.md`](./flow-wiring-validation.md)
- [`guardrail-retry-loop.md`](./guardrail-retry-loop.md)
- [`handoff-reminder-filter.md`](./handoff-reminder-filter.md)
- [`history-sanitization-ladder.md`](./history-sanitization-ladder.md)
- [`instructions-sandwich-trace.md`](./instructions-sandwich-trace.md)
- [`loop-affine-mcp-proxy.md`](./loop-affine-mcp-proxy.md)
- [`runtime-subagent-registry.md`](./runtime-subagent-registry.md)
- [`send-message-bus.md`](./send-message-bus.md)
- [`send-message-extra-params.md`](./send-message-extra-params.md)
- [`starter-cache-replay.md`](./starter-cache-replay.md)
- [`tool-concurrency-guard.md`](./tool-concurrency-guard.md)
