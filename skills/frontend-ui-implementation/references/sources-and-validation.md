...
The original complete commands were not retained, and the first probe's thinking setting is unknown. The following retained prompts support a new explicitly configured evaluation, not an exact replay of the historical runs:
...
Full response artifacts and complete invocation settings were not preserved in the publication record; the reported scores are unverified historical observations, not reproducible benchmark evidence or revision certification. Do not reconstruct missing settings or fabricate matching responses. A future evaluation must save its exact command (provider, model, thinking, skill revision, and prompt) together with its response. A prior no-tools probe could not load branch references and therefore declined to quote exact PDF values. That failure is retained: agents must read the branch named by `SKILL.md`; entry-file retrieval alone is insufficient for detailed implementation.
