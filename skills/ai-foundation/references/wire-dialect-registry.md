...
**Probe:** `grep -n "if (!chunk.success)" $REFERENCE_ROOT/ai/packages/ai/src/ui/default-chat-transport.ts && grep -c "it(" $REFERENCE_ROOT/ai/packages/ai/src/ui/direct-chat-transport.test.ts` → :28 and 7.
