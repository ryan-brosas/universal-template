...
- `./word-index-mmap-promotion.md` — When does a read-only index pay heap materialization, and what state does each mode hold?
- `./word-index-removal-and-shards.md` — How do bulk-loaded docs get deleted, and why does merged shard order reproduce serial doc ids?
- `./trigram-bloom-masks.md` — Which candidate Bloom filters are sound, and why must masks be per-file?
- `./regex-decomposition-soundness.md` — When must regex prefiltering be abandoned entirely to avoid silent misses?
- `./sparse-ngram-boundaries.md` — How do variable content-defined n-grams stay substring-searchable without alignment?
- `./trigram-overlay-lattice.md` — Why promote an mmap index to base+overlay instead of materializing on first write?
- `./disk-format-status-meta.md` — What does OOM-proof lazy loading look like, and which counts must never build the engine?
- `./tiered-search-ladder.md` — Where does each tier early-exit and which tiers are provably skippable?
- `./rerank-signal-composition.md` — Which boosts are multiplicative ≥1 (never filters) and how do priors layer?
- `./bm25-plus-ranked-search.md` — What exact BM25 variant runs, and why does N fall back when doc lengths are missing?
- `./bm25-accounting-subtokens.md` — Which fields constitute ranking state, and when is an identifier split?
- `./call-graph-weight-split.md` — How are calls resolved without type info, and why must graph signals be additive-only?
- `./import-graph-basename-duality.md` — When must basename matching in imported-by be refused as ambiguous?
- `./content-cache-clock-borrowed.md` — How does one cache hold owned copies and borrowed mmap views under a byte budget?
- `./generation-caches.md` — What makes caching search results safe while lazy signals build mid-flight?
- `./line-offset-cache.md` — What identity check keeps cached byte-offset tables stale-proof over reusable buffers?
- `./lazy-completion-latches.md` — Which completion flags gate "index complete" and how do fast-loaded snapshots defer honestly?
...
- [`bm25-accounting-subtokens.md`](./bm25-accounting-subtokens.md)
- [`bm25-plus-ranked-search.md`](./bm25-plus-ranked-search.md)
- [`call-graph-weight-split.md`](./call-graph-weight-split.md)
- [`content-cache-clock-borrowed.md`](./content-cache-clock-borrowed.md)
- [`disk-format-status-meta.md`](./disk-format-status-meta.md)
- [`generation-caches.md`](./generation-caches.md)
- [`import-graph-basename-duality.md`](./import-graph-basename-duality.md)
- [`lazy-completion-latches.md`](./lazy-completion-latches.md)
- [`line-offset-cache.md`](./line-offset-cache.md)
- [`regex-decomposition-soundness.md`](./regex-decomposition-soundness.md)
- [`rerank-signal-composition.md`](./rerank-signal-composition.md)
- [`sparse-ngram-boundaries.md`](./sparse-ngram-boundaries.md)
- [`tiered-search-ladder.md`](./tiered-search-ladder.md)
- [`trigram-bloom-masks.md`](./trigram-bloom-masks.md)
- [`trigram-overlay-lattice.md`](./trigram-overlay-lattice.md)
- [`word-index-mmap-promotion.md`](./word-index-mmap-promotion.md)
- [`word-index-removal-and-shards.md`](./word-index-removal-and-shards.md)
