...
- **WASM plane** — `wasm-parser-section-finalize`, `wasm-import-rewriter-binary-surgery`, `wasm-api-handle-table-gen-server`: the pure-Elixir disassembler (metadata + import oracle), order-validated import injection as binary surgery, and the integer handle table that bridges JS `WebAssembly.*` builtins to NIF module/instance refs.
