...
- `references/interval-subtraction-ladder.md` — carve booked time out of a provider's day: five-case overlap grammar, exception-replaces-plan rule.
- `references/break-removal-four-case.md` — in-place break/unavailability subtraction twins with fragment chaining.
- `references/multi-attendant-group-slots.md` — sliding-window slots for group services; same-service counts, other-service vetoes.
- `references/provider-tz-booking-windows.md` — book-advance timeout + future booking limit thresholds in provider timezone, terminal sort.
- `references/conflict-gate-two-grammars.md` — save-time strict overlap predicate vs boundary-shifted slot counting; UI gate vs API permissiveness.
- `references/any-provider-resolution.md` — `'any-provider'` sentinel → max-free-hours provider ranking with hour filter.
- `references/webhook-dispatcher.md` — comma-action matching, custom secret header envelope, swallow-and-log synchronous delivery, `is_active` trap.
- `references/api-dual-auth.md` — timing-safe bearer token → admin-gated Basic → 401 challenge ladder with multi-source header reading.
- `references/customer-access-scoping.md` — appointment-relationship-derived customer visibility with one-hop secretary delegation.
- `references/permission-bitmask-ladder.md` — 1/2/4/8 view/add/edit/delete decode highest-bit-first; deliberate add/edit swap on save paths.
- `references/two-tier-rate-limiting.md` — global 100/120s per-IP limiter vs login 5/300s counter; fail-closed vs fail-open postures.
- `references/login-hardening-ladder.md` — charset allowlist, DB→LDAP fallback, randomized uniform failures, delete-on-regenerate sessions.
- `references/legacy-hash-migration.md` — bcrypt cost 12 with lazy upgrade of iterated-SHA-256 legacy hashes on successful login.
- `references/best-effort-sync-fanout.md` — Google/CalDAV mirror-id-keyed create/update with immediate write-back and log-and-continue errors.
- `references/retention-cleanup.md` — retention purge predicate (no live/future appointments AND old enough), per-row failure isolation.
- `references/api-resource-mapping.md` — camelCase↔snake_case maps, two-gate ORDER BY sanitization, keyExists-guarded partial decodes.
- `references/session-reconciliation-boot.md` — every-request session-vs-DB reconciliation plus the global rate-limit call in EA_Controller.
