...
- `./connection-dispatch-lifecycle.md` — how does one message pipe serve both promise responses and object events?
- `./guid-object-tree.md` — how do remote objects get created, adopted, GC-collected, and disposed?
- `./proxy-channel-apiscope.md` — why is every channel a Proxy, and when does an API call get reported?
- `./zones-asynclocalstorage.md` — how does call metadata cross awaits without leaking into server events?
- `./eventemitter-async-safety.md` — what must a hand-rolled EventEmitter guarantee for async listeners?
- `./subscription-mapping.md` — when do protocol events actually get subscribed on the wire?
- `./waiter-failure-race.md` — how does waitForEvent lose to crash/close/timeout without leaking listeners?
- `./progress-controller-server.md` — how does the server turn a deadline into cancellation?
- `./timeout-settings-ladder.md` — which timeout wins between option, default, debug mode, and parent?
- `./typed-error-roundtrip.md` — which errors survive the wire as classes, and how are stacks rebuilt?
- `./library-stack-capture.md` — how does the client derive the apiName and user-only stack for a call?
- `./abort-signal-plumbing.md` — how do user-supplied AbortSignals cancel an in-flight RPC?
- `./long-standing-scope-terminate-close.md` — how does one peer-death event cancel every present AND future waiter, without sharing a broken stack?
- `./deadline-poll-ladder.md` — how do you poll a flaky predicate until it passes, bounded by one monotonic deadline?
- `./await-must-use-progress-rule.md` — how is "every awaited call is cancellable" enforced at CI time?
- `./retry-nonretriable-taxonomy.md` — how does a retry loop wake instantly on page close, and which errors must never be retried?
- `./launch-process-fd-layout.md` — how do you spawn a peer process so its whole tree is killable and it speaks on fds 3/4?
- `./graceful-close-ladder.md` — how does shutdown degrade from polite protocol message to timeout to force-kill without zombies?
- `./signal-handler-refcount.md` — how do N launched processes share SIGINT/SIGTERM/exit handlers and uninstall them at exactly the right moment?
- `./kill-tree-portability.md` — how do you force-kill a child tree synchronously on Windows and POSIX?
- `./pipe-transport-framing.md` — how do you frame a raw byte stream into length-prefixed messages without re-entrancy or post-close writes?
- `./launch-ready-state-race.md` — how does launch fail fast when the peer dies before ready, with actionable startup logs?
- `./browser-close-idempotence.md` — how does close() stay idempotent under concurrent callers and fan death out to every dependent?
...
- [`abort-signal-plumbing.md`](./abort-signal-plumbing.md)
- [`await-must-use-progress-rule.md`](./await-must-use-progress-rule.md)
- [`browser-close-idempotence.md`](./browser-close-idempotence.md)
- [`connection-dispatch-lifecycle.md`](./connection-dispatch-lifecycle.md)
- [`deadline-poll-ladder.md`](./deadline-poll-ladder.md)
- [`eventemitter-async-safety.md`](./eventemitter-async-safety.md)
- [`graceful-close-ladder.md`](./graceful-close-ladder.md)
- [`guid-object-tree.md`](./guid-object-tree.md)
- [`kill-tree-portability.md`](./kill-tree-portability.md)
- [`launch-process-fd-layout.md`](./launch-process-fd-layout.md)
- [`launch-ready-state-race.md`](./launch-ready-state-race.md)
- [`library-stack-capture.md`](./library-stack-capture.md)
- [`long-standing-scope-terminate-close.md`](./long-standing-scope-terminate-close.md)
- [`pipe-transport-framing.md`](./pipe-transport-framing.md)
- [`progress-controller-server.md`](./progress-controller-server.md)
- [`proxy-channel-apiscope.md`](./proxy-channel-apiscope.md)
- [`retry-nonretriable-taxonomy.md`](./retry-nonretriable-taxonomy.md)
- [`signal-handler-refcount.md`](./signal-handler-refcount.md)
- [`subscription-mapping.md`](./subscription-mapping.md)
- [`timeout-settings-ladder.md`](./timeout-settings-ladder.md)
- [`typed-error-roundtrip.md`](./typed-error-roundtrip.md)
- [`waiter-failure-race.md`](./waiter-failure-race.md)
- [`zones-asynclocalstorage.md`](./zones-asynclocalstorage.md)
