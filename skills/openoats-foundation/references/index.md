...
- `./session-canonical-layout-seeding.md` — what exactly does a new session persist, and with which durability/permission guarantees?
- `./live-transcript-filehandle-lifecycle.md` — when is the live JSONL handle opened, kept open, and closed?
- `./delayed-enrichment-pending-write-drain.md` — how do you await in-flight async enrichment writes before finalize/shutdown?
- `./final-transcript-tmp-swap-prebatch-backup.md` — how is a destructive final-transcript overwrite made recoverable?
- `./batch-audio-stash-rerun-window.md` — how are batch audio stems retained for reruns yet garbage-collected?
- `./ghost-session-reconciliation.md` — when does an empty calendar-bearing session merge into a real one instead of lingering?
- `./abandoned-session-resume-election.md` — which prior unfinished row may a restart resume instead of creating a duplicate?
- `./internal-tag-preserving-update.md` — how do user tag edits avoid destroying importer-namespaced tags?
- `./imported-manual-session-dedup.md` — how do you create non-recorded session rows without duplicating the one a calendar event already owns.
- `./jsonl-tolerant-repair-backfill.md` — how do you fold late-arriving cleaned text into an append-only transcript without corrupting or losing lines.
- `./notes-mirror-plane.md` — how do you export session notes to a user-chosen folder without blocking the actor or following malicious markdown links.
- `./attachment-import-plaintext-export.md` — how do you import user files into a session store without letting hostile filenames or failed copies corrupt metadata?
- `./finalize-consumer-ladder.md` — in what order must a live session be drained, exported, and collapsed before the UI is told it ended?
- `./audio-retention-empty-session-classification.md` — how do you decide, from pure inputs, whether a session keeps its audio and what went wrong when it ended with zero text?
- `./notes-selection-draft-plane.md` — how do you switch between sessions without losing an unsaved draft or leaking the previous session's state?
...
- [`abandoned-session-resume-election.md`](./abandoned-session-resume-election.md)
- [`attachment-import-plaintext-export.md`](./attachment-import-plaintext-export.md)
- [`audio-retention-empty-session-classification.md`](./audio-retention-empty-session-classification.md)
- [`batch-audio-stash-rerun-window.md`](./batch-audio-stash-rerun-window.md)
- [`delayed-enrichment-pending-write-drain.md`](./delayed-enrichment-pending-write-drain.md)
- [`final-transcript-tmp-swap-prebatch-backup.md`](./final-transcript-tmp-swap-prebatch-backup.md)
- [`finalize-consumer-ladder.md`](./finalize-consumer-ladder.md)
- [`ghost-session-reconciliation.md`](./ghost-session-reconciliation.md)
- [`imported-manual-session-dedup.md`](./imported-manual-session-dedup.md)
- [`internal-tag-preserving-update.md`](./internal-tag-preserving-update.md)
- [`jsonl-tolerant-repair-backfill.md`](./jsonl-tolerant-repair-backfill.md)
- [`live-transcript-filehandle-lifecycle.md`](./live-transcript-filehandle-lifecycle.md)
- [`notes-mirror-plane.md`](./notes-mirror-plane.md)
- [`notes-selection-draft-plane.md`](./notes-selection-draft-plane.md)
- [`session-canonical-layout-seeding.md`](./session-canonical-layout-seeding.md)
