...
Use Fovea for active-project structure or Codebase Memory (`codebase-memory_get_architecture` → `codebase-memory_search_graph`) to build the map; confirm in source. Name every module boundary and call direction before drawing conclusions.
...
2. Build the map with Fovea (active-project structure) or Codebase Memory (`get_architecture` → `search_graph`); confirm claims in source.
