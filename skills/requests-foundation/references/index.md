...
- `./session-setting-merge.md` — four-arm precedence for request-vs-session settings; None strips the key; merge_hooks empty-list suppression.
- `./adapter-prefix-mount.md` — mount maintains longest-prefix-first insertion order; get_adapter first-match dispatch.
- `./session-redirect-resolution.md` — per-hop drain→cap→close→normalize→rebuild choreography and history slicing.
- `./session-strip-auth.md` — should_strip_auth decision table incl. http→https standard-port carve-out; netrc re-application order.
- `./session-rebuild-method.md` — 303/302 downgrade-all-except-HEAD, 301 downgrades POST only, 307/308 untouched.
- `./session-rebuild-proxies.md` — strip Proxy-Authorization before re-evaluating; never inject into TLS tunnels.
- `./session-environment-settings.md` — trust_env proxies via setdefault; REQUESTS_CA_BUNDLE only overrides unset verify.
- `./session-send-pipeline.md` — send stage ordering: guard→adapter→elapsed→hooks→cookies→redirects→assembly→consume.
- `./session-next-lookahead.md` — yield_requests lookahead populates Response.next with full rebuild side effects.
- `./cookies-mock-bridge.md` — MockRequest/MockResponse adapt PreparedRequest+urllib3 to stdlib CookieJar extract/add.
- `./cookie-jar-dict-facade.md` — RequestsCookieJar O(n) dict ops, conflict vs default, None-deletes, pickle lock dance.
- `./adapter-error-taxonomy.md` — MaxRetryError reason-type dispatch incl. NewConnectionError exclusion; hierarchy-as-API.
- `./adapter-pool-manager.md` — Retry(0, read=False) default; poolmanager pickle contract; per-proxy manager cache.
- `./adapter-tls-pool-keys.md` — verify/cert land in PoolKeys so TLS changes fork pools; get_connection_with_tls_context migration.
- `./adapter-cert-verify.md` — bundle existence fail-fast; explicit CA-state clearing on verify=False for reused connections.
- `./adapter-request-url-shaping.md` — absolute-form only for plaintext-http-through-proxy; urldefragauth strips creds.
- `./adapter-send-prelude.md` — chunked predicate, strict timeout-tuple error, four load-bearing urlopen flags.
- `./adapter-build-response.md` — field mapping defaults, encoding-from-headers precedence, connection back-reference requirement.
- `./models-prepare-order.md` — seven fixed stages; auth before hooks (both documented); Content-Length recomputed post-auth.
- `./models-body-preparation.md` — json/data/files/stream tree; allow_nan=False; tri-state _body_position rewind contract.
- `./models-url-preparation.md` — non-http bypass, IDNA/wildcard host gates, param merging, whole-URL requote.
- `./response-content-plane.md` — iter_content exception remaps, tri-state _content, iter_lines pending-line rule, json BOM path.
- `./utils-proxy-selection.md` — four-key proxy precedence; NO_PROXY anchored-suffix + CIDR ladder (bpo-39057).
- `./utils-super-len.md` — remaining-bytes probe ladder with position restore and two OSError adjudications.
- `./utils-netrc-auth.md` — $NETRC override, .netrc/_netrc fallback, fail-silent posture, account-slot login fallback.
- `./utils-requote-uri.md` — unquote-unreserved then quote with %-safe list; bare-percent fallback differs by exactly one char.
- `./utils-json-charset.md` — BOM-first then null-position utf-8/16/32 detection from four bytes.
- `./auth-digest-state-machine.md` — threading.local nonce state, one-shot 401 retry cap, resend via r.connection.send.
- `./hooks-dispatch-contract.md` — single-callable tolerance, None-pass-through chaining, pre-cookie/pre-redirect timing.
- `./exceptions-hierarchy-context.md` — RequestException context back-fill; ConnectTimeout/ContentDecodingError diamond roots.
- `./structures-header-plane.md` — CaseInsensitiveDict tuple-store design; prepare-time header regex validation.
- `./session-shutdown-cascade.md` — with-block exit → Session.close → per-adapter poolmanager+proxy_manager clear.
- `./response-lifecycle-close.md` — close releases unconsumed raw + always release_conn; pickling forces consumption.
- `./response-ok-raise-plane.md` — raise windows are exactly 4xx/5xx; bytes reason utf-8→latin fallback; bool(response)==not-error.
- `./utils-header-links.md` — `, *<` entry split, one-`;` URL/params split, no-= truncates remaining params; rel-keyed with url fallback.
- `./utils-unicode-streaming.md` — encoding-None passes bytes through; incremental decoder + final flush; replay re-slices memoized _content.
- `./auth-basic-encoding.md` — latin-1 wire encode, deprecation shim for non-str, Proxy-Authorization subclass override.
- `./structures-status-registry.md` — alias setattr registry with empty dict store; loud attr miss vs silent item miss.
...
- [`adapter-build-response.md`](./adapter-build-response.md)
- [`adapter-cert-verify.md`](./adapter-cert-verify.md)
- [`adapter-error-taxonomy.md`](./adapter-error-taxonomy.md)
- [`adapter-pool-manager.md`](./adapter-pool-manager.md)
- [`adapter-prefix-mount.md`](./adapter-prefix-mount.md)
- [`adapter-request-url-shaping.md`](./adapter-request-url-shaping.md)
- [`adapter-send-prelude.md`](./adapter-send-prelude.md)
- [`adapter-tls-pool-keys.md`](./adapter-tls-pool-keys.md)
- [`auth-basic-encoding.md`](./auth-basic-encoding.md)
- [`auth-digest-state-machine.md`](./auth-digest-state-machine.md)
- [`cookie-jar-dict-facade.md`](./cookie-jar-dict-facade.md)
- [`cookies-mock-bridge.md`](./cookies-mock-bridge.md)
- [`exceptions-hierarchy-context.md`](./exceptions-hierarchy-context.md)
- [`hooks-dispatch-contract.md`](./hooks-dispatch-contract.md)
- [`models-body-preparation.md`](./models-body-preparation.md)
- [`models-prepare-order.md`](./models-prepare-order.md)
- [`models-url-preparation.md`](./models-url-preparation.md)
- [`response-content-plane.md`](./response-content-plane.md)
- [`response-lifecycle-close.md`](./response-lifecycle-close.md)
- [`response-ok-raise-plane.md`](./response-ok-raise-plane.md)
- [`session-environment-settings.md`](./session-environment-settings.md)
- [`session-next-lookahead.md`](./session-next-lookahead.md)
- [`session-rebuild-method.md`](./session-rebuild-method.md)
- [`session-rebuild-proxies.md`](./session-rebuild-proxies.md)
- [`session-redirect-resolution.md`](./session-redirect-resolution.md)
- [`session-send-pipeline.md`](./session-send-pipeline.md)
- [`session-setting-merge.md`](./session-setting-merge.md)
- [`session-shutdown-cascade.md`](./session-shutdown-cascade.md)
- [`session-strip-auth.md`](./session-strip-auth.md)
- [`structures-header-plane.md`](./structures-header-plane.md)
- [`structures-status-registry.md`](./structures-status-registry.md)
- [`utils-header-links.md`](./utils-header-links.md)
- [`utils-json-charset.md`](./utils-json-charset.md)
- [`utils-netrc-auth.md`](./utils-netrc-auth.md)
- [`utils-proxy-selection.md`](./utils-proxy-selection.md)
- [`utils-requote-uri.md`](./utils-requote-uri.md)
- [`utils-super-len.md`](./utils-super-len.md)
- [`utils-unicode-streaming.md`](./utils-unicode-streaming.md)
