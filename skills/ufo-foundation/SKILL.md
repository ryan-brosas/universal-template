...
- `references/constellation-ready-task-loop.md` — how the orchestrator schedules ready tasks without double-executing them.
- `references/two-copy-dag-state-merge.md` — how agent structure edits merge with orchestrator execution state without losing completions.
- `references/modification-wait-fail-open.md` — how execution pauses for pending edits yet never deadlocks on timeout.
- `references/constellation-cancellation-ladder.md` — how per-constellation cancellation flags and future cancellation compose.
- `references/task-execution-error-funnel.md` — how exceptions become FAILED results and events while dependents still update.
- `references/dag-validation-kahn.md` — how cycle and dangling-dependency detection works before execution.
- `references/dependency-condition-promotion.md` — how completing one task promotes dependents via edge conditions.
- `references/round-state-machine-pump.md` — how sessions pump an agent status FSM to completion with force-finish and step-budget termination.
- `references/fire-forget-orchestration-event-drain.md` — how the planner launches the executor un-awaited and reacts via a coalescing event drain.
- `references/typed-completion-event-gate.md` — how one whitelisted producer keeps terminal-only events in the completion queue.
- `references/merged-base-editing.md` — which constellation copy an editor must re-merge against before each edit pass.
- `references/device-assignment-validation.md` — fail-fast whole-graph device-binding validation plus named assignment strategies.
- `references/busy-device-queue-contract.md` — uniform-await task submission that queues behind busy workers and rejects dead targets.
- `references/pending-task-disconnect-cancellation.md` — device-scoped future cancellation so worker loss unblocks every waiter immediately.
