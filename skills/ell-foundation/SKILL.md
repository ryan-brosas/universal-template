---
name: ell-foundation
description: 'Porting ell (language model programming): LMP decorators, _lstr origin tracing, lexical-closure versioning, provider SPI, SQL/alembic store, evaluations, studio.'
---
...
- `references/lmp-decorator-pipeline.md` — How does a plain function become a tracked, provider-calling LMP?
- `references/lstr-origin-trace-propagation.md` — How do I keep provenance on text through concatenation, formatting, slicing, joins and arbitrary str methods?
- `references/lstr-pydantic-roundtrip-schema.md` — How does a str subclass survive pydantic serialization without losing its metadata?
- `references/content-block-single-field-coercion.md` — How do heterogeneous inputs become validated single-payload content blocks?
- `references/message-tool-call-result-plane.md` — How do tool calls ride inside messages and get executed into follow-up messages?
- `references/tool-decorator-params-model.md` — How do I synthesize a typed JSON schema for a tool from its signature?
- `references/tool-result-content-block-envelope.md` — What exact shape does an invoked tool return, per result type?
- `references/provider-call-lifecycle-validation.md` — Where is the single choke point every provider call passes through, and what does it assert?
- `references/openai-stream-default-and-retraction.md` — Why does ell stream by default and when does it silently stop?
- `references/anthropic-role-collapse-and-stream-reassembly.md` — How are alternating roles collapsed and streamed blocks reassembled for Anthropic?
- `references/bedrock-converse-provider-dual.md` — How does one provider class serve both converse and converse_stream?
- `references/provider-registry-configurator.md` — How do clients map to providers and models to clients at call time?
- `references/lexical-closure-recursion.md` — How is a function's full dependency source extracted as standalone code?
- `references/closure-hash-and-versioning.md` — What exactly is hashed into an LMP version id, and when?
- `references/should-import-boundary.md` — When is a dependency emitted as an import versus inlined as source?
- `references/track-invocation-stack.md` — How are nested LMP calls linked into parent-child invocation trees across threads?
- `references/state-cache-key.md` — What identity makes one LMP invocation cache-equivalent to another?
- `references/invocation-blob-externalization.md` — When do invocation payloads move out of the row into blob storage?
- `references/sqlstore-idempotent-writes.md` — Why can concurrent writers never duplicate versions or corrupt counts?
- `references/migrations-three-case-bootstrap.md` — How does a fresh, pre-alembic, or stale database converge to head safely?
- `references/autocommit-diff-commit-message.md` — How does the library write commit messages for prompt changes using itself?
- `references/evaluation-run-pipeline.md` — How do dataset expansion, API batching, and labeler invocation interleave with persistence?
- `references/evaluation-persistence-labeler-ids.md` — How do evaluation versions, runs, and labels get deterministic identity?
- `references/verbosity-renderer-and-phone-home.md` — What does the verbose logging plane actually do at call time — including the outbound network request a porter must know about?
- `references/studio-db-watcher.md` — How does the UI learn about new invocations without polling the database contents?
