...
## Mode selection (from repository state + user intent)
...
- `AGENTS.md` from `templates/agents.md` — local Verify commands (actually run them), repository facts, non-obvious invariants, traps, enforced conventions only.
...
Mode A: every command in the summary was actually executed (exit code cited). Mode B/C: each written file exists, is reconciled against prior content, and contains only verified or explicitly marked `[NEEDS CLARIFICATION]` claims. Mode D: diff shows only the stale-set that changed.
