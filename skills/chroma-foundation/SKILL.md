...
- `references/hnsw-label-monotonicity.md` — How hnswlib integer labels stay stable across updates, resizes, and deletes.
- `references/persistent-hnsw-layered-batch.md` — Why un-indexed writes must be served by a brute-force twin of the HNSW index.
- `references/batch-operation-accounting.md` — Netting out add/update/delete counts when one ID is mutated repeatedly inside a batch.
- `references/brute-force-index-free-list-tombstones.md` — Freelist + NaN-tombstone slot management for the bounded batch matrix.
- `references/hnsw-params-validation-gate.md` — Where `hnsw:*` knobs are validated (propagation) vs defaulted (construction).
- `references/where-filter-subquery-grammar.md` — Compiling `$eq/$ne/$in` filters into per-key row subqueries with IN/NOT IN.
- `references/metadata-write-path.md` — The RETURNING-id insert ladder, typed value columns, and FTS upsert dance.
- `references/sqlite-wal-returning-batch.md` — Multi-row WAL inserts whose returned seq_ids arrive unordered.
- `references/wal-purge-coalesce-guard.md` — The min-across-segments purge rule that prevents deleting unreplayed log records.
- `references/tx-nested-commit-gate.md` — Reentrant transactions that commit only at stack depth zero.
- `references/maxscore-windowed-wand.md` — Windowed WAND with essential/non-essential terms over blockfile postings.
- `references/topk-threshold-monotone.md` — A k-bound heap whose push() returns the live pruning threshold.
- `references/sparse-directory-exact-count.md` — Overflow-safe document frequencies that refuse to fabricate saturated counts.
- `references/two-pass-posting-suffix-rewrite.md` — Forked-blockfile commits that rewrite only blocks at or after the first delta.
- `references/hnsw-provider-cache-version-gate.md` — Collection-keyed index cache with identity checks and double-checked loading.
- `references/spann-version-tombstone-gc.md` — Version-map tombstones (version=0) and threshold-driven head GC.
- `references/collection-name-s3-rules.md` — The five lexical rules every collection name must pass.
- `references/dimension-lazy-pinning.md` — Writes pin collection dimensionality; queries only validate.
- `references/version-mismatch-retry-ladder.md` — Whole-read retries on VersionMismatchError, not transient faults.
- `references/delete-by-filter-limit-slice.md` — Delete guard clauses: no unconditional wipes, limit needs a filter.
- `references/segment-manager-file-handle-lru.md` — rlimit-derived handle budgets with close-on-evict for mmap-heavy indexes.
- `references/vector-encode-int32-decode-bug.md` — A latent upstream codec asymmetry a porter must fix, not copy.
- `references/segment-api-plan-assembly.md` — Request → Scan/Filter/KNN/Limit/Projection plans; writes go WAL-only.
