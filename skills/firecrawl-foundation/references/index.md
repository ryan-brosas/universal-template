...
- `./meta-object-lifecycle.md` — one Meta object carries id/options/flags/prefetches; tri-state prefetches (`undefined`=not-yet, `null`=came-back-empty) gate antibot fallbacks.
- `./engine-fallback-list.md` — priority-threshold filter over a 16-flag × 15-engine matrix; quality-sign partition keeps specialty engines only when no general engine survives.
- `./engine-waterfall-race.md` — engines start eagerly and race with a waterfall timer; WrappedEngineError carries the engine id; EngineUnsuccessful is silent-by-design.
- `./retry-tracker-feature-ladder.md` — AddFeature/RemoveFeature/PDFAntibot errors are control signals mutating flags under a two-level (global + per-reason) budget.
- `./abort-manager-tiers.md` — AbortManager fans typed abort instances into one signal; tier discriminates fatal from swallowable; child managers dispose per attempt.
### Document enrichment plane (`scraper/scrapeURL/transformers/`)
- `./transformer-stack-order.md` — 20-stage sequential stack where markdown precedes redactPII/LLM extract; out-of-order guards throw; final stage strips unrequested fields.
### Usage ledger & billing plane (`apps/api/src/lib/`)
- `./cost-tracking-ledger.md` — append-only call ledger with record-time stacks, NaN-proof totals, throw-after-record limit; serialized shape is the cross-process contract.
- `./credit-pricing-ladder.md` — base 1 + additive surcharges vs replace-style rules (json=5, deterministicJson=10|3, fire-1=cost×1800); failures bill scans but no base.
### Crawl state & delivery plane (`lib/crawl-redis.ts`, `services/webhook/delivery.ts`)
- `./url-permutation-dedup.md` — www/scheme/index-file permutations collapsed to ONE canonical key via idempotence invariant; SADD arity admits.
- `./crawl-finish-zdr-bookkeeping.md` — completion = jobs_done==jobs AND kickoff AND sitemap done; HSETNX threat records double as billing dedup; unconditional ZDR wipes at finish.
- `./webhook-delivery.md` — HMAC-over-exact-bytes signature, private-IP SSRF gate, RabbitMQ-or-inline split, buffered log insertion that degrades gracefully.
### Worker & error planes (`services/queue-worker.ts`, `lib/error.ts`)
- `./worker-lifecycle.md` — RAM/CPU admission-controlled pull loops, tracked in-flight drain on shutdown, TransportableErrors exempt from crash reporting.
- `./transportable-error-taxonomy.md` — internal control-flow errors never cross processes; TransportableError serialize/deserialize pairs with string-code wire contract.
...
- [`abort-manager-tiers.md`](./abort-manager-tiers.md)
- [`cost-tracking-ledger.md`](./cost-tracking-ledger.md)
- [`crawl-finish-zdr-bookkeeping.md`](./crawl-finish-zdr-bookkeeping.md)
- [`credit-pricing-ladder.md`](./credit-pricing-ladder.md)
- [`engine-fallback-list.md`](./engine-fallback-list.md)
- [`engine-waterfall-race.md`](./engine-waterfall-race.md)
- [`meta-object-lifecycle.md`](./meta-object-lifecycle.md)
- [`retry-tracker-feature-ladder.md`](./retry-tracker-feature-ladder.md)
- [`transformer-stack-order.md`](./transformer-stack-order.md)
- [`transportable-error-taxonomy.md`](./transportable-error-taxonomy.md)
- [`url-permutation-dedup.md`](./url-permutation-dedup.md)
