...
- `references/query-runner-comparison-shapes.md` — run pipeline; two comparison result shapes; goal-index decode.
- `references/optimizer-step-pipeline.md` — ordered business-rule rewrites; time-granularity resolution; split-with-renames.
- `references/table-decider-partitioning.md` — three-input partition closure deciding one-table vs join; smearing gate.
- `references/sql-builder-subquery-joins.md` — subquery recombination, windowed total_rows, dimension provenance side.
- `references/session-metric-sign-arithmetic.md` — CollapsingMergeTree-safe ratio metrics with underflow clamps.
- `references/timeslots-session-smearing.md` — clamped timeSlots expansion; 15-min sub-slots for fractional-hour zones.
- `references/goal-array-index-join.md` — single-scan arrayIntersect goal matching over parallel arrays.
- `references/where-builder-filter-grammar.md` — recursive filter compiler; table-scoped no-ops; has_done subqueries; PK lower bound.
- `references/session-cache-store.md` — 30-min latch, sign-pair updates, phash2-sharded balancer serialization.
- `references/write-buffer-rowbinary.md` — byte-counted batching with compile-time-frozen INSERT statements.
- `references/ingestion-event-pipeline.md` — ordered drop-gates before enrichment; 200ms UA timeout; salted SipHash identity.
- `references/persistor-backend-rollout.md` — percent-gated backend selection keyed on user_id.
- `references/cache-adapter-warmer.md` — exit-catching partitioned ConCache adapter; all/updated-recently refresh pair.
- `references/time-labels-comparison-ranges.md` — label-axis generation; contiguous previous-period shifts; leap-year YoY.
- `references/imported-merge-plane.md` — dimension→import-table routing; lossy top-N×100 join; weighted metric recombination.
- `references/sampling-budget-ladder.md` — 10M-event scan budget → SAMPLE fraction with honesty multipliers (EE).
- `references/session-transfer-tinysock.md` — version-fingerprinted session-cache handoff over Unix sockets between deploys.
- `references/tracker-engagement-protocol.md` — visibility-banked engaged time; monotonic scroll-depth send gate.
