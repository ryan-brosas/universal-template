...
description: "Use when validating finished changes before pushing a PR: coordinate project gates, semantic and graph review, AI-slop rejection, and a revision-bound evidence record. push-pr owns remote delivery."
invocation: internal
...
local readiness verdict and evidence for `../push-pr/SKILL.md`; it does not push,
...
`../skill-catalog/SKILL.md`: tests, security, frontend, native runtime, migration,
...
- **Behavior and gates:** use `../agent-code-quality-gate/SKILL.md`. Run project
...
- **IDE semantics:** for nontrivial symbol/API changes, discover available IDE
  tools and use `../mcp-steroid/SKILL.md`. Confirm the correct project and ready
...
- **Graph review:** use the active working-set graph when available. For an
  existing Codebase Memory index, load `../codebase-memory/SKILL.md`, confirm
...
- **AI-slop rejection:** run the project's objective artifact gate when present.
  ...
  `../code-cleanup/SKILL.md` for justified simplification, preserving behavior.
...
lessons only where they belong, using `../leverage-capture/SKILL.md` when useful.
