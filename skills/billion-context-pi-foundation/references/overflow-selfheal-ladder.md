...
### Decisive source
```ts
// overflow-selfheal.ts:117-137 + index.ts:416-417 / :154-158 / :207-211 — the two halves
...
**Probe:** `cd /mnt/hdd/utopia/inspo/billion-context-pi && npx tsx --test tests/overflow-selfheal.test.ts` — 21/21 GREEN at pin 6a88c556 (executed pass 12; window parse per provider phrasing, marker superset vs Bedrock-throttle non-match, headroom reservation matrix).
**Retrieve:**
```ts
await mcp.codebase_memory.search_graph({ project: "mnt-hdd-utopia-inspo-billion-context-pi", query: "OverflowEpisode inspectOverflowMessage reserveOutputHeadroom armed", limit: 10 });
```
