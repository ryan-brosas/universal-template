...
**Probe:** `cd /mnt/hdd/utopia/inspo/coding-agents/billion-context-pi && node_modules/.bin/tsx --test tests/footer-status.test.ts` → 6 pass / 0 fail executed GREEN at pin (dedupe-once, clear-on-zero ×2, dispose detach, boundary ladder, churn guard).
**Retrieve:**
```ts
