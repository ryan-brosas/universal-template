...
**Path/Symbol:** `src/tag-tokens.ts`:3-15 whole file (14 lines: `formatTokens` :3-7, `stableTagTokens` :9-11, `rewriteTagTokens` :13-15); consumed at `src/messages.ts`:278, :313, :315 (both the kernel-body rebuild path AND the stable-append path); producer side documented in `src/system-prompt.ts`:9.
