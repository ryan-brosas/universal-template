...
**Probe:** `cd /mnt/hdd/utopia/inspo/billion-context-pi && node_modules/.bin/tsx --test tests/tag-tokens.test.ts` → 7 pass / 0 fail executed GREEN at pin (covers format ladder boundaries incl. 999→"999"/10000→"10K", CJK counting, tool-result type preservation, truncation re-tagging).
**Retrieve:**
```ts
