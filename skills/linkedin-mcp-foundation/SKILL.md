...
- `references/cli-startup-ladder.md` — claim-before-touch startup ordering; protocol-clean stdout; stored-config transport write-back; fail-soft shared-owner election; strict host/origin protection.
- `references/background-browser-setup.md` — start instantly, provision in the background, gate every tool call on disk-truth readiness.
- `references/trace-capture-retention.md` — on-error trace capture: ephemeral by default, three-signal retention re-checked at teardown.
- `references/owner-call-liveness-tracker.md` — owner-side liveness tracker: cancellable-vs-running split, stall-aware expiry, shutdown-vs-abandonment triage.
