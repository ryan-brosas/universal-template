...
- `references/log-routing-to-stdout.md` — app code writes unbuffered to stdout/stderr ONLY; log destinations (file/DB/SaaS) belong to the execution environment's log-driver, so changing them never requires a deploy.
