...
- `references/floating-all-day-options.md` — how are date-only options encoded so they never shift across timezones?
- `references/booking-scheduled-event-times.md` — when is a booked option an all-day UTC-midnight span vs a timed instant?
- `references/poll-lifecycle-close-reopen.md` — when does a poll close itself, reopen, and why must manual closes stick?
- `references/inactivity-retention-ladder.md` — how do polls get deleted without ever losing active ones?
- `references/soft-delete-invisibility.md` — what does "deleted" mean for polls, participants, and votes?
- `references/guest-edit-token-actors.md` — how do anonymous participants keep editing their own responses?
- `references/participant-visibility-ladder.md` — what does hideParticipants actually hide, per viewer?
- `references/score-formula-top-choice.md` — how are winning options ranked and what makes a "top choice"?
- `references/booking-transaction-invites.md` — how does a poll option become a scheduled event with deduped invites?
- `references/upcoming-past-predicate.md` — how do all-day and timed events split "upcoming" without drift?
- `references/email-datetime-rendering.md` — which timezone does each recipient's event email use?
- `references/calendar-date-encoding.md` — how do you get a zone's YYYY-MM-DD without trusting locale patterns?
- `references/client-option-display-zones.md` — how does one renderer show tz-pinned, floating, and all-day options correctly?
- `references/content-moderation-funnel.md` — how is user-generated text gated from regex to AI to verdict?
- `references/procedure-access-ladder.md` — which tRPC procedure guards each mutation tier, and where does self-hosting bypass paywalls?
- `references/membership-gating.md` — when is space membership "effective" and what do guests get to keep?
- `references/housekeeping-cron-routes.md` — how are cron jobs exposed and bounded safely?
- `references/rsvp-atomic-registration.md` — how is duplicate registration rejected without a check-then-insert race?
- `references/deferred-email-dispatch.md` — why does every side-effectful mutation send mail through after()?
- `references/vote-write-path.md` — how are vote updates made safe against stale option ids and activity detection?
- `references/notification-preference-jsonb-merge.md` — how do concurrent preference toggles avoid clobbering each other's key?
- `references/activity-event-prefs-codec.md` — what does a reader get when stored notification prefs are corrupt or legacy-shaped?
- `references/notification-recipient-gate-ladder.md` — in what order are creator-notification suppressions checked, and who is silently skipped?
- `references/poll-mute-owner-scoped-toggle.md` — how is ownership enforced on a boolean flip without a check-then-update race?
- `references/safe-action-procedure-ladder.md` — where does the second mutation framework live and why does session revocation differ from tRPC?
- `references/safe-action-error-code-projection.md` — how does the client turn string server codes into localized feedback safely?
- `references/footer-href-url-parse-gate.md` — how do you validate an admin-supplied href so `javascript:` can never survive into an anchor?
- `references/footer-links-drop-on-read-roundtrip.md` — what happens when stored footer JSONB was edited outside the app?
- `references/singleton-tag-cache-store.md` — how is a one-row global config cached, invalidated, and defaulted when the row is missing?
