...
| One selected inspiration repository | project-local `<project>/reference/<repo>/` | Fovea on that root, then source + tests | see `reference-driven-development`; consult when a relevant checkout already exists or the user names one |
| Rendered visual or runtime evidence from a live website (DOM, CSS, screenshots, behavior) | **web-reference** (bounded capture) or existing `reference/web/<site>/` | inspect the capture like source | capture is evidence; the current project stays the acceptance authority |
