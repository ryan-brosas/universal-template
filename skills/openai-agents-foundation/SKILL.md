...
- **Tracing usage projection** — `usage-span-projection`: three-granularity span usage shapes with snapshot deltas and zero-guards.
- **Resume reconciliation** — `resume-approval-reconciliation`: approval re-validation and stale-run rebinding for interrupted-turn resume.
- **AnyLLM adapter plane** — `anyllm-provider-plane`, `anyllm-transport-sanitize-close`: capability-gated API/provider cache/normalization plus transport-kwargs, replay sanitization, and close discipline.
