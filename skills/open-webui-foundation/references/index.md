...
- `./config-store.md` — How does app config persist to DB while env-seeded defaults never override persisted values?
- `./event-bus.md` — How do you fan one validated domain event to pluggable sinks without leaking secrets or cross-sink failures?
- `./socket-events.md` — How do chat events reach every device of a user across instances, and how does the backend safely call back into one browser session?
- `./hybrid-retrieval.md` — How do BM25 and vector search fuse under a reranker, degrading gracefully when the vector store lacks native hybrid?
- `./plugin-loader.md` — How does DB-stored user Python become an importable module without corrupting `sys.modules`?
- `./filter-pipeline.md` — In what order do filter plugins run, and how are their params and valves bound?
- `./agentic-tool-loop.md` — How do streamed tool calls become a bounded agent loop whose OR-style display state stays aligned across continuation rounds?
- `./stream-tag-scanner.md` — How do you parse `<think>`/`<code>`-style tags incrementally over a token stream without rescanning or re-detecting your own start tag?
- `./delta-coalescing.md` — How do you throttle high-frequency stream deltas into bounded socket writes without reordering or losing a type?
- `./cancellation-ladder.md` — What must happen on client cancel mid-stream so nothing leaks upstream or corrupts persisted state?
- `./builtin-authz-gates.md` — How do you guarantee a legacy second execution path enforces identical authorization to the primary path?
- `./access-grants-acl.md` — How do you replace a per-model JSON ACL column with a relational grant table without changing semantics?
- `./shared-session-pool.md` — How do you share one outbound aiohttp ClientSession across a FastAPI monolith without leaking connections or closing the pool?
- `./provider-timeout-ladder.md` — What should "unset" mean for an outbound LLM timeout env var, and how do streaming calls differ from unary ones?
- `./ollama-send-request-ladder.md` — When a proxied provider call fails, what reaches the client vs telemetry, and who releases the connection?
- `./provider-backend-resolution.md` — How do you route one logical model across N backends with per-backend keys/config without letting callers escape the allow-list?
- `./openai-completion-proxy.md` — How do you normalize one OpenAI-ish payload across vanilla, Azure (v1 + legacy deployment), and Responses dialects in a single entry point?
- `./proxy-auth-header-ladder.md` — How do you send credentials upstream (bearer/session/OAuth/Entra) without leaking your own transport headers back to clients?
- `./ollama-model-aggregation.md` — How do you fan out a model census across N backends so failures and disabled entries never corrupt per-index post-processing?
- `./upload-process-status-pipeline.md` — How does a browser upload become processed content with live SSE progress, and why must client-supplied linkage metadata be re-gated server-side?
- `./process-file-ingest-ladder.md` — How does one endpoint turn a stored file into embedded chunks without holding DB connections across slow embedding calls?
- `./save-docs-vector-db-kernel.md` — What makes landing chunks into a vector collection safe: hash dedup scoped by file, splitter validation, embedding_config stamping, overwrite/add lifecycle?
- `./knowledge-file-binding-lifecycle.md` — In what order do embed, bind-row, and destructive cleanup run when attaching, refreshing, or removing a file from a shared collection?
- `./kb-metadata-embedding-maintenance.md` — How do you upsert fail-soft self-description embeddings into one fixed catalog collection without exhausting the pool during admin reindex?
- `./transitive-file-access-resolver.md` — How do you resolve read/write on a file that arrives transitively via collections/channels/chats/models without read access laundering into write?
- `./auth-token-transport-ladder.md` — How do you authenticate one FastAPI dependency across header/cookie/custom-header transports without holding a DB session?
- `./jwt-issue-revoke-algebra.md` — How do you revoke stateless JWTs both per-token (sign-out) and per-user (IdP logout) with TTL-bounded Redis state?
- `./api-key-config-gates.md` — How do you gate API keys by runtime config, user permissions, AND endpoint allow-lists without trusting proxy-visible paths?
- `./password-hash-prefix-dispatch.md` — How do you migrate password hashing algorithms without breaking stored hashes?
- `./mcp-client-lifecycle.md` — How do you host MCP client sessions whose SDK forbids shielded/cross-task teardown?
...
- [`access-grants-acl.md`](./access-grants-acl.md)
- [`agentic-tool-loop.md`](./agentic-tool-loop.md)
- [`api-key-config-gates.md`](./api-key-config-gates.md)
- [`auth-token-transport-ladder.md`](./auth-token-transport-ladder.md)
- [`builtin-authz-gates.md`](./builtin-authz-gates.md)
- [`cancellation-ladder.md`](./cancellation-ladder.md)
- [`config-store.md`](./config-store.md)
- [`delta-coalescing.md`](./delta-coalescing.md)
- [`event-bus.md`](./event-bus.md)
- [`filter-pipeline.md`](./filter-pipeline.md)
- [`hybrid-retrieval.md`](./hybrid-retrieval.md)
- [`jwt-issue-revoke-algebra.md`](./jwt-issue-revoke-algebra.md)
- [`kb-metadata-embedding-maintenance.md`](./kb-metadata-embedding-maintenance.md)
- [`knowledge-file-binding-lifecycle.md`](./knowledge-file-binding-lifecycle.md)
- [`mcp-client-lifecycle.md`](./mcp-client-lifecycle.md)
- [`ollama-model-aggregation.md`](./ollama-model-aggregation.md)
- [`ollama-send-request-ladder.md`](./ollama-send-request-ladder.md)
- [`openai-completion-proxy.md`](./openai-completion-proxy.md)
- [`password-hash-prefix-dispatch.md`](./password-hash-prefix-dispatch.md)
- [`plugin-loader.md`](./plugin-loader.md)
- [`process-file-ingest-ladder.md`](./process-file-ingest-ladder.md)
- [`provider-backend-resolution.md`](./provider-backend-resolution.md)
- [`provider-timeout-ladder.md`](./provider-timeout-ladder.md)
- [`proxy-auth-header-ladder.md`](./proxy-auth-header-ladder.md)
- [`save-docs-vector-db-kernel.md`](./save-docs-vector-db-kernel.md)
- [`shared-session-pool.md`](./shared-session-pool.md)
- [`socket-events.md`](./socket-events.md)
- [`stream-tag-scanner.md`](./stream-tag-scanner.md)
- [`transitive-file-access-resolver.md`](./transitive-file-access-resolver.md)
- [`upload-process-status-pipeline.md`](./upload-process-status-pipeline.md)
