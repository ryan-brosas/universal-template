...
- `./fusion-weight-per-backend.md` — which weights reach each store's fusion expression and why ES is pinned to "0.001,1".
- `./empty-retrieval-retry-ladder.md` — the single ordered retry that relaxes lexical matching while tightening vector similarity.
- `./es-vectorless-rerank.md` — second-pass ids-filtered KNN scoring so chunk vectors never leave the engine.
- `./rerank-window-pagination.md` — candidate window MUST be a multiple of page_size or deep pages silently drop.
- `./citation-threshold-decay.md` — decaying absolute threshold × relative max bar attaching ≤4 citations per sentence.
- `./query-boost-grammar.md` — two-branch (EN/ZH) boosted query grammar with lexer-safe escaping.
- `./stale-chunk-prune-safety-net.md` — post-search existence re-check filtering chunks of deleted docs before rerank.
- `./tag-feature-pagerank-boost.md` — add-after-blend tag cosine ×10 + PageRank; GaussDB double-add trap.
- `./task-mode-dispatch.md` — one consumer, five task types, drain-then-consume, exactly-once ack.
- `./progress-cancel-latch.md` — persist-then-raise cancellation checkpoints and partial-chunk cleanup.
- `./thread-pool-exec-contextvars.md` — why every blocking call crosses threads via ctx.run on a per-call executor.
- `./toc-parent-expansion.md` — mom_id child→parent synthesis and LLM-ranked TOC additive boosts.
- `./graphrag-resume-tiers.md` — doc-store subgraphs + content-addressed Redis checkpoints + KB-scoped phase markers; merge clears markers.
- `./graphrag-retry-ladder.md` — coro-factory retries with per-phase timeouts, capped backoff, cancel passthrough, clamp-to-default config.
- `./graphrag-gleaning-extraction.md` — CONTINUE/LOOP gleaning with forced single-token probe, cached chat wrapper, error-budget breaker.
- `./graphrag-node-merge-concurrency.md` — snapshot-before-mutate fold of duplicate nodes; GraphChange bookkeeping; serialized merges.
- `./entity-resolution-pipeline.md` — similarity-gated candidates → batched checkpointed LLM verdicts → component merge.
- `./leiden-stable-communities.md` — canonical input ordering + fixed seed = reproducible partitions.
- `./community-report-recovery.md` — membership-keyed replay, brace-repair JSON ladder, typed schema gate, drop-not-fail.
- `./insert-then-prune-replacement.md` — deterministic ids + insert-before-prune keeps the old complete set on crash.
- `./kg-query-three-channel-fusion.md` — question → entity/relation channels → sim×pagerank ranking → one token-budgeted pseudo-chunk inserted first.
- `./kg-query-rewrite-json-repair.md` — type-pool-hinted LLM keyword extraction with two-tier JSON repair; failure downgrades to the raw question.
- `./kg-community-report-go-twin.md` — community reports by entity membership + weight order, mirrored byte-for-byte in the tested Go service.
- `./ner-extractor-dispatch.md` — config dispatch fail-to-light; NER path skips chunk batching; spaCy keywords + co-occurrence edges, zero LLM extraction.
- `./set-graph-build-before-delete.md` — build all chunks (graph/subgraphs/embeddings) before any delete; batch embed pre-warm; bounded edge-delete retries.
- `./nhop-weight-cross-layer-contract.md` — stored n-hop paths+weights feed distance-decayed edge credit; absent/empty/malformed degrade to [].
...
- [`citation-threshold-decay.md`](./citation-threshold-decay.md)
- [`community-report-recovery.md`](./community-report-recovery.md)
- [`empty-retrieval-retry-ladder.md`](./empty-retrieval-retry-ladder.md)
- [`entity-resolution-pipeline.md`](./entity-resolution-pipeline.md)
- [`es-vectorless-rerank.md`](./es-vectorless-rerank.md)
- [`fusion-weight-per-backend.md`](./fusion-weight-per-backend.md)
- [`graphrag-gleaning-extraction.md`](./graphrag-gleaning-extraction.md)
- [`graphrag-node-merge-concurrency.md`](./graphrag-node-merge-concurrency.md)
- [`graphrag-resume-tiers.md`](./graphrag-resume-tiers.md)
- [`graphrag-retry-ladder.md`](./graphrag-retry-ladder.md)
- [`insert-then-prune-replacement.md`](./insert-then-prune-replacement.md)
- [`kg-community-report-go-twin.md`](./kg-community-report-go-twin.md)
- [`kg-query-rewrite-json-repair.md`](./kg-query-rewrite-json-repair.md)
- [`kg-query-three-channel-fusion.md`](./kg-query-three-channel-fusion.md)
- [`leiden-stable-communities.md`](./leiden-stable-communities.md)
- [`ner-extractor-dispatch.md`](./ner-extractor-dispatch.md)
- [`nhop-weight-cross-layer-contract.md`](./nhop-weight-cross-layer-contract.md)
- [`progress-cancel-latch.md`](./progress-cancel-latch.md)
- [`query-boost-grammar.md`](./query-boost-grammar.md)
- [`rerank-window-pagination.md`](./rerank-window-pagination.md)
- [`set-graph-build-before-delete.md`](./set-graph-build-before-delete.md)
- [`stale-chunk-prune-safety-net.md`](./stale-chunk-prune-safety-net.md)
- [`tag-feature-pagerank-boost.md`](./tag-feature-pagerank-boost.md)
- [`task-mode-dispatch.md`](./task-mode-dispatch.md)
- [`thread-pool-exec-contextvars.md`](./thread-pool-exec-contextvars.md)
- [`toc-parent-expansion.md`](./toc-parent-expansion.md)
