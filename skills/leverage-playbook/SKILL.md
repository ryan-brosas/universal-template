...
4. **Capture before compaction memory is lost.** When the session has compacted 3–4 times, run the capture pass (exact prompt in `references/capture-ritual.md`) and fold the *recurring* procedures and edge cases into skill files — one-off details stay in code.
...
- When a capture pass ran, it produced a promoted skill or a reusable change to an existing skill — evidence: `ls ~/.agents/skills` diff, or a `references/` capsule added. A session with nothing recurring legitimately captures nothing.
...
- `references/session-principles.md` — discovery/small-model + "code is ground truth" (Tom, 8/21/26)
