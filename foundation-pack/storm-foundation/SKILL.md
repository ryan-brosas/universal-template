...
- `references/dual-layer-lm-cache.md` — LRU-over-litellm-disk cache keyed on the serialized request JSON; env-default cost-map and drop_params wiring.
- `references/text-completion-provider-degradation.md` — serving chat configs through completions endpoints via provider-prefix rewrite and messages flattening.
- `references/usage-ledger-reset-protocol.md` — `_lm`/`rm` naming convention + get-and-reset drain so every stage's token/query cost lands once.
### Citation grounding plane (`utils.py`, dataclasses)
- `references/two-phase-citation-renumbering.md` — placeholder-moat remapping of `[old]→[new]` without collisions; the latent no-op twins to avoid.
- `references/url-unified-reference-merge.md` — per-section local numbers folded into one URL-keyed bibliography, then reordered by first appearance.
- `references/grounded-answer-citation-sanitizer.md` — group-split → dedupe → last-sentence truncation → over-range strip ladder that keeps `[n]` honest.
- `references/information-identity-citation-ledger.md` — md5 identity over (url, sorted snippets, meta) and hash→uuid mint-once under lock.
### Outline & article assembly plane
- `references/outline-grammar-coercion.md` — topic reset, bullet→heading promotion, twelve tail-section amputations before tree parsing.
- `references/article-tree-markdown-parser.md` — `(node, level)` stack ingestion of flat markdown into nested section dicts.
- `references/section-skip-heuristics.md` — skip intro/conclusion/summary sections; write the lead LAST from the finished draft.
- `references/word-budget-line-preserving-truncation.md` — word-cap context clamping that never emits a partial line.
### Retrieval plane (`rm.py`, `storm_dataclass.py`)
- `references/retriever-adapter-contract.md` — uniform four-key dict contract, usage counters, skip-on-error loops, and the exclude_urls dummy traps.
- `references/snippet-retrieval-dedup.md` — flat parallel arrays + ascending-argsort top-k slice + clone-before-narrow retrieval.
- `references/parallel-embedding-order-restore.md` — concurrent embedding fan-out with sort-back-to-input-order.
### Orchestration plane (`engine.py`, curation)
- `references/runner-resume-from-artifacts.md` — four-stage pipeline where every stage loads its predecessor's artifacts on skip.
- `references/persona-conversation-simulation.md` — sentinel termination, sliding-window dialogue context, always-first default persona, ground-truth exclusion.
- `references/wiki-toc-persona-induction.md` — fail-soft persona induction from related Wikipedia TOCs; stack-derived heading indentation; numbered-line output grammar.
- `references/outline-draft-then-refine.md` — topic-only draft outline + unconditional conversation-guided refinement; dialogue denoising; old-outline-as-OutputField prompt trick.
- `references/knowledge-base-mindmap-lifecycle.md` — clean→expand→clean→relabel reorganization of the Co-STORM mind map.
- `references/hierarchical-event-logging.md` — stage buckets + event stacks + drain-on-close usage attribution.
### Co-STORM discourse plane (`collaborative_storm/`)
- `references/costorm-turn-policy-ladder.md` — who-speaks-next priority chain with one-shot moderator latch, dry-run-safe rotation, and answer-domination trigger.
- `references/expert-action-grammar-parse.md` — four-label contribution grammar with bracketed parse fallback, fail-loud Undefined, and the load-bearing `resposne` typo pair.
- `references/moderator-novelty-snippet-scoring.md` — identity-hash unused-snippet exclusion + claim-gated dissimilarity scoring + round-robin cross-turn merge.
- `references/grounded-ask-module-contract.md` — decompose→retrieve→budget-format→refuse-by-default→sanitize→rebind lifecycle of one cited conversational answer.
- `references/warmstart-mini-storm.md` — background research → perspectives → outline-first KB seed → synthesized catch-up transcript, under lock and swallow-isolation rules.
- `references/mindmap-intent-placement.md` — embedding-ranked candidate choice then layer-by-layer LM navigation; frozen-snapshot parallel vs mutating sequential inserts.
### Guardrails plane
- `references/input-appropriateness-gate.md` — length → charset → small-model verdict gate that fails closed.
