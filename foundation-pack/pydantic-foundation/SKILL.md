...
- `references/annotated-metadata-collection-partition.md` — Annotated metadata split into known-constraint dict vs opaque remainder; GroupedMetadata flattening; FieldInfo copy-before-clear; None-drop tail.
- `references/constraint-application-ladder.md` — four-rung constraint→schema ladder (direct key-set / str wrap-chain / after-validator+js-stamp / RuntimeError); None return means unknown → original schema.
- `references/single-annotation-schema-shape-branches.md` — shape dispatch before constraints: FieldInfo recursion, nullable pass-through, MISSING-sentinel union filtering, ref cloning under `_{repr(metadata)}` with def dedup.
- `references/config-wrapper-precedence-ladder.md` — bases→namespace→kwargs config merge with pop-on-consume, config-both gate, deferred both-false alias check, core-config key renames.
- `references/ns-resolver-frame-stack.md` — push/pop type frame stack for forward-ref evaluation; ascending-priority locals ending in the type's own name; dual cache invalidation.
