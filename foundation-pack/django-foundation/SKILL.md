...
- `references/middleware-mode-adaptive-chain.md` — where sync↔async adapters install in the middleware onion and what decides each hop's mode.
- `references/hook-side-tables.md` — process_view/process_template_response/process_exception execution order outside the onion.
- `references/exception-conversion-per-hop.md` — per-hop exception→response conversion and the 403/400/404/500 classification ladder.
- `references/asgi-disconnect-race.md` — TaskGroup race between response delivery and client disconnect with sentinel-exception success signalling.
- `references/asgi-header-funnel.md` — scope-bytes→META decoding with underscore-spoofing rejection and duplicate-header join rules.
- `references/asgi-body-spool.md` — SpooledTemporaryFile body buffering with per-chunk roll detection and per-request thread sensitivity.
- `references/asgi-response-send.md` — http.response.start/body framing, cookie headerization, and aclosing(aiter()) stream consumption.
- `references/wsgi-environ-decoding.md` — latin1 environ byte-recovery and mod_rewrite SCRIPT_NAME reconstruction ladder.
- `references/wsgi-file-wrapper-close.md` — close-delegation patch so platform-managed file bodies still release resources.
- `references/urlresolvers-resolve-tried.md` — depth-first first-wins resolution, kwargs merge order, and the tried-path 404 ledger.
- `references/routepattern-fast-path.md` — path() grammar compile, converter two-way contract, and fall-through on to_python ValueError.
- `references/reverse-populate-namespaces.md` — reversed-order reverse-dict population, re-entrancy latch, and namespace walking in reverse().
- `references/body-single-consumption.md` — _read_started latch plus the declared-then-actual size gate for DATA_UPLOAD limits.
- `references/host-validation-ladder.md` — grammar-first ALLOWED_HOSTS validation against X-Forwarded-Host spoofing.
- `references/response-header-cookie-lifecycle.md` — set-time charset/newline enforcement and prefix-aware delete_cookie.
- `references/streaming-dual-mode.md` — iterator-kind latching with buffered cross-mode sync↔async consumption bridges.
