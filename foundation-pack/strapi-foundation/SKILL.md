...
- `references/status-sort-expression.md` — how a virtual `status` sort key becomes a parameterized CASE rank that survives SELECT DISTINCT.
- `references/deep-sort-wrap.md` — how row-number partition wrapping dedupes parents when sorting by joined-relation columns while keeping pagination correct.
- `references/metadata-load-models.md` — how model configs become DB metadata in three passes without double-processing identifier names.
- `references/morph-one-cascade-delete.md` — how exclusive morphOne links are re-derived from incoming rows when a morphToMany collection is rewritten.
...
- `references/transfer-engine-stage-pipeline.md` — how a fixed-order multi-stage streaming transfer gets per-stage skip, mid-stage cancellation, and destination-only rollback.
- `references/progress-reporting-plane.md` — how per-stage count/bytes/ETA are measured with stream-replacing trackers that never consume or buffer the payload.
- `references/remote-push-flow-state-machine.md` — how an untrusted WS step protocol is kept ordered and single-stage by an index-difference flow plus lock/unlock.
- `references/ws-uuid-replay-dispatch.md` — how request/response over a fire-and-forget socket stays idempotent when the client resends the same uuid.
- `references/asset-chunk-wire-negotiation.md` — how a binary-over-JSON wire format evolves (base64 vs legacy Buffer.toJSON) via init-echo negotiation and a union decode ladder.
