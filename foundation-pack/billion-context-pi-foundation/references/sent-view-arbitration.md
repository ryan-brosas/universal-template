...
**Source:** billion-context-pi (MIT) `master@6a88c5565355baebccfaf27398a6008fe08619ed`; Codebase Memory project `mnt-hdd-utopia-inspo-billion-context-pi`. **Question:** Your host reports a context-usage number — should nudge/emergency decisions trust it? What is the contract for computing the decision-scale token count?

## The host's real usage has two regimes; only one is decision-safe
**Path/Symbol:** `src/index.ts`:179-214 (`realUsage` capture, sent-view assembly, calibration, armed-emergency boost); `src/tokens.ts`: `estimateTokens` (:12-20, skips `toolName === "compress"` messages and covered ids over chars/4), `calibrateTokens` (:27-29); shared-panel twin `src/commands.ts`:115-163 (`statusReport` — same arithmetic for the human view); `src/footer-status.ts` whole (51L).
...
### Decisive source
```ts
// index.ts:179-201 — the false-emergency narrative that justifies the whole seam:
...
**Probe:** `cd /mnt/hdd/utopia/inspo/billion-context-pi && npx tsx --test tests/sent-view-arbitration.test.ts tests/footer-status.test.ts tests/tokens.test.ts` — 16/16 GREEN at pin 6a88c556 (executed pass 12).
**Retrieve:**
```ts
await mcp.codebase_memory.search_graph({ project: "mnt-hdd-utopia-inspo-billion-context-pi", query: "estimateTokens calibrateTokens getContextUsage sent view statusReport", limit: 10 });
```
