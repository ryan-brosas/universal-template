...
- `references/focus-in-bitmask-loop.md` — bitmask Focus algebra, wraparound arithmetic, and the try-focus-until-activeElement loop.
- `references/focusable-enumeration.md` — selector set, tabIndex sort, Strict-vs-Loose modes, and restore-after-unmount.
- `references/focus-trap-guards.md` — sentinel buttons, FocusLock blur redirect, InitialFocus microtask ladder, RestoreFocus from click history.
- `references/top-layer-stack-machine.md` — scoped id stacks, push-moves-to-top, isTopLayer enable-window semantics.
- `references/inert-others-refcount.md` — sibling-crawl inert marking with refcounted original-value restoration.
- `references/outside-click-pointer-pair.md` — pointerdown-target capture vs touchend target, 30px move threshold, iframe blur.
- `references/scroll-lock-store.md` — document-keyed PUSH/POP counter, willChange edge detection, lazy meta fold.
- `references/ios-touch-scroll-lock.md` — smooth-behavior override, anchor capture, overscroll-contain, portal-root preventDefault ladder.
- `references/dialog-wiring-order.md` — Dialog composition stack and the isClosing feature-shutdown window.
- `references/open-closed-context.md` — four-bit Open/Closing context and Transition-aware child demotion.
- `references/machine-react-glue.md` — abstract Machine with shallowEqual slices and useSyncExternalStoreWithSelector bridging.
- `references/listbox-dom-order-registry.md` — pendingShouldSort rAF deferral and active-index re-lookup after re-sort.
- `references/listbox-typeahead.md` — rotation-from-active search over sorted options with disabled skipping.
- `references/calculate-active-index.md` — disabled-skip navigation returning null/current on failure, never throwing.
- `references/form-fields-hoisting.md` — objectToFormEntries key paths and the hoisted-hidden-inputs pattern.
- `references/portal-nesting-registration.md` — shared `#headlessui-portal-root`, child `data-headlessui-portal` wrappers, recursive registration.
- `references/stable-collection-index.md` — render-side index allocation with effect-release for SSR-stable ordering.
- `references/root-containers-resolution.md` — which body-level subtrees count as "outside" and the main-tree probe fallback.
- `references/floating-anchor-config.md` — anchor gap/offset/padding CSS-variable→px resolution via probe element.
- `references/hidden-element-styles.md` — HiddenFeatures style/aria matrix for guards, sr-only, and display-none markers.
- `references/server-handoff-gating.md` — env handoff protocol and React-18 hydration detection suppressing DOM features.
- `references/text-value-computation.md` — typeahead text precedence chain and clone-and-strip innerText algorithm.
- `references/focus-visible-tracking.md` — global listeners maintaining data-headlessui-focus-visible on <html>.
- `references/disposables-scheduler.md` — cancellable rAF/microtask/style-effect bag powering all DOM hooks.
- `references/scrollbar-compensation.md` — before/after scrollbar measurement pair preventing layout shift.
- `references/active-element-history.md` — global focus-history ring feeding RestoreFocus snapshots.
