...
- `references/retryer-pause-resume-machine.md` — how does one retry loop serve online/offline + focus pause without losing retries?
- `references/query-fetch-dedup-revert-ladder.md` — when does a concurrent fetch reuse the in-flight promise vs silently cancel vs revert?
- `references/query-dispatch-reducer.md` — which state fields does each action own, and what does an error do to existing data?
- `references/fetch-state-optimistic-transitions.md` — how does the same fetchState helper drive real transitions and render-time optimism?
- `references/hash-key-canonical-json.md` — why is `{a:1,b:2}` and `{b:2,a:1}` the same cache entry?
- `references/partial-match-key-prefix.md` — what makes array/object keys match by prefix for invalidation?
- `references/replace-equal-depth-sharing.md` — how does referential identity survive refetches?
- `references/replace-data-structural-gate.md` — when is structural sharing skipped, customized, or crash-guarded?
- `references/notify-manager-transaction-batch.md` — why can notifications never fire synchronously mid-mutation?
- `references/removable-gc-refcount.md` — what keeps live entries alive and evicts exactly when idle?
- `references/focus-online-lazy-listener.md` — how do global event sources stay free until first subscribe?
- `references/timeout-manager-provider-audit.md` — how are thousands of timers kept swappable and auditable?
- `references/query-cache-build-remove-guard.md` — what stops removal races from deleting a newer replacement?
- `references/client-defaults-dependent-options.md` — which defaults depend on other options, and why is `_defaulted` idempotent?
- `references/observer-result-select-placeholder-memo.md` — how do select and placeholderData stay stable across renders?
- `references/tracked-prop-proxy-gate.md` — how do unused result properties stop triggering re-renders?
- `references/mutation-callback-await-ledger.md` — in what exact order do mutation callbacks run, and who owns their errors?
- `references/mutation-scope-serialization.md` — how do scoped mutations queue-and-chain instead of racing?
- `references/infinite-page-directional-behavior.md` — how does one behavior hook produce forward/backward/refetch page walks?
- `references/hydration-newer-wins-promise.md` — how does server state merge without clobbering fresher client data or in-flight promises?
- `references/streamed-query-refetch-mode.md` — how does an AsyncIterable stream land in the cache chunk-by-chunk?
- `references/use-base-query-render-contract.md` — what must a framework binding do in render vs effect order?
