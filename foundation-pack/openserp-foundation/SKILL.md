...
- `references/engine-contract.md` — what a new engine must implement and how the pipeline wraps it.
- `references/error-taxonomy.md` — sentinel errors: which degrade proxies, which are challenges, which map to which HTTP status.
- `references/rank-state.md` — ads never shift organic rank; absolute-rank interleaving and negative-rank feature convention.
- `references/result-normalization.md` — SearchResult→v2 Result enrichment, Bing click-tracking unwrap, stable IDs.
- `references/response-envelope.md` — Envelope/Finalize pagination semantics and the four output formats.
- `references/dual-parser.md` — one selector table + one classifier feeding both rod and goquery parsers.
- `references/google-adapter.md` — canonical-result selector ladder, PAA expansion polling, virtualized image grid harvesting.
- `references/serp-features.md` — declarative SerpFeatureSelector specs, SingleMatch, placeholder filtering, mirror-to-feature.
- `references/url-builders.md` — per-engine query-param dialects (gl/hl/uule, kl inversion, lr, freshness buckets).
- `references/region-uule.md` — dependency-free region resolution and Google UULE v1 encoding.
- `references/resilient-pipeline.md` — circuit gate → policy resolve → limiter → proxy mode switch → recovered invocation.
- `references/retry-budget.md` — non-retryable sentinels, jittered backoff, derived request timeout.
- `references/circuit-breaker.md` — closed/open/half-open FSM plus avg-success-latency for fastest-engine mode.
- `references/proxy-policy.md` — off/request_url/tag_pool resolution and the X-Use-Proxy / X-Proxy-URL override ladder.
- `references/proxy-registry.md` — round-robin with challenged-skip, failure-disable threshold, tag quarantine + probe recovery.
- `references/browser-lifecycle.md` — persistent connection hygiene (never bake a deadline), isolated contexts, ordered teardown.
- `references/proxy-auth-listener.md` — persistent CDP Fetch auth listener and its exclusivity with HijackRequests.
- `references/fingerprint-profiles.md` — weighted profile catalog, runtime version patching, full CDP emulation set, lane cookies.
- `references/raw-tls-client.md` — tls-client Chrome profiles picked by salted hash, header order, guarded redirect loop.
- `references/server-routing.md` — route registration with aliases, dedicated-endpoint flow, middleware order.
- `references/cache-contract.md` — cache-key composition with market fields and every BYPASS rule.
- `references/extract-auto.md` — fast/rendered escalation, only-rendered-is-longer guard, thin-article fallback to full body.
- `references/llms-txt.md` — root-only /llms-full.txt→/llms.txt probe with HTML-sniff rejection.
- `references/mega-clusters.md` — balanced/any/fastest modes, partial results on deadline, dedupe, cross-engine agreement score.
- `references/server-browser-pool.md` — per-authenticated-proxy Chrome pooling with LRU + idle eviction.
- `references/batch-extract.md` — Open WebUI-compatible bare-array batch contract with per-item error isolation.
- `references/fingerprint-check.md` — Detector SPI and the debug fingerprint endpoint's validation ladder.
- `references/yandex-partial-success-funnel.md` — mid-pagination captcha degrades to partial results; only first-page failure propagates.
- `references/yandex-hydration-re-poll.md` — selector-wait then count-to-target grace re-poll for progressively rendered rows.
- `references/yandex-image-state-json.md` — image results harvested from data-state JSON blobs with three-tier selector fallback.
- `references/yandex-lr-region-policy.md` — lr-only region targeting; rstr deliberately dropped to cut captcha frequency.
- `references/baidu-mu-canonical-url.md` — mu= canonical swap over opaque /link?url= redirects; relative links rejected as non-organic.
- `references/baidu-hashed-class-prefix.md` — prefix-match build-rotated CSS hashes on unique stems; exact pins only on generic stems.
- `references/raw-status-gate.md` — HTTP status→sentinel table (401/403→blocked, 429→rate-limited, ≥5xx→blocked) gating body reads.
- `references/raw-offset-rebase.md` — skip→rebase→offset post-passes make one-page raw fetches rank as if started at #35.
- `references/fpcheck-detection-kernel.md` — normalize→classify→escalate grammar turning five incompatible detector DOMs into one verdict map.
- `references/fpcheck-rebrowser-verdicts.md` — JSON+table fusion with last-write-wins keyed merge and emoji→rating fallback ladder.
- `references/yandex-neuro-answer-exclusion.md` — AI answer cards skipped from ranks but captured as ai_summary features with allowlisted citations.
