...
- `references/signal-core.md` — readSignal's symmetric observers/sourceSlots bookkeeping and last-observer fast path.
- `references/write-path.md` — writeSignal marks STALE vs PENDING by purity; never runs work inline; infinite-loop cap.
- `references/runtop-pull-ladder.md` — runTop/lookUpstream ancestor walk with ExecCount epoch: topo order without a topo sort.
- `references/ownership-disposal.md` — createComputation registration, cleanNode reverse cleanups, slot-swap unsubscription.
- `references/effect-tiers.md` — computed (sync) / renderEffect / user-effect tier contract + hydration effect stash.
- `references/batch-transitions.md` — outermost-runUpdates batching; transition tValue shadow commit protocol.
- `references/resource-fsm.md` — createResource five-state machine, `pr === p` stale-response guard, keep-last-value refresh.
- `references/error-boundaries.md` — symbol-keyed context handler lists, deferred synthetic-effect dispatch, handler bubbling.
- `references/map-array-diff.md` — mapArray prefix/suffix/backward-map identity diff; indexArray mirror.
- `references/time-slice-scheduler.md` — MessageChannel 5ms frame budget scheduler (enableScheduling substrate).
- `references/external-source-bridge.md` — composable factory chain wrapping computation fns; transition re-subscription fix.
- `references/store-proxy-traps.md` — store get/has traps: lazy per-property equals:false nodes, $TRACK/$RAW fast paths.
- `references/store-write-fanout.md` — setProperty ordered notification ($HAS → property → length → $SELF) + updatePath grammar.
- `references/reconcile-produce.md` — applyState keyed replace-vs-recurse diff; produce's WeakMap setter proxies.
- `references/create-mutable.md` — writable twin: real set/delete traps, batched built-in array methods, prototype-chain setters.
- `references/store-server-twin.md` — the signal-free server store: what survives compile-out (guards do).
- `references/ssr-shadow-kernel.md` — server reactive.ts deletion list: frozen memos, no-op effects, surviving owner tree.
- `references/ssr-streaming.md` — placeholder markers, registerFragment/block serialization, error replay markers.
- `references/lazy-component.md` — failure-uncaching load promise across client/hydrate/SSR modes.
- `references/show-switch-flow.md` — truthiness-equality condition memos + narrowed-accessor stale-read guards.
- `references/suspense-ladder.md` — counter context, parked effects resume, fallback-root flicker latch, SuspenseList orders.
- `references/error-boundary-component.md` — boundary-as-memo catching its own children via catchError.
- `references/merge-props.md` — last-wins skip-undefined props proxy with live getters; function sources become memos.
- `references/split-props.md` — first-group-wins claimed set + inverted rest view on both proxy and plain paths.
- `references/hydration-ids.md` — pre-order deterministic id grammar (`id + letter-width + digits`).
- `references/web-layer-surface.md` — Portal owner-anchored content, Dynamic typeof dispatch, dom-expressions boundary.
- `references/babel-preset-config.md` — JSX transform config surface, builtIns escape list, getter-props convention.
- `references/observable-bridge.md` — accessor→ES Observable: per-subscribe root+effect, untracked handler, owner-tied disposal.
- `references/from-producer-bridge.md` — producer→Accessor: two producer shapes, equals:false push semantics, no subscription sharing.
